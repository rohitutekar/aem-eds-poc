import { i as e, n as t, r as n, t as r } from "./jsx-runtime-CvRU0VyQ.js";
import { $ as i, A as a, B as o, C as s, D as c, E as l, F as u, G as d, H as f, I as p, J as m, K as h, L as g, M as _, N as v, O as y, P as b, Q as x, R as S, S as C, T as w, U as T, V as E, W as D, X as O, Y as k, Z as A, _ as j, a as M, at as N, b as P, c as F, ct as I, d as ee, dt as L, et as R, f as z, g as te, h as ne, i as B, it as re, j as ie, k as ae, l as oe, lt as V, m as H, n as se, nt as ce, o as le, ot as ue, p as de, q as U, r as W, rt as G, s as fe, st as pe, t as me, tt as he, u as ge, ut as _e, v as ve, w as ye, x as be, y as xe, z as Se } from "./src-aGDJs71C.js";
import { a as Ce, i as we, n as Te, o as Ee, r as De, t as Oe } from "./Button-BXgO-PT2.js";
//#region ../../packages/ui-react/src/brands/DiscoveryLogo.tsx
var K = r();
function ke({ title: e, height: t = 16, className: n, ...r }) {
	let i = e ? {
		role: "img",
		"aria-label": e
	} : {
		"aria-hidden": !0,
		focusable: !1
	};
	return /* @__PURE__ */ (0, K.jsxs)("svg", {
		viewBox: "0 0 140 10",
		xmlns: "http://www.w3.org/2000/svg",
		fill: "none",
		height: t,
		style: { width: "auto" },
		className: V("shrink-0", n),
		...i,
		...r,
		children: [e ? /* @__PURE__ */ (0, K.jsx)("title", { children: e }) : null, /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M13.5349 6.72338C13.4771 7.79485 13.2489 8.57974 12.6781 9.07385C12.106 9.56916 11.198 9.81712 9.95338 9.81712H0V5.00939V4.99069V0.182349H9.95338C11.198 0.182349 12.106 0.43091 12.6781 0.926223C13.2489 1.42033 13.4747 2.20463 13.5349 3.2767C13.5993 4.42599 13.5993 5.53125 13.5349 6.72338ZM2.77888 4.99069V5.00939V7.56499H8.53293C9.2543 7.56499 9.78237 7.43105 10.1166 7.16439C10.4508 6.89833 10.6163 6.47904 10.6163 5.90952C10.6242 5.60968 10.6296 5.30923 10.6302 5.00939V5.00456V4.99491V4.99069C10.6296 4.69024 10.6242 4.38979 10.6163 4.08995C10.6163 3.52043 10.4508 3.10174 10.1166 2.83508C9.78237 2.56902 9.2543 2.43448 8.53293 2.43448H2.77888V4.99069ZM59.68 0.0393656C58.2065 0.065911 57.1371 1.23029 57.001 2.67279C56.9155 3.45467 56.877 4.21544 56.8764 5.00155C56.8752 5.7786 56.9288 6.56109 57.001 7.32487C57.1142 8.76858 58.2071 9.92873 59.68 9.95769C62.6076 10.0144 65.5225 10.0138 68.4375 9.95769C69.9103 9.92934 70.9779 8.76617 71.1158 7.32487C71.1899 6.54902 71.2404 5.77317 71.2404 4.99612C71.2404 4.22027 71.182 3.44743 71.1158 2.67279C71.0068 1.22848 69.9097 0.065911 68.4375 0.0393656C65.5165 -0.0131219 62.6003 -0.0131219 59.68 0.0393656ZM60.4597 2.27944H63.9979H64.0214H67.6571C68.1334 2.27944 68.5224 2.68908 68.5224 3.18982V6.80603C68.5224 7.30677 68.1334 7.71581 67.6571 7.71581H64.0051H63.9979H60.4597C59.9834 7.71581 59.5944 7.30677 59.5944 6.80603V3.18982C59.5944 2.68908 59.9834 2.27944 60.4597 2.27944ZM130.4 9.81712H131.477H131.486H132.108H132.117H133.195V6.31434L140 0.182349H136.069L131.797 4.02118L127.525 0.182349H123.595L130.399 6.31434L130.4 9.81712ZM118.328 2.43931C118.351 2.4375 118.375 2.43629 118.399 2.43629C118.869 2.43629 119.252 2.82784 119.252 3.30747C119.252 3.78709 118.869 4.17864 118.399 4.17864C118.386 4.17864 118.373 4.17803 118.36 4.17743C118.351 4.17864 118.34 4.17924 118.331 4.17984H110.576V2.43629H118.313C118.318 2.4375 118.322 2.43871 118.328 2.43931ZM107.814 9.81712H110.576V6.44344H116.417V6.46215L119.238 9.81712H122.896L119.956 6.31313C121.248 5.92641 122.194 4.72584 122.194 3.31109C122.194 1.58503 120.786 0.182349 119.059 0.182349H107.814V9.81712ZM92.0613 9.81712H104.884L104.884 7.57826H94.8094V6.03742H103.527V3.97653H94.8064V2.4194H104.884L104.886 0.182349H92.0625L92.0613 9.81712ZM81.236 7.54146L86.3801 0.186572L89.7581 0.192002L82.8937 9.81712H81.236V9.8135H79.5783L72.7133 0.188382L76.0919 0.182349L81.236 7.54146ZM22.8512 6.64495H26.1112V6.80603C26.1112 7.17284 26.2178 7.41356 26.4165 7.55171C26.6586 7.71883 27.0674 7.78278 27.7183 7.78278L32.3638 7.78157C32.7841 7.78157 33.081 7.71762 33.2598 7.59395C33.4399 7.46906 33.5296 7.26454 33.5296 6.97616C33.5296 6.68235 33.426 6.4718 33.2189 6.3445C33.0136 6.21841 32.571 6.1074 31.8882 6.0893L26.3027 5.94692C24.9696 5.91254 24.0694 5.68811 23.6003 5.27726C23.1306 4.86641 22.8952 4.12072 22.8952 3.03477C22.8952 2.47551 22.9566 2.01458 23.0812 1.652C23.2059 1.28881 23.4046 0.987157 23.6792 0.747645C24.0019 0.476761 24.4246 0.284307 24.9467 0.170283C25.4681 0.0562582 26.3184 0.000150826 27.4937 0.000150826H32.6703C33.9529 0.000150826 34.8766 0.205878 35.439 0.615521C36.0038 1.02758 36.2868 1.69423 36.2868 2.61668C36.2868 2.67641 36.2838 2.74157 36.2814 2.81336C36.279 2.88274 36.2832 2.99676 36.2675 3.15362L33.0635 3.13432V3.0939C33.0635 2.77173 32.9696 2.5292 32.7835 2.36631C32.5987 2.20463 32.3181 2.12499 31.9442 2.12499L27.7147 2.12258C27.0632 2.12197 26.6333 2.17808 26.4243 2.29271C26.216 2.40673 26.1112 2.60582 26.1112 2.88998C26.1112 3.16991 26.2052 3.37202 26.3936 3.4963C26.5845 3.62239 26.9295 3.72737 27.4395 3.73642L33.2442 3.83596C34.5165 3.85828 35.4366 4.11831 35.9556 4.59069C36.4747 5.06248 36.7342 5.86849 36.7342 7.00995C36.7342 7.68806 36.6372 8.21173 36.447 8.58336C36.2561 8.9544 35.9327 9.27234 35.4823 9.53417C35.1843 9.70008 34.7772 9.81833 34.266 9.89254C33.7542 9.96373 33.0136 9.99992 32.0441 9.99992H27.5244C25.6717 9.99992 24.4325 9.81652 23.8038 9.45152C23.2721 9.14142 22.9873 8.60448 22.8958 7.83889C22.8633 7.56559 22.8573 7.28566 22.8482 7.00995C22.844 6.88205 22.8428 6.75596 22.8512 6.64495ZM53.9518 6.64072C53.9373 6.90859 53.9072 7.17706 53.8922 7.32668C53.744 8.76858 52.6765 9.96131 51.2024 9.96131L46.8224 9.99992L42.4419 9.96131C40.9684 9.96131 39.8677 8.7722 39.7623 7.32668C39.6461 5.74482 39.6208 4.25586 39.7623 2.67279C39.8906 1.22969 40.9678 0.0393656 42.4419 0.0393656L46.8224 0.000150826L51.2024 0.0393656C52.6765 0.0393656 53.7766 1.22788 53.8825 2.67279L53.9307 3.32858V3.33281H51.2879V3.31591V3.19103C51.2879 2.69029 50.8983 2.28064 50.422 2.28064H46.7851H46.7616H43.2222C42.7466 2.28064 42.3564 2.69029 42.3564 3.19103V6.80844C42.3564 7.30919 42.7466 7.71943 43.2222 7.71943H46.7616H46.7688H50.422C50.8983 7.71943 51.2879 7.30919 51.2879 6.80844V6.64072H53.9518ZM19.6966 9.81712H16.7341V0.182349H19.6972L19.6966 9.81712Z",
			fill: "currentColor"
		})]
	});
}
//#endregion
//#region ../../packages/ui-react/src/brands/JaguarLogo.tsx
function Ae({ title: e, height: t = 16, className: n, ...r }) {
	let i = e ? {
		role: "img",
		"aria-label": e
	} : {
		"aria-hidden": !0,
		focusable: !1
	};
	return /* @__PURE__ */ (0, K.jsxs)("svg", {
		viewBox: "0 0 130 16",
		xmlns: "http://www.w3.org/2000/svg",
		fill: "none",
		height: t,
		style: { width: "auto" },
		className: V("shrink-0", n),
		...i,
		...r,
		children: [e ? /* @__PURE__ */ (0, K.jsx)("title", { children: e }) : null, /* @__PURE__ */ (0, K.jsx)("path", {
			d: "M118.618 1.90032C119.872 0.662582 121.663 0 123.598 0C126.055 0 128.365 1.16872 129.712 3.2577L127.634 4.65648C126.583 3.2807 125.241 2.50769 123.561 2.50769C122.385 2.50769 121.275 2.86659 120.474 3.67641C119.724 4.43102 119.234 5.59514 119.234 7.28841V15.6213H116.67V6.95251C116.67 4.7163 117.419 3.08285 118.623 1.90032M99.74 13.4495C102.744 13.4495 105.183 11.1121 105.183 7.9786C105.183 4.84513 102.744 2.50769 99.74 2.50769C96.7364 2.50769 94.2789 4.84513 94.2789 7.9786C94.2789 11.1121 96.7364 13.4495 99.74 13.4495ZM91.6547 7.9786C91.6547 3.59359 95.2693 0 99.6984 0C101.883 0 103.984 0.878842 105.474 2.36045C106.858 3.73623 107.723 5.61815 107.723 7.997V15.6213H105.16V13.3023C104.09 14.7839 101.883 15.9526 99.6151 15.9526C95.1813 15.9526 91.6547 12.4464 91.6547 7.97399M71.2587 13.7808C70.1248 12.5891 69.4306 10.9602 69.4306 9.05989V0.335892H71.9946V8.7286C71.9946 10.2746 72.4805 11.3375 73.2349 12.1105C74.1004 13.0032 75.3129 13.4495 76.5116 13.4495C77.7103 13.4495 78.9229 13.0078 79.7883 12.1105C80.5381 11.3329 81.0286 10.2746 81.0286 8.7286V0.335892H83.5926V9.0645C83.5926 10.9648 82.8984 12.5891 81.7645 13.7854C80.4594 15.1612 78.5711 15.9572 76.5116 15.9572C74.4521 15.9572 72.5639 15.1612 71.2587 13.7854M45.2535 7.9786C45.2535 3.55218 48.7616 0 53.3202 0C56.2822 0 58.6148 1.21013 60.0402 3.2807L57.9206 4.67949C56.8469 3.2577 55.2548 2.50769 53.3202 2.50769C50.3814 2.50769 47.8776 4.57366 47.8776 7.9786C47.8776 11.1305 50.1454 13.4495 53.3619 13.4495C56.0693 13.4495 58.152 11.7976 58.675 9.37738H55.2085V6.93411H61.2806C61.3222 7.24699 61.3639 7.60129 61.3639 7.9786C61.3639 12.3406 57.8558 15.9572 53.3202 15.9572C48.7847 15.9572 45.2535 12.428 45.2535 7.9786ZM29.2079 13.4495C32.2115 13.4495 34.6505 11.1121 34.6505 7.9786C34.6505 4.84513 32.2115 2.50769 29.2079 2.50769C26.2043 2.50769 23.7467 4.84513 23.7467 7.9786C23.7467 11.1121 26.2043 13.4495 29.2079 13.4495ZM21.1226 7.9786C21.1134 3.59359 24.7279 0 29.157 0C31.3414 0 33.4426 0.878842 34.9328 2.36045C36.3213 3.73623 37.1821 5.61815 37.1821 7.997V15.6213H34.6181V13.3023C33.5444 14.7839 31.3414 15.9526 29.0737 15.9526C24.64 15.9526 21.1134 12.4464 21.1134 7.97399M0.00462809 12.6949L2.08264 11.2961C3.13322 12.6765 4.47537 13.4449 6.15536 13.4449C7.3309 13.4449 8.44164 13.086 9.2423 12.2762C9.99205 11.5216 10.4826 10.3574 10.4826 8.66418V0.335892H13.0466V9.00008C13.0466 11.2363 12.2968 12.8697 11.0935 14.0523C9.83932 15.29 8.04825 15.9526 6.11371 15.9526C3.65619 15.9526 1.34677 14.7839 0 12.6949",
			fill: "currentColor"
		})]
	});
}
//#endregion
//#region ../../packages/ui-react/src/brands/RangeRoverLogo.tsx
function je({ title: e, height: t = 16, className: n, ...r }) {
	let i = e ? {
		role: "img",
		"aria-label": e
	} : {
		"aria-hidden": !0,
		focusable: !1
	};
	return /* @__PURE__ */ (0, K.jsxs)("svg", {
		viewBox: "0 0 187 10",
		xmlns: "http://www.w3.org/2000/svg",
		fill: "none",
		height: t,
		style: { width: "auto" },
		className: V("shrink-0", n),
		...i,
		...r,
		children: [e ? /* @__PURE__ */ (0, K.jsx)("title", { children: e }) : null, /* @__PURE__ */ (0, K.jsxs)("g", { children: [
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M21.9286 5.82528L23.7516 2.60736L25.5734 5.82528H21.9298H21.9286ZM24.6563 0.00497437H22.8457C21.0003 3.28246 19.1574 6.55871 17.3096 9.8362H19.6774C20.0845 9.11269 20.4903 8.39539 20.8973 7.67189H26.601C27.0018 8.38298 27.4014 9.0916 27.8022 9.80269L27.8283 9.8362H30.1924C28.3483 6.55995 26.5054 3.28246 24.6563 0.00497437Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M62.8053 8.13597H57.7656C57.4988 8.13597 57.227 8.13597 56.9639 8.0702C56.6971 8.00691 56.4638 7.88033 56.2851 7.66439C56.1101 7.45218 56.0183 7.19653 55.9699 6.93096C55.9227 6.6629 55.9227 6.39485 55.9227 6.12307V3.87685C55.9227 3.60507 55.9227 3.34074 55.9699 3.07268C56.017 2.80835 56.1076 2.55146 56.2851 2.33925C56.4638 2.12331 56.6971 1.99673 56.9639 1.93344C57.227 1.87015 57.4988 1.86767 57.7656 1.86767H62.8053C63.1391 1.86767 63.4916 1.86767 63.8155 2.0017C64.153 2.13945 64.3752 2.38517 64.5067 2.72024H66.6065C66.5134 2.27348 66.3533 1.84037 66.1051 1.45441C65.852 1.06722 65.5268 0.749525 65.1297 0.516216C64.7326 0.281667 64.2982 0.147639 63.8465 0.0769016C63.3873 0.00616442 62.9456 0.00244141 62.4851 0.00244141H58.1056C57.5013 0.00244141 56.923 0.00616441 56.3248 0.162531C55.739 0.315174 55.2141 0.601846 54.7959 1.05233C54.3801 1.50281 54.1319 2.0501 53.9979 2.63957C53.8602 3.2427 53.8564 3.82473 53.8564 4.43406V5.5733C53.8564 6.18139 53.8602 6.76094 53.9979 7.36407C54.1319 7.95355 54.3801 8.50331 54.7959 8.95504C55.2116 9.41048 55.7378 9.69219 56.3248 9.84483C56.9217 10.0012 57.5013 10.0049 58.1056 10.0049H62.4851C63.0895 10.0049 63.6678 10.0012 64.266 9.84483C64.8517 9.69219 65.3767 9.40552 65.7949 8.95504C66.2106 8.50455 66.4588 7.95727 66.5929 7.36779C66.7306 6.76591 66.7331 6.18263 66.7331 5.5733V4.29507H59.9709V6.12679H64.6532C64.6532 6.39857 64.6532 6.6629 64.606 6.93096C64.5589 7.19529 64.4683 7.45218 64.2908 7.66439C64.1121 7.88033 63.8788 8.00691 63.612 8.0702C63.3489 8.13349 63.0771 8.13597 62.8103 8.13597H62.8066V8.1397L62.8053 8.13597Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M9.74932 3.83194C9.74932 3.9883 9.74932 4.14095 9.72325 4.29359C9.69719 4.44623 9.64507 4.59019 9.53338 4.70809C9.42169 4.8235 9.27649 4.88307 9.12633 4.90789C8.97369 4.93891 8.82229 4.93891 8.66592 4.93891H2.0551V2.02256H8.66468C8.82105 2.02256 8.97245 2.02256 9.12509 2.05358C9.27773 2.08461 9.42169 2.13797 9.52966 2.25338C9.64135 2.3688 9.69223 2.51772 9.71953 2.66788C9.74559 2.82052 9.74559 2.97316 9.74559 3.12953V3.83442H9.75056L9.74932 3.83194ZM11.308 6.43308C11.2075 6.32139 11.0958 6.23948 10.963 6.17123C10.829 6.10794 10.6912 6.06698 10.556 6.02231C10.8091 5.93792 11.0536 5.84733 11.2658 5.66862C11.478 5.48744 11.5972 5.25413 11.6704 4.99476C11.7424 4.74159 11.7672 4.46237 11.7821 4.20548C11.7945 3.93742 11.7945 3.67309 11.7945 3.40379V2.30178C11.7945 2.00022 11.7883 1.73216 11.7014 1.43804C11.6195 1.15509 11.4756 0.90193 11.2671 0.693442C11.0598 0.484953 10.8067 0.339756 10.5249 0.256609C10.2308 0.172221 9.96525 0.166016 9.66493 0.166016H0V9.83467H2.06379V6.80662H8.15711C8.37304 6.80662 8.62869 6.80662 8.84338 6.82524C8.95507 6.83765 9.07421 6.85626 9.18094 6.88853C9.29263 6.92203 9.39315 6.97292 9.47505 7.05606C9.55696 7.13797 9.60908 7.24221 9.64631 7.35018C9.67982 7.45939 9.69843 7.57728 9.7096 7.69022C9.73194 7.90615 9.72822 8.16428 9.72822 8.37897V9.83591H11.792V8.95976C11.792 8.40751 11.7982 7.76468 11.6729 7.21988C11.6096 6.94437 11.5066 6.65522 11.313 6.43432L11.3093 6.4306H11.308V6.43308Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M45.4235 7.80599V0.173828H47.4873V9.83628H44.422L44.3997 9.81145L37.7318 2.16564V9.83876H35.668V0.173828H38.7332L38.7543 0.194925L45.4235 7.80599Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M73.1064 9.83628V0.173828H84.2097V2.02044H75.1702V4.08795H83.2454V5.90602H75.1702V7.98594H84.2097V9.83628H73.1064Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M138.936 0.17395H141.303L145.373 7.40155L149.449 0.17395H151.816L146.375 9.8364H144.375L138.936 0.17395Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M113.068 3.83194C113.068 3.9883 113.068 4.14095 113.043 4.29359C113.017 4.44623 112.965 4.59019 112.854 4.70809C112.743 4.8235 112.599 4.88307 112.445 4.90789C112.292 4.93891 112.141 4.93891 111.984 4.93891H105.375V2.02256H111.984C112.141 2.02256 112.292 2.02256 112.445 2.05358C112.598 2.08461 112.741 2.13797 112.849 2.25338C112.96 2.3688 113.012 2.51772 113.038 2.66788C113.064 2.82052 113.064 2.97316 113.064 3.12953V3.83442H113.069L113.068 3.83194ZM114.623 6.43308C114.522 6.32139 114.412 6.23948 114.278 6.17123C114.144 6.10794 114.006 6.06698 113.868 6.02231C114.119 5.93792 114.366 5.84733 114.578 5.66862C114.789 5.48744 114.91 5.25413 114.981 4.99476C115.051 4.74159 115.077 4.46237 115.092 4.20548C115.104 3.93742 115.104 3.67309 115.104 3.40379V2.30178C115.104 2.00022 115.101 1.73216 115.011 1.43804C114.929 1.15509 114.785 0.90193 114.577 0.693442C114.368 0.484953 114.117 0.339756 113.834 0.256609C113.539 0.172221 113.274 0.166016 112.974 0.166016H103.322V9.8297H105.385V6.80166H111.478C111.694 6.80166 111.95 6.80166 112.164 6.82027C112.276 6.83268 112.395 6.8513 112.502 6.88356C112.614 6.91707 112.712 6.96795 112.796 7.0511C112.878 7.133 112.93 7.23725 112.967 7.34522C113.001 7.45442 113.019 7.57232 113.031 7.68525C113.053 7.90119 113.049 8.15931 113.049 8.37401V9.83095H115.109V8.9548C115.109 8.40255 115.116 7.75971 114.99 7.21491C114.927 6.93941 114.824 6.64777 114.63 6.42936H114.624V6.43308H114.623Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M184.108 3.83194C184.108 3.9883 184.108 4.14095 184.084 4.29359C184.059 4.44623 184.007 4.59019 183.894 4.70809C183.783 4.8235 183.639 4.88307 183.484 4.90789C183.331 4.93891 183.18 4.93891 183.024 4.93891H176.415V2.02256H183.024C183.18 2.02256 183.331 2.02256 183.484 2.05358C183.638 2.08461 183.782 2.13797 183.889 2.25338C184 2.3688 184.051 2.51772 184.08 2.66788C184.105 2.82052 184.105 2.97316 184.105 3.12953V3.83442H184.108V3.83194ZM185.663 6.43308C185.563 6.32139 185.451 6.23948 185.318 6.17123C185.183 6.10794 185.048 6.06698 184.909 6.02231C185.161 5.93792 185.406 5.84733 185.619 5.66862C185.831 5.48744 185.95 5.25413 186.023 4.99476C186.093 4.74159 186.119 4.46237 186.135 4.20548C186.147 3.93742 186.147 3.67309 186.147 3.40379V2.30178C186.147 2.00022 186.144 1.73216 186.054 1.43804C185.971 1.15509 185.827 0.90193 185.62 0.693442C185.413 0.484953 185.159 0.339756 184.879 0.256609C184.584 0.172221 184.319 0.166016 184.018 0.166016H174.365V9.8297H176.43V6.80166H182.522C182.738 6.80166 182.994 6.80166 183.207 6.82027C183.319 6.83268 183.438 6.8513 183.546 6.88356C183.655 6.91707 183.756 6.96795 183.839 7.0511C183.921 7.133 183.972 7.23725 184.01 7.34522C184.044 7.45442 184.062 7.57232 184.072 7.68525C184.095 7.90119 184.091 8.15931 184.091 8.37401V9.83095H186.151V8.9548C186.151 8.40255 186.157 7.75971 186.032 7.21491C185.97 6.93941 185.867 6.64777 185.672 6.42936H185.666V6.43308H185.663Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M132.264 6.2844C132.264 6.54625 132.264 6.787 132.198 7.04017C132.135 7.28961 132.016 7.51423 131.834 7.69914C131.653 7.88281 131.426 8.0007 131.178 8.064C130.921 8.12977 130.68 8.12977 130.425 8.12977H125.385C125.126 8.12977 124.885 8.12977 124.632 8.064C124.382 8.0007 124.16 7.88281 123.975 7.69914C123.789 7.51795 123.674 7.28961 123.61 7.04017C123.545 6.78452 123.545 6.54128 123.545 6.2844V3.71428C123.545 3.45243 123.545 3.21167 123.61 2.95851C123.674 2.70907 123.792 2.48445 123.975 2.29954C124.156 2.11587 124.382 1.99797 124.632 1.93468C124.887 1.86891 125.129 1.86891 125.385 1.86891H130.425C130.684 1.86891 130.925 1.86891 131.178 1.93468C131.427 1.99797 131.65 2.11587 131.834 2.29954C132.016 2.48321 132.136 2.70907 132.198 2.95851C132.264 3.21416 132.264 3.45739 132.264 3.71428V6.2844ZM134.305 3.36059C134.283 3.0702 134.242 2.78353 134.164 2.50058C134.012 1.93592 133.734 1.42091 133.314 1.00393C132.894 0.585713 132.379 0.311451 131.815 0.162531C131.533 0.0880706 131.246 0.0471175 130.957 0.0247795C130.668 0.00244141 130.379 0.00244141 130.088 0.00244141H125.715C125.425 0.00244141 125.137 0.00244141 124.846 0.0247795C124.556 0.0471175 124.271 0.0880706 123.988 0.162531C123.424 0.311451 122.908 0.586954 122.488 1.00393C122.069 1.42215 121.791 1.93592 121.638 2.50058C121.56 2.78353 121.519 3.06524 121.498 3.36059C121.476 3.65099 121.476 3.94138 121.476 4.23178V5.76938C121.476 6.05978 121.476 6.35017 121.498 6.64056C121.521 6.93096 121.561 7.21763 121.638 7.50058C121.791 8.06648 122.069 8.58025 122.488 8.99723C122.908 9.41545 123.424 9.68971 123.988 9.83863C124.269 9.91309 124.556 9.95404 124.846 9.97638C125.136 9.99872 125.425 9.99872 125.715 9.99872H130.088C130.378 9.99872 130.667 9.99872 130.957 9.97638C131.247 9.95404 131.533 9.91309 131.815 9.83863C132.379 9.68971 132.893 9.41421 133.314 8.99723C133.732 8.57901 134.011 8.06524 134.164 7.50058C134.242 7.21763 134.283 6.93592 134.305 6.64056C134.328 6.35017 134.328 6.05978 134.328 5.76938V4.23178C134.328 3.94138 134.328 3.65099 134.305 3.36059Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M156.89 9.8364V0.17395H167.997V2.02056H158.951V4.08807H167.03V5.90614H158.951V7.98606H167.997V9.8364H156.89Z",
				fill: "currentColor"
			})
		] })]
	});
}
//#endregion
//#region ../../packages/ui-react/src/brands/BrandLogo.tsx
function Me({ brand: e, ...t }) {
	switch (e) {
		case "range-rover": return /* @__PURE__ */ (0, K.jsx)(je, { ...t });
		case "jaguar": return /* @__PURE__ */ (0, K.jsx)(Ae, { ...t });
		case "discovery": return /* @__PURE__ */ (0, K.jsx)(ke, { ...t });
	}
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/constants.mjs
var Ne = "react-aria-clear-focus", Pe = "react-aria-focus";
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/focus/virtualFocus.mjs
function Fe(e) {
	let t = Re(T(e));
	t !== e && (t && Ie(t, e), e && Le(e, t));
}
function Ie(e, t) {
	e.dispatchEvent(new FocusEvent("blur", { relatedTarget: t })), e.dispatchEvent(new FocusEvent("focusout", {
		bubbles: !0,
		relatedTarget: t
	}));
}
function Le(e, t) {
	e.dispatchEvent(new FocusEvent("focus", { relatedTarget: t })), e.dispatchEvent(new FocusEvent("focusin", {
		bubbles: !0,
		relatedTarget: t
	}));
}
function Re(e) {
	let t = o(e), n = t?.getAttribute("aria-activedescendant");
	return n && e.getElementById(n) || t;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/keyboard.mjs
function ze(e) {
	return ie() ? e.metaKey : e.ctrlKey;
}
var Be = /* @__PURE__ */ new Set([
	"checkbox",
	"radio",
	"range",
	"color",
	"file",
	"image",
	"button",
	"submit",
	"reset"
]);
function Ve(e) {
	return e instanceof HTMLInputElement && !Be.has(e.type) || e instanceof HTMLTextAreaElement || e instanceof HTMLElement && e.isContentEditable;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useEvent.mjs
var q = /* @__PURE__ */ e(t(), 1);
function He(e, t, n, r) {
	let i = be(n), a = n == null;
	(0, q.useEffect)(() => {
		if (a || !e.current) return;
		let n = e.current;
		return n.addEventListener(t, i, r), () => {
			n.removeEventListener(t, i, r);
		};
	}, [
		e,
		t,
		r,
		a
	]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useLabels.mjs
function Ue(e, t) {
	let { id: n, "aria-label": r, "aria-labelledby": i } = e;
	return n = G(n), i && r ? i = [.../* @__PURE__ */ new Set([n, ...i.trim().split(/\s+/)])].join(" ") : i &&= i.trim().split(/\s+/).join(" "), !r && !i && t && (r = t), {
		id: n,
		"aria-label": r,
		"aria-labelledby": i
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/i18n/utils.mjs
var We = /* @__PURE__ */ new Set([
	"Arab",
	"Syrc",
	"Samr",
	"Mand",
	"Thaa",
	"Mend",
	"Nkoo",
	"Adlm",
	"Rohg",
	"Hebr"
]), Ge = /* @__PURE__ */ new Set([
	"ae",
	"ar",
	"arc",
	"bcc",
	"bqi",
	"ckb",
	"dv",
	"fa",
	"glk",
	"he",
	"ku",
	"mzn",
	"nqo",
	"pnb",
	"ps",
	"sd",
	"ug",
	"ur",
	"yi"
]);
function Ke(e) {
	if (Intl.Locale) {
		let t = new Intl.Locale(e).maximize(), n = typeof t.getTextInfo == "function" ? t.getTextInfo() : t.textInfo;
		if (n) return n.direction === "rtl";
		if (t.script) return We.has(t.script);
	}
	let t = e.split("-")[0];
	return Ge.has(t);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/i18n/useDefaultLocale.mjs
var qe = Symbol.for("react-aria.i18n.locale");
function Je() {
	let e = typeof window < "u" && window[qe] || typeof navigator < "u" && (navigator.language || navigator.userLanguage) || "en-US";
	try {
		Intl.DateTimeFormat.supportedLocalesOf([e]);
	} catch {
		e = "en-US";
	}
	return {
		locale: e,
		direction: Ke(e) ? "rtl" : "ltr"
	};
}
var Ye = Je(), Xe = /* @__PURE__ */ new Set();
function Ze() {
	Ye = Je();
	for (let e of Xe) e(Ye);
}
function Qe() {
	let e = re(), [t, n] = (0, q.useState)(Ye);
	return (0, q.useEffect)(() => (Xe.size === 0 && window.addEventListener("languagechange", Ze), Xe.add(n), () => {
		Xe.delete(n), Xe.size === 0 && window.removeEventListener("languagechange", Ze);
	}), []), e ? {
		locale: typeof window < "u" && window[qe] || "en-US",
		direction: "ltr"
	} : t;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/i18n/I18nProvider.mjs
var $e = /*#__PURE__*/ q.createContext(null);
function et() {
	let e = Qe();
	return (0, q.useContext)($e) || e;
}
//#endregion
//#region ../../node_modules/.pnpm/@internationalized+string@3.2.9/node_modules/@internationalized/string/dist/private/LocalizedStringDictionary.mjs
var tt = Symbol.for("react-aria.i18n.locale"), nt = Symbol.for("react-aria.i18n.strings"), rt = void 0, it = class e {
	constructor(e, t = "en-US") {
		this.strings = Object.fromEntries(Object.entries(e).filter(([, e]) => e)), this.defaultLocale = t;
	}
	getStringForLocale(e, t) {
		let n = this.getStringsForLocale(t)[e];
		if (!n) throw Error(`Could not find intl message ${e} in ${t} locale`);
		return n;
	}
	getStringsForLocale(e) {
		let t = this.strings[e];
		return t || (t = at(e, this.strings, this.defaultLocale), this.strings[e] = t), t;
	}
	static getGlobalDictionaryForPackage(t) {
		if (typeof window > "u") return null;
		let n = window[tt];
		if (rt === void 0) {
			let t = window[nt];
			if (!t) return null;
			rt = {};
			for (let r in t) rt[r] = new e({ [n]: t[r] }, n);
		}
		let r = rt?.[t];
		if (!r) throw Error(`Strings for package "${t}" were not included by LocalizedStringProvider. Please add it to the list passed to createLocalizedStringDictionary.`);
		return r;
	}
};
function at(e, t, n = "en-US") {
	if (t[e]) return t[e];
	let r = ot(e);
	if (t[r]) return t[r];
	for (let e in t) if (e.startsWith(r + "-")) return t[e];
	return t[n];
}
function ot(e) {
	return Intl.Locale ? new Intl.Locale(e).language : e.split("-")[0];
}
//#endregion
//#region ../../node_modules/.pnpm/@internationalized+string@3.2.9/node_modules/@internationalized/string/dist/private/LocalizedStringFormatter.mjs
var st = /* @__PURE__ */ new Map(), ct = /* @__PURE__ */ new Map(), lt = class {
	constructor(e, t) {
		this.locale = e, this.strings = t;
	}
	format(e, t) {
		let n = this.strings.getStringForLocale(e, this.locale);
		return typeof n == "function" ? n(t, this) : n;
	}
	plural(e, t, n = "cardinal") {
		let r = t["=" + e];
		if (r) return typeof r == "function" ? r() : r;
		let i = this.locale + ":" + n, a = st.get(i);
		return a || (a = new Intl.PluralRules(this.locale, { type: n }), st.set(i, a)), r = t[a.select(e)] || t.other, typeof r == "function" ? r() : r;
	}
	number(e) {
		let t = ct.get(this.locale);
		return t || (t = new Intl.NumberFormat(this.locale), ct.set(this.locale, t)), t.format(e);
	}
	select(e, t) {
		let n = e[t] || e.other;
		return typeof n == "function" ? n() : n;
	}
}, ut = /* @__PURE__ */ new WeakMap();
function dt(e) {
	let t = ut.get(e);
	return t || (t = new it(e), ut.set(e, t)), t;
}
function ft(e, t) {
	return t && it.getGlobalDictionaryForPackage(t) || dt(e);
}
function pt(e, t) {
	let { locale: n } = et(), r = ft(e, t);
	return (0, q.useMemo)(() => new lt(n, r), [n, r]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/utils/useControlledState.mjs
var mt = typeof document < "u" ? q.useInsertionEffect ?? q.useLayoutEffect : () => {};
function ht(e, t, n) {
	let [r, i] = (0, q.useState)(e || t), a = (0, q.useRef)(r), o = (0, q.useRef)(e !== void 0), s = e !== void 0;
	(0, q.useEffect)(() => {
		o.current, o.current = s;
	}, [s]);
	let c = s ? e : r;
	mt(() => {
		a.current = c;
	});
	let [, l] = (0, q.useReducer)(() => ({}), {});
	return [c, (0, q.useCallback)((e, ...t) => {
		let r = typeof e == "function" ? e(a.current) : e;
		Object.is(a.current, r) || (a.current = r, i(r), l(), n?.(r, ...t));
	}, [n])];
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Autocomplete.mjs
var gt = /*#__PURE__*/ (0, q.createContext)(null), _t = /*#__PURE__*/ (0, q.createContext)(null), vt = class {
	constructor(e) {
		this.value = null, this.level = 0, this.hasChildNodes = !1, this.rendered = null, this.textValue = "", this["aria-label"] = void 0, this.index = 0, this.parentKey = null, this.prevKey = null, this.nextKey = null, this.firstChildKey = null, this.lastChildKey = null, this.props = {}, this.colSpan = null, this.colIndex = null, this.type = this.constructor.type, this.key = e;
	}
	get childNodes() {
		throw Error("childNodes is not supported");
	}
	clone() {
		let e = new this.constructor(this.key);
		return e.value = this.value, e.level = this.level, e.hasChildNodes = this.hasChildNodes, e.rendered = this.rendered, e.textValue = this.textValue, e["aria-label"] = this["aria-label"], e.index = this.index, e.parentKey = this.parentKey, e.prevKey = this.prevKey, e.nextKey = this.nextKey, e.firstChildKey = this.firstChildKey, e.lastChildKey = this.lastChildKey, e.props = this.props, e.render = this.render, e.colSpan = this.colSpan, e.colIndex = this.colIndex, e;
	}
	filter(e, t, n) {
		let r = this.clone();
		return t.addDescendants(r, e), r;
	}
}, yt = class extends vt {
	filter(e, t, n) {
		let [r, i] = Ct(e, t, this.firstChildKey, n), a = this.clone();
		return a.firstChildKey = r, a.lastChildKey = i, a;
	}
};
(class extends vt {
	static {
		this.type = "header";
	}
});
var bt = class extends vt {
	static {
		this.type = "loader";
	}
}, xt = class extends yt {
	static {
		this.type = "item";
	}
	filter(e, t, n) {
		if (n(this.textValue, this)) {
			let n = this.clone();
			return t.addDescendants(n, e), n;
		}
		return null;
	}
};
(class extends yt {
	static {
		this.type = "section";
	}
	filter(e, t, n) {
		let r = super.filter(e, t, n);
		if (r && r.lastChildKey !== null) {
			let t = e.getItem(r.lastChildKey);
			if (t && t.type !== "header") return r;
		}
		return null;
	}
});
var St = class {
	get size() {
		return this.itemCount;
	}
	getKeys() {
		return this.keyMap.keys();
	}
	*[Symbol.iterator]() {
		let e = this.firstKey == null ? void 0 : this.keyMap.get(this.firstKey);
		for (; e;) yield e, e = e.nextKey == null ? void 0 : this.keyMap.get(e.nextKey);
	}
	getChildren(e) {
		let t = this.keyMap;
		return { *[Symbol.iterator]() {
			let n = t.get(e), r = n?.firstChildKey == null ? null : t.get(n.firstChildKey);
			for (; r;) yield r, r = r.nextKey == null ? void 0 : t.get(r.nextKey);
		} };
	}
	getKeyBefore(e) {
		let t = this.keyMap.get(e);
		if (!t) return null;
		if (t.prevKey != null) {
			for (t = this.keyMap.get(t.prevKey); t && t.type !== "item" && t.lastChildKey != null;) t = this.keyMap.get(t.lastChildKey);
			return t?.key ?? null;
		}
		return t.parentKey;
	}
	getKeyAfter(e) {
		let t = this.keyMap.get(e);
		if (!t) return null;
		if (t.type !== "item" && t.firstChildKey != null) return t.firstChildKey;
		for (; t;) {
			if (t.nextKey != null) return t.nextKey;
			if (t.parentKey != null) t = this.keyMap.get(t.parentKey);
			else return null;
		}
		return null;
	}
	getFirstKey() {
		return this.firstKey;
	}
	getLastKey() {
		let e = this.lastKey == null ? null : this.keyMap.get(this.lastKey);
		for (; e?.lastChildKey != null;) e = this.keyMap.get(e.lastChildKey);
		return e?.key ?? null;
	}
	getItem(e) {
		return this.keyMap.get(e) ?? null;
	}
	at() {
		throw Error("Not implemented");
	}
	clone() {
		let e = this.constructor, t = new e();
		return t.keyMap = new Map(this.keyMap), t.firstKey = this.firstKey, t.lastKey = this.lastKey, t.itemCount = this.itemCount, t;
	}
	addNode(e) {
		if (this.frozen) throw Error("Cannot add a node to a frozen collection");
		e.type === "item" && this.keyMap.get(e.key) == null && this.itemCount++, this.keyMap.set(e.key, e);
	}
	addDescendants(e, t) {
		this.addNode(e);
		let n = t.getChildren(e.key);
		for (let e of n) this.addDescendants(e, t);
	}
	removeNode(e) {
		if (this.frozen) throw Error("Cannot remove a node to a frozen collection");
		let t = this.keyMap.get(e);
		t != null && t.type === "item" && this.itemCount--, this.keyMap.delete(e);
	}
	commit(e, t, n = !1) {
		if (this.frozen) throw Error("Cannot commit a frozen collection");
		this.firstKey = e, this.lastKey = t, this.frozen = !n;
	}
	filter(e) {
		let t = new this.constructor(), [n, r] = Ct(this, t, this.firstKey, e);
		return t?.commit(n, r), t;
	}
	constructor() {
		this.keyMap = /* @__PURE__ */ new Map(), this.firstKey = null, this.lastKey = null, this.frozen = !1, this.itemCount = 0;
	}
};
function Ct(e, t, n, r) {
	if (n == null) return [null, null];
	let i = null, a = null, o = e.getItem(n);
	for (; o != null;) {
		let n = o.filter(e, t, r);
		n != null && (n.nextKey = null, a && (n.prevKey = a.key, a.nextKey = n.key), i ??= n, t.addNode(n), a = n), o = o.nextKey == null ? null : e.getItem(o.nextKey);
	}
	if (a && a.type === "separator") {
		let e = a.prevKey;
		t.removeNode(a.key), e == null ? a = null : (a = t.getItem(e), a.nextKey = null);
	}
	return [i?.key ?? null, a?.key ?? null];
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/collections/Document.mjs
var wt = class {
	constructor(e) {
		this._firstChild = null, this._lastChild = null, this._previousSibling = null, this._nextSibling = null, this._parentNode = null, this._minInvalidChildIndex = null, this.ownerDocument = e;
	}
	*[Symbol.iterator]() {
		let e = this.firstChild;
		for (; e;) yield e, e = e.nextSibling;
	}
	get firstChild() {
		return this._firstChild;
	}
	set firstChild(e) {
		this._firstChild = e, this.ownerDocument.markDirty(this);
	}
	get lastChild() {
		return this._lastChild;
	}
	set lastChild(e) {
		this._lastChild = e, this.ownerDocument.markDirty(this);
	}
	get previousSibling() {
		return this._previousSibling;
	}
	set previousSibling(e) {
		this._previousSibling = e, this.ownerDocument.markDirty(this);
	}
	get nextSibling() {
		return this._nextSibling;
	}
	set nextSibling(e) {
		this._nextSibling = e, this.ownerDocument.markDirty(this);
	}
	get parentNode() {
		return this._parentNode;
	}
	set parentNode(e) {
		this._parentNode = e, this.ownerDocument.markDirty(this);
	}
	get isConnected() {
		return this.parentNode?.isConnected || !1;
	}
	invalidateChildIndices(e) {
		(this._minInvalidChildIndex == null || !this._minInvalidChildIndex.isConnected || e.index < this._minInvalidChildIndex.index) && (this._minInvalidChildIndex = e, this.ownerDocument.markDirty(this));
	}
	updateChildIndices() {
		let e = this._minInvalidChildIndex;
		for (; e;) e.index = e.previousSibling ? e.previousSibling.index + 1 : 0, e = e.nextSibling;
		this._minInvalidChildIndex = null;
	}
	appendChild(e) {
		e.parentNode && e.parentNode.removeChild(e), this.firstChild ??= e, this.lastChild ? (this.lastChild.nextSibling = e, e.index = this.lastChild.index + 1, e.previousSibling = this.lastChild) : (e.previousSibling = null, e.index = 0), e.parentNode = this, e.nextSibling = null, this.lastChild = e, this.ownerDocument.markDirty(this), this.isConnected && this.ownerDocument.queueUpdate();
	}
	insertBefore(e, t) {
		if (t == null) return this.appendChild(e);
		e.parentNode && e.parentNode.removeChild(e), e.nextSibling = t, e.previousSibling = t.previousSibling, e.index = t.index - 1, this.firstChild === t ? this.firstChild = e : t.previousSibling && (t.previousSibling.nextSibling = e), t.previousSibling = e, e.parentNode = t.parentNode, this.invalidateChildIndices(e), this.isConnected && this.ownerDocument.queueUpdate();
	}
	removeChild(e) {
		e.parentNode === this && (this._minInvalidChildIndex === e && (this._minInvalidChildIndex = null), e.nextSibling && (this.invalidateChildIndices(e.nextSibling), e.nextSibling.previousSibling = e.previousSibling), e.previousSibling && (e.previousSibling.nextSibling = e.nextSibling), this.firstChild === e && (this.firstChild = e.nextSibling), this.lastChild === e && (this.lastChild = e.previousSibling), e.parentNode = null, e.nextSibling = null, e.previousSibling = null, e.index = 0, this.ownerDocument.markDirty(e), this.isConnected && this.ownerDocument.queueUpdate());
	}
	addEventListener() {}
	removeEventListener() {}
	get previousVisibleSibling() {
		let e = this.previousSibling;
		for (; e && e.isHidden;) e = e.previousSibling;
		return e;
	}
	get nextVisibleSibling() {
		let e = this.nextSibling;
		for (; e && e.isHidden;) e = e.nextSibling;
		return e;
	}
	get firstVisibleChild() {
		let e = this.firstChild;
		for (; e && e.isHidden;) e = e.nextSibling;
		return e;
	}
	get lastVisibleChild() {
		let e = this.lastChild;
		for (; e && e.isHidden;) e = e.previousSibling;
		return e;
	}
}, Tt = class e extends wt {
	constructor(e, t) {
		super(t), this.nodeType = 8, this.isMutated = !0, this._index = 0, this.isHidden = !1, this.node = null;
	}
	get index() {
		return this._index;
	}
	set index(e) {
		this._index = e, this.ownerDocument.markDirty(this);
	}
	get level() {
		return this.parentNode instanceof e ? this.parentNode.level + +(this.parentNode.node?.type === "item") : 0;
	}
	getMutableNode() {
		return this.node == null ? null : (this.isMutated ||= (this.node = this.node.clone(), !0), this.ownerDocument.markDirty(this), this.node);
	}
	updateNode() {
		let t = this.nextVisibleSibling, n = this.getMutableNode();
		if (n != null && (n.index = this.index, n.level = this.level, n.parentKey = this.parentNode instanceof e ? this.parentNode.node?.key ?? null : null, n.prevKey = this.previousVisibleSibling?.node?.key ?? null, n.nextKey = t?.node?.key ?? null, n.hasChildNodes = !!this.firstChild, n.firstChildKey = this.firstVisibleChild?.node?.key ?? null, n.lastChildKey = this.lastVisibleChild?.node?.key ?? null, (n.colSpan != null || n.colIndex != null) && t)) {
			let e = (n.colIndex ?? n.index) + (n.colSpan ?? 1);
			if (t.node != null && e !== t.node.colIndex) {
				let n = t.getMutableNode();
				n.colIndex = e;
			}
		}
	}
	setProps(e, t, n, r, i) {
		let a, { value: o, textValue: s, id: c, ...l } = e;
		if (this.node == null ? (a = new n(c ?? `react-aria-${++this.ownerDocument.nodeId}`), this.node = a) : a = this.getMutableNode(), l.ref = t, a.props = l, a.rendered = r, a.render = i, a.value = o, e["aria-label"] && (a["aria-label"] = e["aria-label"]), a.textValue = s || (typeof l.children == "string" ? l.children : "") || e["aria-label"] || "", c != null && c !== a.key) throw Error("Cannot change the id of an item");
		l.colSpan != null && (a.colSpan = l.colSpan), this.isConnected && this.ownerDocument.queueUpdate();
	}
	get style() {
		let e = this;
		return {
			get display() {
				return e.isHidden ? "none" : "";
			},
			set display(t) {
				let n = t === "none";
				if (e.isHidden !== n) {
					(e.parentNode?.firstVisibleChild === e || e.parentNode?.lastVisibleChild === e) && e.ownerDocument.markDirty(e.parentNode);
					let t = e.previousVisibleSibling, r = e.nextVisibleSibling;
					t && e.ownerDocument.markDirty(t), r && e.ownerDocument.markDirty(r), e.isHidden = n, e.ownerDocument.markDirty(e);
				}
			}
		};
	}
	hasAttribute() {}
	setAttribute() {}
	setAttributeNS() {}
	removeAttribute() {}
}, Et = class extends wt {
	constructor(e) {
		super(null), this.nodeType = 11, this.ownerDocument = this, this.dirtyNodes = /* @__PURE__ */ new Set(), this.isSSR = !1, this.nodeId = 0, this.nodesByProps = /* @__PURE__ */ new WeakMap(), this.nextCollection = null, this.subscriptions = /* @__PURE__ */ new Set(), this.queuedRender = !1, this.inSubscription = !1, this.collection = e, this.nextCollection = e;
	}
	get isConnected() {
		return !0;
	}
	createElement(e) {
		return new Tt(e, this);
	}
	getMutableCollection() {
		return this.nextCollection ||= this.collection.clone(), this.nextCollection;
	}
	markDirty(e) {
		this.dirtyNodes.add(e);
	}
	addNode(e) {
		if (e.isHidden || e.node == null) return;
		let t = this.getMutableCollection();
		if (!t.getItem(e.node.key)) for (let t of e) this.addNode(t);
		t.addNode(e.node);
	}
	removeNode(e) {
		for (let t of e) this.removeNode(t);
		e.node && this.getMutableCollection().removeNode(e.node.key);
	}
	getCollection() {
		return this.inSubscription ? this.collection : (this.queuedRender = !1, this.updateCollection(), this.collection);
	}
	updateCollection() {
		for (let e of this.dirtyNodes) e instanceof Tt && (!e.isConnected || e.isHidden) ? this.removeNode(e) : e.updateChildIndices();
		for (let e of this.dirtyNodes) e instanceof Tt ? (e.isConnected && !e.isHidden && (e.updateNode(), this.addNode(e)), e.node && this.dirtyNodes.delete(e), e.isMutated = !1) : this.dirtyNodes.delete(e);
		this.nextCollection && (this.nextCollection.commit(this.firstVisibleChild?.node?.key ?? null, this.lastVisibleChild?.node?.key ?? null, this.isSSR), this.isSSR || (this.collection = this.nextCollection, this.nextCollection = null));
	}
	queueUpdate() {
		if (!(this.dirtyNodes.size === 0 || this.queuedRender)) {
			this.queuedRender = !0, this.inSubscription = !0, this.isSSR || (this.collection = this.collection.clone());
			for (let e of this.subscriptions) e();
			this.inSubscription = !1;
		}
	}
	subscribe(e) {
		return this.subscriptions.add(e), () => this.subscriptions.delete(e);
	}
	resetAfterSSR() {
		this.isSSR && (this.isSSR = !1, this.firstChild = null, this.lastChild = null, this.nodeId = 0);
	}
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/collections/useCachedChildren.mjs
function Dt(e) {
	let { children: t, items: n, idScope: r, addIdAndValue: i, dependencies: a = [] } = e, o = (0, q.useMemo)(() => void 0, [t]), s = (0, q.useMemo)(() => /* @__PURE__ */ new WeakMap(), [...a, o]);
	return (0, q.useMemo)(() => {
		if (n && typeof t == "function") {
			let e = [];
			for (let a of n) {
				let n = Ot(a) ? a : null, o = n ? s.get(n) : null;
				if (!o) {
					o = t(a);
					let c = o.props.id ?? a?.key ?? a?.id;
					r != null && o.props.id == null && c != null && (c = r + ":" + c);
					let l = c ?? e.length;
					o = (0, q.cloneElement)(o, i ? {
						key: l,
						id: c,
						value: a
					} : { key: l }), n && s.set(n, o);
				}
				e.push(o);
			}
			return e;
		} else if (typeof t != "function") return t;
	}, [
		t,
		n,
		s,
		r,
		i
	]);
}
function Ot(e) {
	switch (typeof e) {
		case "object": return e != null;
		case "function":
		case "symbol": return !0;
		default: return !1;
	}
}
//#endregion
//#region ../../node_modules/.pnpm/use-sync-external-store@1.6.0_react@19.2.4/node_modules/use-sync-external-store/cjs/use-sync-external-store-shim.production.js
var kt = /* @__PURE__ */ n(((e) => {
	var n = t();
	function r(e, t) {
		return e === t && (e !== 0 || 1 / e == 1 / t) || e !== e && t !== t;
	}
	var i = typeof Object.is == "function" ? Object.is : r, a = n.useState, o = n.useEffect, s = n.useLayoutEffect, c = n.useDebugValue;
	function l(e, t) {
		var n = t(), r = a({ inst: {
			value: n,
			getSnapshot: t
		} }), i = r[0].inst, l = r[1];
		return s(function() {
			i.value = n, i.getSnapshot = t, u(i) && l({ inst: i });
		}, [
			e,
			n,
			t
		]), o(function() {
			return u(i) && l({ inst: i }), e(function() {
				u(i) && l({ inst: i });
			});
		}, [e]), c(n), n;
	}
	function u(e) {
		var t = e.getSnapshot;
		e = e.value;
		try {
			var n = t();
			return !i(e, n);
		} catch {
			return !0;
		}
	}
	function d(e, t) {
		return t();
	}
	var f = typeof window > "u" || window.document === void 0 || window.document.createElement === void 0 ? d : l;
	e.useSyncExternalStore = n.useSyncExternalStore === void 0 ? f : n.useSyncExternalStore;
})), At = /* @__PURE__ */ n(((e, t) => {
	t.exports = kt();
})), jt = /* @__PURE__ */ e(L(), 1), Mt = At(), Nt = /*#__PURE__*/ (0, q.createContext)(!1), Pt = /*#__PURE__*/ (0, q.createContext)(null);
function Ft(e) {
	if ((0, q.useContext)(Pt)) return e.content;
	let { collection: t, document: n } = zt(e.createCollection);
	return /*#__PURE__*/ q.createElement(q.Fragment, null, /*#__PURE__*/ q.createElement(Ce, null, /*#__PURE__*/ q.createElement(Pt.Provider, { value: n }, e.content)), /*#__PURE__*/ q.createElement(It, {
		render: e.children,
		collection: t
	}));
}
function It({ collection: e, render: t }) {
	return t(e);
}
function Lt(e, t, n) {
	let r = re(), i = (0, q.useRef)(r);
	return i.current = r, (0, Mt.useSyncExternalStore)(e, (0, q.useCallback)(() => i.current ? n() : t(), [t, n]));
}
var Rt = typeof q.useSyncExternalStore == "function" ? q.useSyncExternalStore : Lt;
function zt(e) {
	let [t] = (0, q.useState)(() => new Et(e?.() || new St()));
	return {
		collection: Rt((0, q.useCallback)((e) => t.subscribe(e), [t]), (0, q.useCallback)(() => {
			let e = t.getCollection();
			return t.isSSR && t.resetAfterSSR(), e;
		}, [t]), (0, q.useCallback)(() => (t.isSSR = !0, t.getCollection()), [t])),
		document: t
	};
}
var Bt = /*#__PURE__*/ (0, q.createContext)(null);
function Vt(e) {
	return class extends vt {
		static {
			this.type = e;
		}
	};
}
function Ht(e, t, n, r, i, a) {
	typeof e == "string" && (e = Vt(e));
	let o = (0, q.useCallback)((i) => {
		i?.setProps(t, n, e, r, a);
	}, [
		t,
		n,
		r,
		a,
		e
	]), s = (0, q.useContext)(Bt);
	if (s) {
		let o = s.ownerDocument.nodesByProps.get(t);
		return o || (o = s.ownerDocument.createElement(e.type), o.setProps(t, n, e, r, a), s.appendChild(o), s.ownerDocument.updateCollection(), s.ownerDocument.nodesByProps.set(t, o)), i ? /*#__PURE__*/ q.createElement(Bt.Provider, { value: o }, i) : null;
	}
	return /*#__PURE__*/ q.createElement(e.type, { ref: o }, i);
}
function Ut(e, t) {
	let n = ({ node: e }) => t(e.props, e.props.ref, e), r = (0, q.forwardRef)((r, i) => {
		let a = (0, q.useContext)(te);
		if (!(0, q.useContext)(Nt)) {
			if (t.length >= 3) throw Error(t.name + " cannot be rendered outside a collection.");
			return t(r, i);
		}
		return Ht(e, r, i, "children" in r ? r.children : null, null, (e) => /*#__PURE__*/ q.createElement(te.Provider, { value: a }, /*#__PURE__*/ q.createElement(n, { node: e })));
	});
	return r.displayName = t.name, r;
}
function Wt(e) {
	return Dt({
		...e,
		addIdAndValue: !0
	});
}
var Gt = /*#__PURE__*/ (0, q.createContext)(null);
function Kt(e) {
	let t = (0, q.useContext)(Gt), n = (t?.dependencies || []).concat(e.dependencies), r = e.idScope ?? t?.idScope, i = Wt({
		...e,
		idScope: r,
		dependencies: n
	});
	return (0, q.useContext)(Pt) && (i = /*#__PURE__*/ q.createElement(qt, null, i)), t = (0, q.useMemo)(() => ({
		dependencies: n,
		idScope: r
	}), [r, ...n]), /*#__PURE__*/ q.createElement(Gt.Provider, { value: t }, i);
}
function qt({ children: e }) {
	let t = (0, q.useContext)(Pt), n = (0, q.useMemo)(() => /*#__PURE__*/ q.createElement(Pt.Provider, { value: null }, /*#__PURE__*/ q.createElement(Nt.Provider, { value: !0 }, e)), [e]);
	return re() ? /*#__PURE__*/ q.createElement(Bt.Provider, { value: t }, n) : /*#__PURE__*/ (0, jt.createPortal)(n, t);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Collection.mjs
var Jt = /*#__PURE__*/ (0, q.createContext)(null), Yt = {
	CollectionRoot({ collection: e, renderDropIndicator: t }) {
		return Xt(e, null, t);
	},
	CollectionBranch({ collection: e, parent: t, renderDropIndicator: n }) {
		return Xt(e, t, n);
	}
};
function Xt(e, t, n) {
	return Dt({
		items: t ? e.getChildren(t.key) : e,
		dependencies: [n],
		children(t) {
			if (t.type === "content") return /*#__PURE__*/ q.createElement(q.Fragment, null);
			let r = t.render(t);
			return !n || t.type !== "item" ? r : /*#__PURE__*/ q.createElement(q.Fragment, null, n({
				type: "item",
				key: t.key,
				dropPosition: "before"
			}), r, Zt(e, t, n));
		}
	});
}
function Zt(e, t, n) {
	let r = t.key, i = e.getKeyAfter(r), a = i == null ? null : e.getItem(i);
	for (; a != null && a.type !== "item";) i = e.getKeyAfter(a.key), a = i == null ? null : e.getItem(i);
	let o = t.nextKey == null ? null : e.getItem(t.nextKey);
	for (; o != null && o.type !== "item";) o = o.nextKey == null ? null : e.getItem(o.nextKey);
	let s = [];
	if (o == null) {
		let r = t;
		for (; r?.type === "item" && (!a || r.parentKey !== a.parentKey && a.level < r.level);) {
			let t = n({
				type: "item",
				key: r.key,
				dropPosition: "after"
			});
			/*#__PURE__*/ (0, q.isValidElement)(t) && s.push(/*#__PURE__*/ (0, q.cloneElement)(t, { key: `${r.key}-after` })), r = r.parentKey == null ? null : e.getItem(r.parentKey);
		}
	}
	return s;
}
var Qt = /*#__PURE__*/ (0, q.createContext)(Yt);
function $t(e) {
	return (0, q.useMemo)(() => e == null ? null : /* @__PURE__ */ new Set([e]), [e]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Label.mjs
var en = /*#__PURE__*/ (0, q.createContext)({}), tn = /*#__PURE__*/ we(function(e, t) {
	[e, t] = h(e, t, en);
	let { elementType: n = "label", ...r } = e, i = O[n];
	return /*#__PURE__*/ q.createElement(i, {
		className: "react-aria-Label",
		...r,
		ref: t
	});
});
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/label/useLabel.mjs
function nn(e) {
	let { id: t, label: n, "aria-labelledby": r, "aria-label": i, labelElementType: a = "label" } = e;
	t = G(t);
	let o = G(), s = {};
	n && (r = r ? `${o} ${r}` : o, s = {
		id: o,
		htmlFor: a === "label" ? t : void 0
	});
	let c = Ue({
		id: t,
		"aria-label": i,
		"aria-labelledby": r
	});
	return {
		labelProps: s,
		fieldProps: c
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/utils/number.mjs
function rn(e, t = -Infinity, n = Infinity) {
	return Math.min(Math.max(e, t), n);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Heading.mjs
var an = /*#__PURE__*/ (0, q.createContext)({}), on = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	[e, t] = h(e, t, an);
	let { children: n, level: r = 3, className: i, ...a } = e, o = O[`h${r}`];
	return /*#__PURE__*/ q.createElement(o, {
		...a,
		ref: t,
		className: i ?? "react-aria-Heading"
	}, n);
}), sn = /*#__PURE__*/ (0, q.createContext)({}), cn = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	[e, t] = h(e, t, sn);
	let { elementType: n = "span", ...r } = e, i = O[n];
	return /*#__PURE__*/ q.createElement(i, {
		className: "react-aria-Text",
		...r,
		ref: t
	});
});
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/isScrollable.mjs
function ln(e, t) {
	if (!e) return !1;
	let n = window.getComputedStyle(e), r = document.scrollingElement || document.documentElement, i = /(auto|scroll)/.test(n.overflow + n.overflowX + n.overflowY);
	return e === r && n.overflow !== "hidden" && (i = !0), i && t && (i = e.scrollHeight !== e.clientHeight || e.scrollWidth !== e.clientWidth), i;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/getScrollParent.mjs
function un(e, t) {
	let n = e;
	for (ln(n, t) && (n = n.parentElement); n && !ln(n, t);) n = n.parentElement;
	return n || document.scrollingElement || document.documentElement;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/getScrollParents.mjs
function dn(e, t) {
	let n = [], r = document.scrollingElement || document.documentElement;
	for (; e && (ln(e, t) && n.push(e), e !== r);) e = e.parentElement;
	return n;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/scrollIntoView.mjs
function fn(e, t, n = {}) {
	let { block: r = "nearest", inline: i = "nearest" } = n;
	if (e === t) return;
	let a = e.scrollTop, o = e.scrollLeft, s = t.getBoundingClientRect(), c = e.getBoundingClientRect(), l = window.getComputedStyle(t), u = window.getComputedStyle(e), d = document.scrollingElement || document.documentElement, f = e === d, p = e === d ? 0 : c.top, m = e === d ? e.clientHeight : c.bottom, h = e === d ? 0 : c.left, g = e === d ? e.clientWidth : c.right, _ = parseFloat(l.scrollMarginTop) || 0, v = parseFloat(l.scrollMarginBottom) || 0, y = parseFloat(l.scrollMarginLeft) || 0, x = parseFloat(l.scrollMarginRight) || 0, S = parseFloat(u.scrollPaddingTop) || 0, C = parseFloat(u.scrollPaddingBottom) || 0, w = parseFloat(u.scrollPaddingLeft) || 0, T = parseFloat(u.scrollPaddingRight) || 0, E = parseFloat(u.borderTopWidth) || 0, D = parseFloat(u.borderBottomWidth) || 0, O = parseFloat(u.borderLeftWidth) || 0, k = parseFloat(u.borderRightWidth) || 0, A = s.top - _, j = s.bottom + v, M = s.left - y, N = s.right + x, P = e === d ? 0 : O + k, F = e === d ? 0 : E + D, I = e === d ? 0 : e.offsetWidth - e.clientWidth - P, ee = e === d ? 0 : e.offsetHeight - e.clientHeight - F, L = p + (f ? 0 : E) + S, R = m - (f ? 0 : D) - C - ee, z = h + (f ? 0 : O) + w, te = g - (f ? 0 : k) - T;
	u.direction === "rtl" && !b() ? z += I : te -= I;
	let ne = A < L || j > R, B = M < z || N > te;
	if (ne && r === "start") a += A - L;
	else if (ne && r === "center") a += (A + j) / 2 - (L + R) / 2;
	else if (ne && r === "end") a += j - R;
	else if (ne && r === "nearest") {
		let e = A - L, t = j - R;
		a += Math.abs(e) <= Math.abs(t) ? e : t;
	}
	if (B && i === "start") o += M - z;
	else if (B && i === "center") o += (M + N) / 2 - (z + te) / 2;
	else if (B && i === "end") o += N - te;
	else if (B && i === "nearest") {
		let e = M - z, t = N - te;
		o += Math.abs(e) <= Math.abs(t) ? e : t;
	}
	e.scrollTo({
		left: o,
		top: a
	});
}
function pn(e, t = {}) {
	let { containingElement: n } = t;
	if (e && e.isConnected) {
		let t = document.scrollingElement || document.documentElement;
		if (window.getComputedStyle(t).overflow !== "hidden") {
			let { left: t, top: r } = e.getBoundingClientRect();
			e?.scrollIntoView?.({ block: "nearest" });
			let { left: i, top: a } = e.getBoundingClientRect();
			(Math.abs(t - i) > 1 || Math.abs(r - a) > 1) && (n?.scrollIntoView?.({
				block: "center",
				inline: "center"
			}), e.scrollIntoView?.({ block: "nearest" }));
		} else {
			let { left: t, top: r } = e.getBoundingClientRect(), i = dn(e, !0);
			for (let t of i) fn(t, e);
			let { left: a, top: o } = e.getBoundingClientRect();
			if (Math.abs(t - a) > 1 || Math.abs(r - o) > 1) {
				i = n ? dn(n, !0) : [];
				for (let e of i) fn(e, n, {
					block: "center",
					inline: "center"
				});
				for (let t of dn(e, !0)) fn(t, e);
			}
		}
	}
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useDescription.mjs
var mn = 0, hn = /* @__PURE__ */ new Map();
function gn(e) {
	let [t, n] = (0, q.useState)();
	return N(() => {
		if (!e) return;
		let t = hn.get(e);
		if (t) n(t.element.id);
		else {
			let r = `react-aria-description-${mn++}`;
			n(r);
			let i = document.createElement("div");
			i.id = r, i.style.display = "none", i.textContent = e, document.body.appendChild(i), t = {
				refCount: 0,
				element: i
			}, hn.set(e, t);
		}
		return t.refCount++, () => {
			t && --t.refCount === 0 && (t.element.remove(), hn.delete(e));
		};
	}, [e]), { "aria-describedby": e ? t : void 0 };
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/visually-hidden/VisuallyHidden.mjs
var _n = {
	border: 0,
	clip: "rect(0 0 0 0)",
	clipPath: "inset(50%)",
	height: "1px",
	margin: "-1px",
	overflow: "hidden",
	padding: 0,
	position: "absolute",
	width: "1px",
	whiteSpace: "nowrap"
};
function vn(e = {}) {
	let { style: t, isFocusable: n } = e, [r, i] = (0, q.useState)(!1), { focusWithinProps: a } = oe({
		isDisabled: !n,
		onFocusWithinChange: (e) => i(e)
	}), o = (0, q.useMemo)(() => r ? t : t ? {
		..._n,
		...t
	} : _n, [r]);
	return { visuallyHiddenProps: {
		...a,
		style: o
	} };
}
function yn(e) {
	let { children: t, elementType: n = "div", isFocusable: r, style: i, ...a } = e, { visuallyHiddenProps: o } = vn(e);
	return /*#__PURE__*/ q.createElement(n, R(a, o), t);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/FieldError.mjs
var bn = /*#__PURE__*/ (0, q.createContext)(null), xn = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	return (0, q.useContext)(bn)?.isInvalid ? /*#__PURE__*/ q.createElement(Sn, {
		...e,
		ref: t
	}) : null;
}), Sn = /*#__PURE__*/ (0, q.forwardRef)((e, t) => {
	let n = (0, q.useContext)(bn), { elementType: r, ...i } = e, a = H(i, { global: !0 }), o = U({
		...i,
		defaultClassName: "react-aria-FieldError",
		defaultChildren: n.validationErrors.length === 0 ? void 0 : n.validationErrors.join(" "),
		values: n
	});
	return o.children == null ? null : /*#__PURE__*/ q.createElement(cn, {
		slot: "errorMessage",
		elementType: r,
		...a,
		...o,
		ref: t
	});
}), Cn = {
	badInput: !1,
	customError: !1,
	patternMismatch: !1,
	rangeOverflow: !1,
	rangeUnderflow: !1,
	stepMismatch: !1,
	tooLong: !1,
	tooShort: !1,
	typeMismatch: !1,
	valueMissing: !1,
	valid: !0
}, wn = {
	...Cn,
	customError: !0,
	valid: !1
}, Tn = {
	isInvalid: !1,
	validationDetails: Cn,
	validationErrors: []
}, En = (0, q.createContext)({}), Dn = "__reactAriaFormValidationState";
function On(e) {
	if (e.__reactAriaFormValidationState) {
		let { realtimeValidation: t, displayValidation: n, updateValidation: r, resetValidation: i, commitValidation: a } = e[Dn];
		return {
			realtimeValidation: t,
			displayValidation: n,
			updateValidation: r,
			resetValidation: i,
			commitValidation: a
		};
	}
	return kn(e);
}
function kn(e) {
	let { isInvalid: t, validationState: n, name: r, value: i, builtinValidation: a, validate: o, validationBehavior: s = "aria" } = e;
	n && (t ||= n === "invalid");
	let c = t === void 0 ? null : {
		isInvalid: t,
		validationErrors: [],
		validationDetails: wn
	}, l = (0, q.useMemo)(() => !o || i == null ? null : Mn(jn(o, i)), [o, i]);
	a?.validationDetails.valid && (a = void 0);
	let u = (0, q.useContext)(En), d = (0, q.useMemo)(() => r ? Array.isArray(r) ? r.flatMap((e) => An(u[e])) : An(u[r]) : [], [u, r]), [f, p] = (0, q.useState)(u), [m, h] = (0, q.useState)(!1);
	u !== f && (p(u), h(!1));
	let g = (0, q.useMemo)(() => Mn(m ? [] : d), [m, d]), _ = (0, q.useRef)(Tn), [v, y] = (0, q.useState)(Tn), b = (0, q.useRef)(Tn), x = () => {
		if (!S) return;
		C(!1);
		let e = l || a || _.current;
		Nn(e, b.current) || (b.current = e, y(e));
	}, [S, C] = (0, q.useState)(!1);
	return (0, q.useEffect)(x), {
		realtimeValidation: c || g || l || a || Tn,
		displayValidation: s === "native" ? c || g || v : c || g || l || a || v,
		updateValidation(e) {
			s === "aria" && !Nn(v, e) ? y(e) : _.current = e;
		},
		resetValidation() {
			let e = Tn;
			Nn(e, b.current) || (b.current = e, y(e)), s === "native" && C(!1), h(!0);
		},
		commitValidation() {
			s === "native" && C(!0), h(!0);
		}
	};
}
function An(e) {
	return e ? Array.isArray(e) ? e : [e] : [];
}
function jn(e, t) {
	if (typeof e == "function") {
		let n = e(t);
		if (n && typeof n != "boolean") return An(n);
	}
	return [];
}
function Mn(e) {
	return e.length ? {
		isInvalid: !0,
		validationErrors: e,
		validationDetails: wn
	} : null;
}
function Nn(e, t) {
	return e === t || !!e && !!t && e.isInvalid === t.isInvalid && e.validationErrors.length === t.validationErrors.length && e.validationErrors.every((e, n) => e === t.validationErrors[n]) && Object.entries(e.validationDetails).every(([e, n]) => t.validationDetails[e] === n);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Form.mjs
var Pn = /*#__PURE__*/ (0, q.createContext)(null);
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/label/useField.mjs
function Fn(e) {
	let { description: t, errorMessage: n, isInvalid: r, validationState: i } = e, { labelProps: a, fieldProps: o } = nn(e), s = ce([
		!!t,
		!!n,
		r,
		i
	]), c = ce([
		!!t,
		!!n,
		r,
		i
	]);
	return o = R(o, { "aria-describedby": [
		s,
		c,
		e["aria-describedby"]
	].filter(Boolean).join(" ") || void 0 }), {
		labelProps: a,
		fieldProps: o,
		descriptionProps: { id: s },
		errorMessageProps: { id: c }
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useFormReset.mjs
function In(e, t, n) {
	let r = be((e) => {
		n && !e.defaultPrevented && n(t);
	});
	(0, q.useEffect)(() => {
		let t = e?.current?.form;
		return t?.addEventListener("reset", r), () => {
			t?.removeEventListener("reset", r);
		};
	}, [e]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/form/useFormValidation.mjs
function Ln(e, t, n) {
	let { validationBehavior: r, focus: i } = e;
	N(() => {
		if (r === "native" && n?.current && "setCustomValidity" in n.current && !n.current.disabled) {
			let e = t.realtimeValidation.isInvalid ? t.realtimeValidation.validationErrors.join(" ") || "Invalid value." : "";
			n.current.setCustomValidity(e), n.current.hasAttribute("title") || (n.current.title = ""), t.realtimeValidation.isInvalid || t.updateValidation(zn(n.current));
		}
	});
	let a = (0, q.useRef)(!1), o = be(() => {
		a.current || t.resetValidation();
	}), c = be((e) => {
		t.displayValidation.isInvalid || t.commitValidation();
		let r = n?.current?.form;
		!e.defaultPrevented && n && r && Bn(r) === n.current && (i ? i() : n.current?.focus(), s("keyboard")), e.preventDefault();
	}), l = be(() => {
		t.commitValidation();
	});
	(0, q.useEffect)(() => {
		let e = n?.current;
		if (!e) return;
		let t = e.form, r = t?.reset;
		return t && (t.reset = () => {
			a.current = !window.event || window.event.type === "message" && E(window.event) instanceof MessagePort, r?.call(t), a.current = !1;
		}), e.addEventListener("invalid", c), e.addEventListener("change", l), t?.addEventListener("reset", o), () => {
			e.removeEventListener("invalid", c), e.removeEventListener("change", l), t?.removeEventListener("reset", o), t && (t.reset = r);
		};
	}, [n, r]);
}
function Rn(e) {
	let t = e.validity;
	return {
		badInput: t.badInput,
		customError: t.customError,
		patternMismatch: t.patternMismatch,
		rangeOverflow: t.rangeOverflow,
		rangeUnderflow: t.rangeUnderflow,
		stepMismatch: t.stepMismatch,
		tooLong: t.tooLong,
		tooShort: t.tooShort,
		typeMismatch: t.typeMismatch,
		valueMissing: t.valueMissing,
		valid: t.valid
	};
}
function zn(e) {
	return {
		isInvalid: !e.validity.valid,
		validationDetails: Rn(e),
		validationErrors: e.validationMessage ? [e.validationMessage] : []
	};
}
function Bn(e) {
	for (let t = 0; t < e.elements.length; t++) {
		let n = e.elements[t];
		if (n.validity?.valid === !1) return n;
	}
	return null;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/ar-AE.mjs
var Vn = {};
Vn = {
	colorSwatchPicker: "تغييرات الألوان",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "حدد عنصرًا",
	tableResizer: "أداة تغيير الحجم"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/bg-BG.mjs
var Hn = {};
Hn = {
	colorSwatchPicker: "Цветови мостри",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Изберете предмет",
	tableResizer: "Преоразмерител"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/cs-CZ.mjs
var Un = {};
Un = {
	colorSwatchPicker: "Vzorky barev",
	dropzoneLabel: "Místo pro přetažení",
	selectPlaceholder: "Vyberte položku",
	tableResizer: "Změna velikosti"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/da-DK.mjs
var Wn = {};
Wn = {
	colorSwatchPicker: "Farveprøver",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Vælg et element",
	tableResizer: "Størrelsesændring"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/de-DE.mjs
var Gn = {};
Gn = {
	colorSwatchPicker: "Farbfelder",
	dropzoneLabel: "Ablegebereich",
	selectPlaceholder: "Element wählen",
	tableResizer: "Größenanpassung"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/el-GR.mjs
var Kn = {};
Kn = {
	colorSwatchPicker: "Χρωματικά δείγματα",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Επιλέξτε ένα αντικείμενο",
	tableResizer: "Αλλαγή μεγέθους"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/en-US.mjs
var qn = {};
qn = {
	selectPlaceholder: "Select an item",
	tableResizer: "Resizer",
	dropzoneLabel: "DropZone",
	colorSwatchPicker: "Color swatches"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/es-ES.mjs
var Jn = {};
Jn = {
	colorSwatchPicker: "Muestras de colores",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Seleccionar un artículo",
	tableResizer: "Cambiador de tamaño"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/et-EE.mjs
var Yn = {};
Yn = {
	colorSwatchPicker: "Värvinäidised",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Valige üksus",
	tableResizer: "Suuruse muutja"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/fi-FI.mjs
var Xn = {};
Xn = {
	colorSwatchPicker: "Värimallit",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Valitse kohde",
	tableResizer: "Koon muuttaja"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/fr-FR.mjs
var Zn = {};
Zn = {
	colorSwatchPicker: "Échantillons de couleurs",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Sélectionner un élément",
	tableResizer: "Redimensionneur"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/he-IL.mjs
var Qn = {};
Qn = {
	colorSwatchPicker: "דוגמיות צבע",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "בחר פריט",
	tableResizer: "שינוי גודל"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/hr-HR.mjs
var $n = {};
$n = {
	colorSwatchPicker: "Uzorci boja",
	dropzoneLabel: "Zona spuštanja",
	selectPlaceholder: "Odaberite stavku",
	tableResizer: "Promjena veličine"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/hu-HU.mjs
var er = {};
er = {
	colorSwatchPicker: "Színtárak",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Válasszon ki egy elemet",
	tableResizer: "Átméretező"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/it-IT.mjs
var tr = {};
tr = {
	colorSwatchPicker: "Campioni di colore",
	dropzoneLabel: "Zona di rilascio",
	selectPlaceholder: "Seleziona un elemento",
	tableResizer: "Ridimensionamento"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/ja-JP.mjs
var nr = {};
nr = {
	colorSwatchPicker: "カラースウォッチ",
	dropzoneLabel: "ドロップゾーン",
	selectPlaceholder: "項目を選択",
	tableResizer: "サイズ変更ツール"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/ko-KR.mjs
var rr = {};
rr = {
	colorSwatchPicker: "색상 견본",
	dropzoneLabel: "드롭 영역",
	selectPlaceholder: "항목 선택",
	tableResizer: "크기 조정기"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/lt-LT.mjs
var ir = {};
ir = {
	colorSwatchPicker: "Spalvų pavyzdžiai",
	dropzoneLabel: "„DropZone“",
	selectPlaceholder: "Pasirinkite elementą",
	tableResizer: "Dydžio keitiklis"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/lv-LV.mjs
var ar = {};
ar = {
	colorSwatchPicker: "Krāsu paraugi",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Izvēlēties vienumu",
	tableResizer: "Izmēra mainītājs"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/nb-NO.mjs
var or = {};
or = {
	colorSwatchPicker: "Fargekart",
	dropzoneLabel: "Droppsone",
	selectPlaceholder: "Velg et element",
	tableResizer: "Størrelsesendrer"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/nl-NL.mjs
var sr = {};
sr = {
	colorSwatchPicker: "kleurstalen",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Selecteer een item",
	tableResizer: "Resizer"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/pl-PL.mjs
var cr = {};
cr = {
	colorSwatchPicker: "Próbki kolorów",
	dropzoneLabel: "Strefa upuszczania",
	selectPlaceholder: "Wybierz element",
	tableResizer: "Zmiana rozmiaru"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/pt-BR.mjs
var lr = {};
lr = {
	colorSwatchPicker: "Amostras de cores",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Selecione um item",
	tableResizer: "Redimensionador"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/pt-PT.mjs
var ur = {};
ur = {
	colorSwatchPicker: "Amostras de cores",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Selecione um item",
	tableResizer: "Redimensionador"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/ro-RO.mjs
var dr = {};
dr = {
	colorSwatchPicker: "Specimene de culoare",
	dropzoneLabel: "Zonă de plasare",
	selectPlaceholder: "Selectați un element",
	tableResizer: "Instrument de redimensionare"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/ru-RU.mjs
var fr = {};
fr = {
	colorSwatchPicker: "Цветовые образцы",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Выберите элемент",
	tableResizer: "Средство изменения размера"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/sk-SK.mjs
var pr = {};
pr = {
	colorSwatchPicker: "Vzorkovníky farieb",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Vyberte položku",
	tableResizer: "Nástroj na zmenu veľkosti"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/sl-SI.mjs
var mr = {};
mr = {
	colorSwatchPicker: "Barvne palete",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Izberite element",
	tableResizer: "Spreminjanje velikosti"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/sr-SP.mjs
var hr = {};
hr = {
	colorSwatchPicker: "Uzorci boje",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Izaberite stavku",
	tableResizer: "Promena veličine"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/sv-SE.mjs
var gr = {};
gr = {
	colorSwatchPicker: "Färgrutor",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Välj en artikel",
	tableResizer: "Storleksändrare"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/tr-TR.mjs
var _r = {};
_r = {
	colorSwatchPicker: "Renk örnekleri",
	dropzoneLabel: "Bırakma Bölgesi",
	selectPlaceholder: "Bir öğe seçin",
	tableResizer: "Yeniden boyutlandırıcı"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/uk-UA.mjs
var vr = {};
vr = {
	colorSwatchPicker: "Зразки кольорів",
	dropzoneLabel: "DropZone",
	selectPlaceholder: "Виберіть елемент",
	tableResizer: "Засіб змінення розміру"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/zh-CN.mjs
var yr = {};
yr = {
	colorSwatchPicker: "颜色色板",
	dropzoneLabel: "放置区域",
	selectPlaceholder: "选择一个项目",
	tableResizer: "尺寸调整器"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intl/zh-TW.mjs
var br = {};
br = {
	colorSwatchPicker: "色票",
	dropzoneLabel: "放置區",
	selectPlaceholder: "選取項目",
	tableResizer: "大小調整器"
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/intlStrings.mjs
var xr = {};
xr = {
	"ar-AE": Vn,
	"bg-BG": Hn,
	"cs-CZ": Un,
	"da-DK": Wn,
	"de-DE": Gn,
	"el-GR": Kn,
	"en-US": qn,
	"es-ES": Jn,
	"et-EE": Yn,
	"fi-FI": Xn,
	"fr-FR": Zn,
	"he-IL": Qn,
	"hr-HR": $n,
	"hu-HU": er,
	"it-IT": tr,
	"ja-JP": nr,
	"ko-KR": rr,
	"lt-LT": ir,
	"lv-LV": ar,
	"nb-NO": or,
	"nl-NL": sr,
	"pl-PL": cr,
	"pt-BR": lr,
	"pt-PT": ur,
	"ro-RO": dr,
	"ru-RU": fr,
	"sk-SK": pr,
	"sl-SI": mr,
	"sr-SP": hr,
	"sv-SE": gr,
	"tr-TR": _r,
	"uk-UA": vr,
	"zh-CN": yr,
	"zh-TW": br
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/DragAndDrop.mjs
var Sr = /*#__PURE__*/ (0, q.createContext)({}), Cr = /*#__PURE__*/ (0, q.createContext)(null), wr = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	let { render: n } = (0, q.useContext)(Cr);
	return /*#__PURE__*/ q.createElement(q.Fragment, null, n(e, t));
});
function Tr(e, t) {
	let n = e?.renderDropIndicator, r = e?.isVirtualDragging?.(), i = (0, q.useCallback)((e) => {
		if (r || t?.isDropTarget(e)) return n ? n(e) : /*#__PURE__*/ q.createElement(wr, { target: e });
	}, [
		t?.target,
		r,
		n
	]);
	return e?.useDropIndicator ? i : void 0;
}
function Er(e, t, n) {
	let r = e.focusedKey, i = null;
	if (t?.isVirtualDragging?.() && n?.target?.type === "item" && (i = n.target.key, n.target.dropPosition === "after")) {
		let e = n.collection.getKeyAfter(i), t = null;
		if (e != null) {
			let r = n.collection.getItem(i)?.level ?? 0;
			for (; e != null;) {
				let i = n.collection.getItem(e);
				if (!i) break;
				if (i.type !== "item") {
					e = n.collection.getKeyAfter(e);
					continue;
				}
				if ((i.level ?? 0) <= r) break;
				t = e, e = n.collection.getKeyAfter(e);
			}
		}
		i = e ?? t ?? i;
	}
	return (0, q.useMemo)(() => new Set([r, i].filter((e) => e != null)), [r, i]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Header.mjs
var Dr = /*#__PURE__*/ (0, q.createContext)({}), Or = /*#__PURE__*/ (0, q.createContext)(null);
function kr(e) {
	let t = (0, q.useRef)({});
	return /*#__PURE__*/ q.createElement(Or.Provider, { value: t }, e.children);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/SelectionIndicator.mjs
var Ar = /*#__PURE__*/ (0, q.createContext)({ isSelected: !1 }), jr = /*#__PURE__*/ (0, q.createContext)({});
(class extends vt {
	static {
		this.type = "separator";
	}
	filter(e, t) {
		let n = t.getItem(this.prevKey);
		if (n && n.type !== "separator") {
			let n = this.clone();
			return t.addDescendants(n, e), n;
		}
		return null;
	}
});
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/listbox/utils.mjs
var Mr = /* @__PURE__ */ new WeakMap();
function Nr(e) {
	return typeof e == "string" ? e.replace(/\s*/g, "") : "" + e;
}
function Pr(e, t) {
	let n = Mr.get(e);
	if (!n) throw Error("Unknown list");
	return `${n.id}-option-${Nr(t)}`;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/shadowdom/ShadowTreeWalker.mjs
var Fr = class {
	constructor(e, t, n, r) {
		this._walkerStack = [], this._currentSetFor = /* @__PURE__ */ new Set(), this._acceptNode = (e) => {
			if (e.nodeType === Node.ELEMENT_NODE) {
				let t = e.shadowRoot;
				if (t) {
					let e = this._doc.createTreeWalker(t, this.whatToShow, { acceptNode: this._acceptNode });
					return this._walkerStack.unshift(e), NodeFilter.FILTER_ACCEPT;
				} else if (typeof this.filter == "function") return this.filter(e);
				else if (this.filter?.acceptNode) return this.filter.acceptNode(e);
				else if (this.filter === null) return NodeFilter.FILTER_ACCEPT;
			}
			return NodeFilter.FILTER_SKIP;
		}, this._doc = e, this.root = t, this.filter = r ?? null, this.whatToShow = n ?? NodeFilter.SHOW_ALL, this._currentNode = t, this._walkerStack.unshift(e.createTreeWalker(t, n, this._acceptNode));
		let i = t.shadowRoot;
		if (i) {
			let e = this._doc.createTreeWalker(i, this.whatToShow, { acceptNode: this._acceptNode });
			this._walkerStack.unshift(e);
		}
	}
	get currentNode() {
		return this._currentNode;
	}
	set currentNode(e) {
		if (!S(this.root, e)) throw Error("Cannot set currentNode to a node that is not contained by the root node.");
		let t = [], n = e, r = e;
		for (this._currentNode = e; n && n !== this.root;) if (n.nodeType === Node.DOCUMENT_FRAGMENT_NODE) {
			let e = n, i = this._doc.createTreeWalker(e, this.whatToShow, { acceptNode: this._acceptNode });
			t.push(i), i.currentNode = r, this._currentSetFor.add(i), n = r = e.host;
		} else n = n.parentNode;
		let i = this._doc.createTreeWalker(this.root, this.whatToShow, { acceptNode: this._acceptNode });
		t.push(i), i.currentNode = r, this._currentSetFor.add(i), this._walkerStack = t;
	}
	get doc() {
		return this._doc;
	}
	firstChild() {
		let e = this.currentNode, t = this.nextNode();
		return S(e, t) ? (t && (this.currentNode = t), t) : (this.currentNode = e, null);
	}
	lastChild() {
		let e = this._walkerStack[0].lastChild();
		return e && (this.currentNode = e), e;
	}
	nextNode() {
		let e = this._walkerStack[0].nextNode();
		if (e) {
			if (e.shadowRoot) {
				let t;
				if (typeof this.filter == "function" ? t = this.filter(e) : this.filter?.acceptNode && (t = this.filter.acceptNode(e)), t === NodeFilter.FILTER_ACCEPT) return this.currentNode = e, e;
				let n = this.nextNode();
				return n && (this.currentNode = n), n;
			}
			return e && (this.currentNode = e), e;
		} else if (this._walkerStack.length > 1) {
			this._walkerStack.shift();
			let e = this.nextNode();
			return e && (this.currentNode = e), e;
		} else return null;
	}
	previousNode() {
		let e = this._walkerStack[0];
		if (e.currentNode === e.root) {
			if (this._currentSetFor.has(e)) if (this._currentSetFor.delete(e), this._walkerStack.length > 1) {
				this._walkerStack.shift();
				let e = this.previousNode();
				return e && (this.currentNode = e), e;
			} else return null;
			return null;
		}
		let t = e.previousNode();
		if (t) {
			if (t.shadowRoot) {
				let e;
				if (typeof this.filter == "function" ? e = this.filter(t) : this.filter?.acceptNode && (e = this.filter.acceptNode(t)), e === NodeFilter.FILTER_ACCEPT) return t && (this.currentNode = t), t;
				let n = this.lastChild();
				return n && (this.currentNode = n), n;
			}
			return t && (this.currentNode = t), t;
		} else if (this._walkerStack.length > 1) {
			this._walkerStack.shift();
			let e = this.previousNode();
			return e && (this.currentNode = e), e;
		} else return null;
	}
	nextSibling() {
		return null;
	}
	previousSibling() {
		return null;
	}
	parentNode() {
		return null;
	}
};
function Ir(e, t, n, r) {
	return f() ? new Fr(e, t, n, r) : e.createTreeWalker(t, n, r);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/focus/FocusScope.mjs
var Lr = /*#__PURE__*/ q.createContext(null), Rr = "react-aria-focus-scope-restore", J = null;
function zr(e) {
	let { children: t, contain: n, restoreFocus: r, autoFocus: i } = e, a = (0, q.useRef)(null), s = (0, q.useRef)(null), c = (0, q.useRef)([]), { parentNode: l } = (0, q.useContext)(Lr) || {}, u = (0, q.useMemo)(() => new oi({ scopeRef: c }), [c]);
	N(() => {
		let e = l || X.root;
		if (X.getTreeNode(e.scopeRef) && J && !Yr(J, e.scopeRef)) {
			let t = X.getTreeNode(J);
			t && (e = t);
		}
		e.addChild(u), X.addNode(u);
	}, [u, l]), N(() => {
		let e = X.getTreeNode(c);
		e && (e.contain = !!n);
	}, [n]), N(() => {
		let e = a.current?.nextSibling, t = [], n = (e) => e.stopPropagation();
		for (; e && e !== s.current;) t.push(e), e.addEventListener(Rr, n), e = e.nextSibling;
		return c.current = t, () => {
			for (let e of t) e.removeEventListener(Rr, n);
		};
	}, [t]), ei(c, r, n), Gr(c, n), ni(c, r, n), $r(c, i), (0, q.useEffect)(() => {
		let e = o(T(c.current ? c.current[0] : void 0)), t = null;
		if (Y(e, c.current)) {
			for (let n of X.traverse()) n.scopeRef && Y(e, n.scopeRef.current) && (t = n);
			t === X.getTreeNode(c) && (J = t.scopeRef);
		}
	}, [c]), N(() => () => {
		let e = X.getTreeNode(c)?.parent?.scopeRef ?? null;
		(c === J || Yr(c, J)) && (!e || X.getTreeNode(e)) && (J = e), X.removeTreeNode(c);
	}, [c]);
	let d = (0, q.useMemo)(() => Br(c), []), f = (0, q.useMemo)(() => ({
		focusManager: d,
		parentNode: u
	}), [u, d]);
	return /*#__PURE__*/ q.createElement(Lr.Provider, { value: f }, /*#__PURE__*/ q.createElement("span", {
		"data-focus-scope-start": !0,
		hidden: !0,
		ref: a
	}), t, /*#__PURE__*/ q.createElement("span", {
		"data-focus-scope-end": !0,
		hidden: !0,
		ref: s
	}));
}
function Br(e) {
	return {
		focusNext(t = {}) {
			let n = e.current, { from: r, tabbable: i, wrap: a, accept: s } = t, c = r || o(T(n[0] ?? void 0)), l = n[0].previousElementSibling, u = ii(Vr(n), {
				tabbable: i,
				accept: s
			}, n);
			u.currentNode = Y(c, n) ? c : l;
			let d = u.nextNode();
			return !d && a && (u.currentNode = l, d = u.nextNode()), d && Xr(d, !0), d;
		},
		focusPrevious(t = {}) {
			let n = e.current, { from: r, tabbable: i, wrap: a, accept: s } = t, c = r || o(T(n[0] ?? void 0)), l = n[n.length - 1].nextElementSibling, u = ii(Vr(n), {
				tabbable: i,
				accept: s
			}, n);
			u.currentNode = Y(c, n) ? c : l;
			let d = u.previousNode();
			return !d && a && (u.currentNode = l, d = u.previousNode()), d && Xr(d, !0), d;
		},
		focusFirst(t = {}) {
			let n = e.current, { tabbable: r, accept: i } = t, a = ii(Vr(n), {
				tabbable: r,
				accept: i
			}, n);
			a.currentNode = n[0].previousElementSibling;
			let o = a.nextNode();
			return o && Xr(o, !0), o;
		},
		focusLast(t = {}) {
			let n = e.current, { tabbable: r, accept: i } = t, a = ii(Vr(n), {
				tabbable: r,
				accept: i
			}, n);
			a.currentNode = n[n.length - 1].nextElementSibling;
			let o = a.previousNode();
			return o && Xr(o, !0), o;
		}
	};
}
function Vr(e) {
	return e[0].parentElement;
}
function Hr(e) {
	let t = X.getTreeNode(J);
	for (; t && t.scopeRef !== e;) {
		if (t.contain) return !1;
		t = t.parent;
	}
	return !0;
}
function Ur(e) {
	if (!e.form) return Array.from(T(e).querySelectorAll(`input[type="radio"][name="${CSS.escape(e.name)}"]`)).filter((e) => !e.form);
	let t = e.form.elements.namedItem(e.name), n = D(e);
	return t instanceof n.RadioNodeList ? Array.from(t).filter((e) => e instanceof n.HTMLInputElement) : t instanceof n.HTMLInputElement ? [t] : [];
}
function Wr(e) {
	if (e.checked) return !0;
	let t = Ur(e);
	return t.length > 0 && !t.some((e) => e.checked);
}
function Gr(e, t) {
	let n = (0, q.useRef)(void 0), r = (0, q.useRef)(void 0);
	N(() => {
		let i = e.current;
		if (!t) {
			r.current &&= (cancelAnimationFrame(r.current), void 0);
			return;
		}
		let a = T(i ? i[0] : void 0), s = (t) => {
			if (t.key !== "Tab" || t.altKey || t.ctrlKey || t.metaKey || !Hr(e) || t.isComposing) return;
			let n = o(a), r = e.current;
			if (!r || !Y(n, r)) return;
			let i = ii(Vr(r), { tabbable: !0 }, r);
			if (!n) return;
			i.currentNode = n;
			let s = t.shiftKey ? i.previousNode() : i.nextNode();
			s ||= (i.currentNode = t.shiftKey ? r[r.length - 1].nextElementSibling : r[0].previousElementSibling, t.shiftKey ? i.previousNode() : i.nextNode()), t.preventDefault(), s && (Xr(s, !0), s instanceof D(s).HTMLInputElement && s.select());
		}, c = (t) => {
			(!J || Yr(J, e)) && Y(E(t), e.current) ? (J = e, n.current = E(t)) : Hr(e) && !qr(E(t), e) ? n.current ? n.current.focus() : J && J.current && Qr(J.current) : Hr(e) && (n.current = E(t));
		}, l = (t) => {
			r.current && cancelAnimationFrame(r.current), r.current = requestAnimationFrame(() => {
				let r = C(), i = (r === "virtual" || r === null) && _() && ae(), s = o(a);
				if (!i && s && Hr(e) && !qr(s, e)) {
					J = e;
					let r = E(t);
					r && r.isConnected ? (n.current = r, n.current?.focus()) : J.current && Qr(J.current);
				}
			});
		};
		return a.addEventListener("keydown", s, !1), a.addEventListener("focusin", c, !1), i?.forEach((e) => e.addEventListener("focusin", c, !1)), i?.forEach((e) => e.addEventListener("focusout", l, !1)), () => {
			a.removeEventListener("keydown", s, !1), a.removeEventListener("focusin", c, !1), i?.forEach((e) => e.removeEventListener("focusin", c, !1)), i?.forEach((e) => e.removeEventListener("focusout", l, !1));
		};
	}, [e, t]), N(() => () => {
		r.current && cancelAnimationFrame(r.current);
	}, [r]);
}
function Kr(e) {
	return qr(e);
}
function Y(e, t) {
	return !e || !t ? !1 : t.some((t) => S(t, e));
}
function qr(e, t = null) {
	if (e instanceof Element && e.closest("[data-react-aria-top-layer]")) return !0;
	for (let { scopeRef: n } of X.traverse(X.getTreeNode(t))) if (n && Y(e, n.current)) return !0;
	return !1;
}
function Jr(e) {
	return qr(e, J);
}
function Yr(e, t) {
	let n = X.getTreeNode(t)?.parent;
	for (; n;) {
		if (n.scopeRef === e) return !0;
		n = n.parent;
	}
	return !1;
}
function Xr(e, t = !1) {
	if (e != null && !t) try {
		P(e);
	} catch {}
	else if (e != null) try {
		e.focus();
	} catch {}
}
function Zr(e, t = !0) {
	let n = e[0].previousElementSibling, r = Vr(e), i = ii(r, { tabbable: t }, e);
	i.currentNode = n;
	let a = i.nextNode();
	return t && !a && (r = Vr(e), i = ii(r, { tabbable: !1 }, e), i.currentNode = n, a = i.nextNode()), a;
}
function Qr(e, t = !0) {
	Xr(Zr(e, t));
}
function $r(e, t) {
	let n = q.useRef(t);
	(0, q.useEffect)(() => {
		n.current && (J = e, !Y(o(T(e.current ? e.current[0] : void 0)), J.current) && e.current && Qr(e.current)), n.current = !1;
	}, [e]);
}
function ei(e, t, n) {
	N(() => {
		if (t || n) return;
		let r = e.current, i = T(r ? r[0] : void 0), a = (t) => {
			let n = E(t);
			Y(n, e.current) ? J = e : Kr(n) || (J = null);
		};
		return i.addEventListener("focusin", a, !1), r?.forEach((e) => e.addEventListener("focusin", a, !1)), () => {
			i.removeEventListener("focusin", a, !1), r?.forEach((e) => e.removeEventListener("focusin", a, !1));
		};
	}, [
		e,
		t,
		n
	]);
}
function ti(e) {
	let t = X.getTreeNode(J);
	for (; t && t.scopeRef !== e;) {
		if (t.nodeToRestore) return !1;
		t = t.parent;
	}
	return t?.scopeRef === e;
}
function ni(e, t, n) {
	let r = (0, q.useRef)(typeof document < "u" ? o(T(e.current ? e.current[0] : void 0)) : null);
	N(() => {
		let r = e.current, i = T(r ? r[0] : void 0);
		if (!t || n) return;
		let a = () => {
			(!J || Yr(J, e)) && Y(o(i), e.current) && (J = e);
		};
		return i.addEventListener("focusin", a, !1), r?.forEach((e) => e.addEventListener("focusin", a, !1)), () => {
			i.removeEventListener("focusin", a, !1), r?.forEach((e) => e.removeEventListener("focusin", a, !1));
		};
	}, [e, n]), N(() => {
		let r = T(e.current ? e.current[0] : void 0);
		if (!t) return;
		let i = (t) => {
			if (t.key !== "Tab" || t.altKey || t.ctrlKey || t.metaKey || !Hr(e) || t.isComposing) return;
			let n = r.activeElement;
			if (!qr(n, e) || !ti(e)) return;
			let i = X.getTreeNode(e);
			if (!i) return;
			let a = i.nodeToRestore, o = ii(r.body, { tabbable: !0 });
			o.currentNode = n;
			let s = t.shiftKey ? o.previousNode() : o.nextNode();
			if ((!a || !a.isConnected || a === r.body) && (a = void 0, i.nodeToRestore = void 0), (!s || !qr(s, e)) && a) {
				o.currentNode = a;
				do
					s = t.shiftKey ? o.previousNode() : o.nextNode();
				while (qr(s, e));
				t.preventDefault(), t.stopPropagation(), s ? Xr(s, !0) : Kr(a) ? Xr(a, !0) : n.blur();
			}
		};
		return n || r.addEventListener("keydown", i, !0), () => {
			n || r.removeEventListener("keydown", i, !0);
		};
	}, [
		e,
		t,
		n
	]), N(() => {
		let n = T(e.current ? e.current[0] : void 0);
		if (!t) return;
		let i = X.getTreeNode(e);
		if (i) return i.nodeToRestore = r.current ?? void 0, () => {
			let r = X.getTreeNode(e);
			if (!r) return;
			let i = r.nodeToRestore, a = o(n);
			if (t && i && (a && qr(a, e) || a === n.body && ti(e))) {
				let t = X.clone();
				requestAnimationFrame(() => {
					if (n.activeElement === n.body) {
						let n = t.getTreeNode(e);
						for (; n;) {
							if (n.nodeToRestore && n.nodeToRestore.isConnected) {
								ri(n.nodeToRestore);
								return;
							}
							n = n.parent;
						}
						for (n = t.getTreeNode(e); n;) {
							if (n.scopeRef && n.scopeRef.current && X.getTreeNode(n.scopeRef)) {
								ri(Zr(n.scopeRef.current, !0));
								return;
							}
							n = n.parent;
						}
					}
				});
			}
		};
	}, [e, t]);
}
function ri(e) {
	e.dispatchEvent(new CustomEvent(Rr, {
		bubbles: !0,
		cancelable: !0
	})) && Xr(e);
}
function ii(e, t, n) {
	let r = t?.tabbable ? p : u, i = T(e?.nodeType === Node.ELEMENT_NODE ? e : null), a = Ir(i, e || i, NodeFilter.SHOW_ELEMENT, { acceptNode(e) {
		return S(t?.from, e) || t?.tabbable && e.tagName === "INPUT" && e.getAttribute("type") === "radio" && (!Wr(e) || a.currentNode.tagName === "INPUT" && a.currentNode.type === "radio" && a.currentNode.name === e.name) ? NodeFilter.FILTER_REJECT : r(e) && (!n || Y(e, n)) && (!t?.accept || t.accept(e)) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP;
	} });
	return t?.from && (a.currentNode = t.from), a;
}
var ai = class e {
	constructor() {
		this.fastMap = /* @__PURE__ */ new Map(), this.root = new oi({ scopeRef: null }), this.fastMap.set(null, this.root);
	}
	get size() {
		return this.fastMap.size;
	}
	getTreeNode(e) {
		return this.fastMap.get(e);
	}
	addTreeNode(e, t, n) {
		let r = this.fastMap.get(t ?? null);
		if (!r) return;
		let i = new oi({ scopeRef: e });
		r.addChild(i), i.parent = r, this.fastMap.set(e, i), n && (i.nodeToRestore = n);
	}
	addNode(e) {
		this.fastMap.set(e.scopeRef, e);
	}
	removeTreeNode(e) {
		if (e === null) return;
		let t = this.fastMap.get(e);
		if (!t) return;
		let n = t.parent;
		for (let e of this.traverse()) e !== t && t.nodeToRestore && e.nodeToRestore && t.scopeRef && t.scopeRef.current && Y(e.nodeToRestore, t.scopeRef.current) && (e.nodeToRestore = t.nodeToRestore);
		let r = t.children;
		n && (n.removeChild(t), r.size > 0 && r.forEach((e) => n && n.addChild(e))), this.fastMap.delete(t.scopeRef);
	}
	*traverse(e = this.root) {
		if (e.scopeRef != null && (yield e), e.children.size > 0) for (let t of e.children) yield* this.traverse(t);
	}
	clone() {
		let t = new e();
		for (let e of this.traverse()) t.addTreeNode(e.scopeRef, e.parent?.scopeRef ?? null, e.nodeToRestore);
		return t;
	}
}, oi = class {
	constructor(e) {
		this.children = /* @__PURE__ */ new Set(), this.contain = !1, this.scopeRef = e.scopeRef;
	}
	addChild(e) {
		this.children.add(e), e.parent = this;
	}
	removeChild(e) {
		this.children.delete(e), e.parent = void 0;
	}
}, X = new ai();
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/selection/utils.mjs
function si(e) {
	return v() ? e.altKey : e.ctrlKey;
}
function ci(e, t) {
	let n = `[data-key="${CSS.escape(String(t))}"]`, r = e.current?.dataset.collection;
	return r && (n = `[data-collection="${CSS.escape(r)}"]${n}`), e.current?.querySelector(n);
}
var li = /* @__PURE__ */ new WeakMap();
function ui(e) {
	let t = G();
	return li.set(e, t), t;
}
function di(e) {
	return li.get(e);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/selection/useTypeSelect.mjs
var fi = 1e3;
function pi(e) {
	let { keyboardDelegate: t, selectionManager: n, onTypeSelect: r } = e, i = (0, q.useRef)({
		search: "",
		timeout: void 0
	});
	return (0, q.useEffect)(() => {
		let e = i.current.timeout;
		return () => {
			clearTimeout(e);
		};
	}, [i]), { typeSelectProps: {
		onKeyDownCapture: t.getKeyForSearch ? (e) => {
			if (i.current.search.length > 0 && e.key === " ") {
				if (e.preventDefault(), (!("continuePropagation" in e) || "continuePropagation" in e && !e.isPropagationStopped()) && e.stopPropagation(), i.current.search += " ", t.getKeyForSearch != null) {
					let e = t.getKeyForSearch(i.current.search, n.focusedKey);
					e ??= t.getKeyForSearch(i.current.search), e != null && (n.setFocusedKey(e), r && r(e));
				}
				clearTimeout(i.current.timeout), i.current.timeout = setTimeout(() => {
					i.current.search = "";
				}, fi);
			}
		} : void 0,
		onKeyDown: t.getKeyForSearch ? (e) => {
			let a = mi(e.key);
			if (!(!a || e.ctrlKey || e.metaKey || e.altKey || !S(e.currentTarget, E(e)) || i.current.search.length === 0 && a === " ")) {
				if (i.current.search += a, t.getKeyForSearch != null) {
					let a = t.getKeyForSearch(i.current.search, n.focusedKey);
					if (a ??= t.getKeyForSearch(i.current.search), a != null) n.setFocusedKey(a), r && r(a), e.preventDefault(), "continuePropagation" in e || e.stopPropagation();
					else {
						i.current.search = "", clearTimeout(i.current.timeout), i.current.timeout = void 0;
						return;
					}
				}
				clearTimeout(i.current.timeout), i.current.timeout = setTimeout(() => {
					i.current.search = "";
				}, fi);
			}
		} : void 0
	} };
}
function mi(e) {
	return e.length === 1 || !/^[A-Z]/i.test(e) ? e : "";
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useUpdateLayoutEffect.mjs
function hi(e, t) {
	let n = (0, q.useRef)(!0), r = (0, q.useRef)(null);
	N(() => (n.current = !0, () => {
		n.current = !1;
	}), []), N(() => {
		n.current ? n.current = !1 : (!r.current || t.some((e, t) => !Object.is(e, r[t]))) && e(), r.current = t;
	}, t);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/selection/useSelectableCollection.mjs
function gi(e) {
	let { selectionManager: t, keyboardDelegate: n, ref: r, autoFocus: i = !1, shouldFocusWrap: a = !1, disallowEmptySelection: s = !1, disallowSelectAll: c = !1, escapeKeyBehavior: l = "clearSelection", selectOnFocus: u = t.selectionBehavior === "replace", disallowTypeAhead: d = !1, shouldUseVirtualFocus: f, allowsTabNavigation: m = !1, scrollRef: h = r, linkBehavior: _ = "action", UNSTABLE_focusOnEntry: v } = e, { direction: b } = et(), x = y(), w = (e) => {
		if (e.altKey && e.key === "Tab" && e.preventDefault(), !r.current || !S(r.current, E(e))) return;
		let i = (n, i) => {
			if (n != null) {
				if (t.isLink(n) && _ === "selection" && u && !si(e)) {
					(0, jt.flushSync)(() => {
						t.setFocusedKey(n, i);
					});
					let a = ci(r, n), o = t.getItemProps(n);
					a && x.open(a, e, o.href, o.routerOptions);
					return;
				}
				if (t.setFocusedKey(n, i), t.isLink(n) && _ === "override") return;
				e.shiftKey && t.selectionMode === "multiple" ? t.extendSelection(n) : u && !si(e) && t.replaceSelection(n);
			}
		};
		switch (e.key) {
			case "ArrowDown":
				if (n.getKeyBelow) {
					let r = t.focusedKey == null ? n.getFirstKey?.() : n.getKeyBelow?.(t.focusedKey);
					r == null && a && (r = n.getFirstKey?.(t.focusedKey)), r != null && (e.preventDefault(), i(r));
				}
				break;
			case "ArrowUp":
				if (n.getKeyAbove) {
					let r = t.focusedKey == null ? n.getLastKey?.() : n.getKeyAbove?.(t.focusedKey);
					r == null && a && (r = n.getLastKey?.(t.focusedKey)), r != null && (e.preventDefault(), i(r));
				}
				break;
			case "ArrowLeft":
				if (n.getKeyLeftOf) {
					let r = t.focusedKey == null ? n.getFirstKey?.() : n.getKeyLeftOf?.(t.focusedKey);
					r == null && a && (r = b === "rtl" ? n.getFirstKey?.(t.focusedKey) : n.getLastKey?.(t.focusedKey)), r != null && (e.preventDefault(), i(r, b === "rtl" ? "first" : "last"));
				}
				break;
			case "ArrowRight":
				if (n.getKeyRightOf) {
					let r = t.focusedKey == null ? n.getFirstKey?.() : n.getKeyRightOf?.(t.focusedKey);
					r == null && a && (r = b === "rtl" ? n.getLastKey?.(t.focusedKey) : n.getFirstKey?.(t.focusedKey)), r != null && (e.preventDefault(), i(r, b === "rtl" ? "last" : "first"));
				}
				break;
			case "Home":
				if (n.getFirstKey) {
					if (t.focusedKey === null && e.shiftKey) return;
					e.preventDefault();
					let r = n.getFirstKey(t.focusedKey, ze(e));
					t.setFocusedKey(r), r != null && (ze(e) && e.shiftKey && t.selectionMode === "multiple" ? t.extendSelection(r) : u && t.replaceSelection(r));
				}
				break;
			case "End":
				if (n.getLastKey) {
					if (t.focusedKey === null && e.shiftKey) return;
					e.preventDefault();
					let r = n.getLastKey(t.focusedKey, ze(e));
					t.setFocusedKey(r), r != null && (ze(e) && e.shiftKey && t.selectionMode === "multiple" ? t.extendSelection(r) : u && t.replaceSelection(r));
				}
				break;
			case "PageDown":
				if (n.getKeyPageBelow && t.focusedKey != null) {
					let r = n.getKeyPageBelow(t.focusedKey);
					r != null && (e.preventDefault(), i(r));
				}
				break;
			case "PageUp":
				if (n.getKeyPageAbove && t.focusedKey != null) {
					let r = n.getKeyPageAbove(t.focusedKey);
					r != null && (e.preventDefault(), i(r));
				}
				break;
			case "a":
				ze(e) && t.selectionMode === "multiple" && c !== !0 && (e.preventDefault(), t.selectAll());
				break;
			case "Escape":
				l === "clearSelection" && !s && t.selectedKeys.size !== 0 && (e.stopPropagation(), e.preventDefault(), t.clearSelection());
				break;
			case "Tab": if (!m) {
				if (e.shiftKey) r.current.focus();
				else {
					let e = ii(r.current, { tabbable: !0 }), t, n;
					do
						n = e.lastChild(), n && (t = n);
					while (n);
					let i = o();
					t && (!Se(t) || i && !p(i)) && g(t);
				}
				break;
			}
		}
	}, T = (0, q.useRef)({
		top: 0,
		left: 0
	});
	He(h, "scroll", () => {
		T.current = {
			top: h.current?.scrollTop ?? 0,
			left: h.current?.scrollLeft ?? 0
		};
	});
	let D = (e) => {
		if (t.isFocused) {
			S(e.currentTarget, E(e)) || t.setFocused(!1);
			return;
		}
		if (!S(e.currentTarget, E(e))) return;
		let i = C();
		t.setFocused(!0);
		let a = (e) => {
			e != null && (t.setFocusedKey(e), u && !t.isSelected(e) && t.replaceSelection(e));
		};
		if (v && (i === "keyboard" || i === "virtual")) a(v === "first" ? n.getFirstKey?.() : n.getLastKey?.());
		else if (t.focusedKey == null) {
			let r = e.relatedTarget;
			r && e.currentTarget.compareDocumentPosition(r) & Node.DOCUMENT_POSITION_FOLLOWING ? a(t.lastSelectedKey ?? n.getLastKey?.()) : a(t.firstSelectedKey ?? n.getFirstKey?.());
		} else h.current && (h.current.scrollTop = T.current.top, h.current.scrollLeft = T.current.left);
		if (t.focusedKey != null && h.current) {
			let e = ci(r, t.focusedKey);
			e instanceof HTMLElement && (!Se(e) && !f && g(e), (i === "keyboard" || v && i === "virtual") && pn(e, { containingElement: r.current }));
		}
	}, O = (e) => {
		S(e.currentTarget, e.relatedTarget) || t.setFocused(!1);
	}, k = (0, q.useRef)(!1);
	He(r, Pe, f ? (e) => {
		let { detail: n } = e;
		e.stopPropagation(), t.setFocused(!0), n?.focusStrategy === "first" && (k.current = !0);
	} : void 0);
	let A = n.getFirstKey?.() ?? null;
	hi(() => {
		if (k.current) if (A == null) {
			let e = o();
			Fe(r.current), Le(e, null), t.collection.size > 0 && (k.current = !1);
		} else t.setFocusedKey(A), k.current = !1;
	}, [A, t.collection.size]), hi(() => {
		t.collection.size > 0 && (k.current = !1);
	}, [t.focusedKey]), He(r, Ne, f ? (e) => {
		e.stopPropagation(), t.setFocused(!1), e.detail?.clearFocusKey && t.setFocusedKey(null);
	} : void 0);
	let j = (0, q.useRef)(i), M = (0, q.useRef)(!1);
	(0, q.useEffect)(() => {
		if (j.current) {
			let e = null;
			i === "first" && (e = n.getFirstKey?.() ?? null), i === "last" && (e = n.getLastKey?.() ?? null);
			let a = t.selectedKeys;
			if (a.size) {
				for (let n of a) if (t.canSelectItem(n)) {
					e = n;
					break;
				}
			}
			t.setFocused(!0), t.setFocusedKey(e), e == null && !f && r.current && P(r.current), t.collection.size > 0 && (j.current = !1, M.current = !0);
		}
	});
	let N = (0, q.useRef)(t.focusedKey), F = (0, q.useRef)(null);
	(0, q.useEffect)(() => {
		if (t.isFocused && t.focusedKey != null && (t.focusedKey !== N.current || M.current) && h.current && r.current) {
			let e = C(), n = ci(r, t.focusedKey);
			if (!(n instanceof HTMLElement)) return;
			(e === "keyboard" || M.current) && (F.current && cancelAnimationFrame(F.current), F.current = requestAnimationFrame(() => {
				h.current && (fn(h.current, n), e !== "virtual" && pn(n, { containingElement: r.current }));
			}));
		}
		!f && t.isFocused && t.focusedKey == null && N.current != null && r.current && P(r.current), N.current = t.focusedKey, M.current = !1;
	}), (0, q.useEffect)(() => () => {
		F.current && cancelAnimationFrame(F.current);
	}, []), He(r, "react-aria-focus-scope-restore", (e) => {
		e.preventDefault(), t.setFocused(!0);
	});
	let I = {
		onKeyDown: w,
		onFocus: D,
		onBlur: O,
		onMouseDown(e) {
			h.current === E(e) && e.preventDefault();
		}
	}, { typeSelectProps: ee } = pi({
		keyboardDelegate: n,
		selectionManager: t
	});
	d || (I = R(ee, I));
	let L;
	f || (L = t.focusedKey == null ? 0 : -1);
	let z = ui(t.collection);
	return { collectionProps: R(I, {
		tabIndex: L,
		"data-collection": z
	}) };
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/selection/DOMLayoutDelegate.mjs
var _i = class {
	constructor(e) {
		this.ref = e;
	}
	getItemRect(e) {
		let t = this.ref.current;
		if (!t) return null;
		let n = e == null ? null : ci(this.ref, e);
		if (!n) return null;
		let r = t.getBoundingClientRect(), i = n.getBoundingClientRect();
		return {
			x: i.left - r.left - t.clientLeft + t.scrollLeft,
			y: i.top - r.top - t.clientTop + t.scrollTop,
			width: i.width,
			height: i.height
		};
	}
	getContentSize() {
		let e = this.ref.current;
		return {
			width: e?.scrollWidth ?? 0,
			height: e?.scrollHeight ?? 0
		};
	}
	getVisibleRect() {
		let e = this.ref.current;
		return {
			x: e?.scrollLeft ?? 0,
			y: e?.scrollTop ?? 0,
			width: e?.clientWidth ?? 0,
			height: e?.clientHeight ?? 0
		};
	}
}, vi = class {
	constructor(...e) {
		if (e.length === 1) {
			let t = e[0];
			this.collection = t.collection, this.ref = t.ref, this.collator = t.collator, this.disabledKeys = t.disabledKeys || /* @__PURE__ */ new Set(), this.disabledBehavior = t.disabledBehavior || "all", this.orientation = t.orientation || "vertical", this.direction = t.direction, this.layout = t.layout || "stack", this.layoutDelegate = t.layoutDelegate || new _i(t.ref);
		} else this.collection = e[0], this.disabledKeys = e[1], this.ref = e[2], this.collator = e[3], this.layout = "stack", this.orientation = "vertical", this.disabledBehavior = "all", this.layoutDelegate = new _i(this.ref);
		this.layout === "stack" && this.orientation === "vertical" && (this.getKeyLeftOf = void 0, this.getKeyRightOf = void 0);
	}
	isDisabled(e) {
		return this.disabledBehavior === "all" && (e.props?.isDisabled || this.disabledKeys.has(e.key)) && e.props?.disabledBehavior !== "selection";
	}
	findNextNonDisabled(e, t, n = !1) {
		let r = e;
		for (; r != null;) {
			let e = this.collection.getItem(r);
			if (e?.type === "item" && (n || !this.isDisabled(e))) return r;
			r = t(r);
		}
		return null;
	}
	getNextKey(e, t) {
		let n = e;
		return n = this.collection.getKeyAfter(n), this.findNextNonDisabled(n, (e) => this.collection.getKeyAfter(e), t?.includeDisabled);
	}
	getPreviousKey(e, t) {
		let n = e;
		return n = this.collection.getKeyBefore(n), this.findNextNonDisabled(n, (e) => this.collection.getKeyBefore(e), t?.includeDisabled);
	}
	findKey(e, t, n) {
		let r = e, i = this.layoutDelegate.getItemRect(r);
		if (!i || r == null) return null;
		let a = i;
		do {
			if (r = t(r), r == null) break;
			i = this.layoutDelegate.getItemRect(r);
		} while (i && n(a, i) && r != null);
		return r;
	}
	isSameRow(e, t) {
		return e.y === t.y || e.x !== t.x;
	}
	isSameColumn(e, t) {
		return e.x === t.x || e.y !== t.y;
	}
	isReversed(e) {
		let t = this.getNextKey(e), n = ci(this.ref, e);
		if (t != null) {
			let e = ci(this.ref, t);
			return !n || !e ? !1 : n.getBoundingClientRect().top > e.getBoundingClientRect().top;
		}
		let r = this.getPreviousKey(e);
		if (r != null) {
			let e = ci(this.ref, r);
			return !n || !e ? !1 : e.getBoundingClientRect().top > n.getBoundingClientRect().top;
		}
		return !1;
	}
	getKeyBelow(e, t) {
		return this.layout === "grid" && this.orientation === "vertical" ? this.findKey(e, (e) => this.getNextKey(e, t), this.isSameRow) : this.orientation === "vertical" && this.isReversed(e) ? this.getPreviousKey(e, t) : this.getNextKey(e, t);
	}
	getKeyAbove(e, t) {
		return this.layout === "grid" && this.orientation === "vertical" ? this.findKey(e, (e) => this.getPreviousKey(e, t), this.isSameRow) : this.orientation === "vertical" && this.isReversed(e) ? this.getNextKey(e, t) : this.getPreviousKey(e, t);
	}
	getNextColumn(e, t, n) {
		return t ? this.getPreviousKey(e, n) : this.getNextKey(e, n);
	}
	getKeyRightOf(e, t) {
		let n = this.direction === "ltr" ? "getKeyRightOf" : "getKeyLeftOf";
		return this.layoutDelegate[n] ? (e = this.layoutDelegate[n](e), this.findNextNonDisabled(e, (e) => this.layoutDelegate[n](e), t?.includeDisabled)) : this.layout === "grid" ? this.orientation === "vertical" ? this.getNextColumn(e, this.direction === "rtl", t) : this.findKey(e, (e) => this.getNextColumn(e, this.direction === "rtl", t), this.isSameColumn) : this.orientation === "horizontal" ? this.getNextColumn(e, this.direction === "rtl", t) : null;
	}
	getKeyLeftOf(e, t) {
		let n = this.direction === "ltr" ? "getKeyLeftOf" : "getKeyRightOf";
		return this.layoutDelegate[n] ? (e = this.layoutDelegate[n](e), this.findNextNonDisabled(e, (e) => this.layoutDelegate[n](e), t?.includeDisabled)) : this.layout === "grid" ? this.orientation === "vertical" ? this.getNextColumn(e, this.direction === "ltr", t) : this.findKey(e, (e) => this.getNextColumn(e, this.direction === "ltr", t), this.isSameColumn) : this.orientation === "horizontal" ? this.getNextColumn(e, this.direction === "ltr", t) : null;
	}
	getFirstKey() {
		let e = this.collection.getFirstKey();
		return this.findNextNonDisabled(e, (e) => this.collection.getKeyAfter(e));
	}
	getLastKey() {
		let e = this.collection.getLastKey();
		return this.findNextNonDisabled(e, (e) => this.collection.getKeyBefore(e));
	}
	getKeyPageAbove(e) {
		let t = this.ref.current, n = this.layoutDelegate.getItemRect(e);
		if (!n) return null;
		let r = this.isReversed(e);
		if (t && !ln(t)) return this.getFirstKey();
		let i = e;
		if (this.orientation === "horizontal") {
			let e = Math.max(0, n.x + n.width - this.layoutDelegate.getVisibleRect().width);
			for (; n && n.x > e && i != null;) i = this.getKeyAbove(i), n = i == null ? null : this.layoutDelegate.getItemRect(i);
		} else {
			let e = this.layoutDelegate.getVisibleRect(), t = r ? n.y - e.height : Math.max(0, n.y + n.height - e.height);
			for (; n && n.y > t && i != null;) i = this.getKeyAbove(i), n = i == null ? null : this.layoutDelegate.getItemRect(i);
		}
		return i ?? (r ? this.getLastKey() : this.getFirstKey());
	}
	getKeyPageBelow(e) {
		let t = this.ref.current, n = this.layoutDelegate.getItemRect(e);
		if (!n) return null;
		let r = this.isReversed(e);
		if (t && !ln(t)) return this.getLastKey();
		let i = e;
		if (this.orientation === "horizontal") {
			let e = Math.min(this.layoutDelegate.getContentSize().width, n.x - n.width + this.layoutDelegate.getVisibleRect().width);
			for (; n && n.x < e && i != null;) i = this.getKeyBelow(i), n = i == null ? null : this.layoutDelegate.getItemRect(i);
		} else {
			let e = Math.min(this.layoutDelegate.getContentSize().height, n.y - n.height + this.layoutDelegate.getVisibleRect().height);
			for (; n && n.y < e && i != null;) i = this.getKeyBelow(i), n = i == null ? null : this.layoutDelegate.getItemRect(i);
		}
		return i ?? (r ? this.getFirstKey() : this.getLastKey());
	}
	getKeyForSearch(e, t) {
		if (!this.collator) return null;
		let n = this.collection, r = t || this.getFirstKey();
		for (; r != null;) {
			let t = n.getItem(r);
			if (!t) return null;
			let i = t.textValue.slice(0, e.length);
			if (t.textValue && this.collator.compare(i, e) === 0) return r;
			r = this.getNextKey(r);
		}
		return null;
	}
}, yi = /* @__PURE__ */ new Map();
function bi(e) {
	let { locale: t } = et(), n = t + (e ? Object.entries(e).sort((e, t) => e[0] < t[0] ? -1 : 1).join() : "");
	if (yi.has(n)) return yi.get(n);
	let r = new Intl.Collator(t, e);
	return yi.set(n, r), r;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/selection/useSelectableList.mjs
function xi(e) {
	let { selectionManager: t, collection: n, disabledKeys: r, ref: i, keyboardDelegate: a, layoutDelegate: o, orientation: s } = e, c = bi({
		usage: "search",
		sensitivity: "base"
	}), l = t.disabledBehavior, u = (0, q.useMemo)(() => a || new vi({
		collection: n,
		disabledKeys: r,
		disabledBehavior: l,
		ref: i,
		collator: c,
		layoutDelegate: o,
		orientation: s
	}), [
		a,
		o,
		n,
		r,
		i,
		c,
		l,
		s
	]), { collectionProps: d } = gi({
		...e,
		ref: i,
		selectionManager: t,
		keyboardDelegate: u
	});
	return { listProps: d };
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/listbox/useListBox.mjs
function Si(e, t, n) {
	let r = H(e, { labelable: !0 }), i = e.selectionBehavior || "toggle", a = e.orientation || "vertical", o = e.linkBehavior || (i === "replace" ? "action" : "override");
	i === "toggle" && o === "action" && (o = "override");
	let { listProps: s } = xi({
		...e,
		ref: n,
		selectionManager: t.selectionManager,
		collection: t.collection,
		disabledKeys: t.disabledKeys,
		linkBehavior: o
	}), { focusWithinProps: c } = oe({
		onFocusWithin: e.onFocus,
		onBlurWithin: e.onBlur,
		onFocusWithinChange: e.onFocusChange
	}), l = G(e.id);
	Mr.set(t, {
		id: l,
		shouldUseVirtualFocus: e.shouldUseVirtualFocus,
		shouldSelectOnPressUp: e.shouldSelectOnPressUp,
		shouldFocusOnHover: e.shouldFocusOnHover,
		isVirtualized: e.isVirtualized,
		onAction: e.onAction,
		linkBehavior: o,
		UNSTABLE_itemBehavior: e.UNSTABLE_itemBehavior
	});
	let { labelProps: u, fieldProps: d } = nn({
		...e,
		id: l,
		labelElementType: "span"
	});
	return {
		labelProps: u,
		listBoxProps: R(r, c, t.selectionManager.selectionMode === "multiple" ? { "aria-multiselectable": "true" } : {}, {
			role: "listbox",
			"aria-orientation": a,
			...R(d, s)
		})
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/interactions/useLongPress.mjs
var Ci = 500;
function wi(e) {
	let { isDisabled: t, onLongPressStart: n, onLongPressEnd: r, onLongPress: i, threshold: a = Ci, accessibilityDescription: o } = e, s = (0, q.useRef)(void 0), { addGlobalListener: c, removeGlobalListener: l } = ee(), { pressProps: u } = ge({
		isDisabled: t,
		onPressStart(e) {
			if (e.continuePropagation(), (e.pointerType === "mouse" || e.pointerType === "touch") && (n && n({
				...e,
				type: "longpressstart"
			}), s.current = setTimeout(() => {
				e.target.dispatchEvent(new PointerEvent("pointercancel", { bubbles: !0 })), T(e.target).activeElement !== e.target && g(e.target), i && i({
					...e,
					type: "longpress"
				}), s.current = void 0;
			}, a), e.pointerType === "touch")) {
				let t = (e) => {
					e.preventDefault();
				}, n = D(e.target);
				c(e.target, "contextmenu", t, { once: !0 }), c(n, "pointerup", () => {
					setTimeout(() => {
						l(e.target, "contextmenu", t);
					}, 30);
				}, { once: !0 });
			}
		},
		onPressEnd(e) {
			s.current && clearTimeout(s.current), r && (e.pointerType === "mouse" || e.pointerType === "touch") && r({
				...e,
				type: "longpressend"
			});
		}
	});
	return { longPressProps: R(u, gn(i && !t ? o : void 0)) };
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/selection/useSelectableItem.mjs
function Ti(e) {
	let { id: t, selectionManager: n, key: r, ref: i, shouldSelectOnPressUp: a, shouldUseVirtualFocus: s, focus: l, isDisabled: u, onAction: d, allowsDifferentPressOrigin: f, linkBehavior: p = "action" } = e, m = y();
	t = G(t);
	let h = (e) => {
		if (e.pointerType === "keyboard" && si(e)) n.toggleSelection(r);
		else {
			if (n.selectionMode === "none") return;
			if (n.isLink(r)) {
				if (p === "selection" && i.current) {
					let t = n.getItemProps(r);
					m.open(i.current, e, t.href, t.routerOptions), n.setSelectedKeys(n.selectedKeys);
					return;
				} else if (p === "override" || p === "none") return;
			}
			n.selectionMode === "single" ? n.isSelected(r) && !n.disallowEmptySelection ? n.toggleSelection(r) : n.replaceSelection(r) : e && e.shiftKey ? n.extendSelection(r) : n.selectionBehavior === "toggle" || e && (ze(e) || e.pointerType === "touch" || e.pointerType === "virtual") ? n.toggleSelection(r) : n.replaceSelection(r);
		}
	};
	(0, q.useEffect)(() => {
		r === n.focusedKey && n.isFocused && (s ? Fe(i.current) : l ? l() : o() !== i.current && i.current && P(i.current));
	}, [
		i,
		r,
		n.focusedKey,
		n.childFocusStrategy,
		n.isFocused,
		s
	]), u ||= n.isDisabled(r);
	let g = {};
	!s && !u ? g = {
		tabIndex: r === n.focusedKey ? 0 : -1,
		onFocus(e) {
			E(e) === i.current && n.setFocusedKey(r);
		}
	} : u && (g.onMouseDown = (e) => {
		e.preventDefault();
	}), (0, q.useEffect)(() => {
		u && n.focusedKey === r && n.setFocusedKey(null);
	}, [
		n,
		u,
		r
	]);
	let _ = n.isLink(r) && p === "override", v = d && e.UNSTABLE_itemBehavior === "action", b = n.isLink(r) && p !== "selection" && p !== "none", x = !u && n.canSelectItem(r) && !_ && !v, S = (d || b) && !u, C = S && (n.selectionBehavior === "replace" ? !x : !x || n.isEmpty), w = S && x && n.selectionBehavior === "replace", T = C || w, D = (0, q.useRef)(null), O = T && x, k = (0, q.useRef)(!1), A = (0, q.useRef)(!1), j = n.getItemProps(r), M = (e) => {
		d && (d(), i.current?.dispatchEvent(new CustomEvent("react-aria-item-action", { bubbles: !0 }))), b && i.current && m.open(i.current, e, j.href, j.routerOptions);
	}, N = { ref: i };
	if (a ? (N.onPressStart = (e) => {
		D.current = e.pointerType, k.current = O, e.pointerType === "keyboard" && (!T || Di(e.key)) && h(e);
	}, f ? (N.onPressUp = C ? void 0 : (e) => {
		e.pointerType === "mouse" && x && h(e);
	}, N.onPress = C ? M : (e) => {
		e.pointerType !== "keyboard" && e.pointerType !== "mouse" && x && h(e);
	}) : N.onPress = (e) => {
		if (C || w && e.pointerType !== "mouse") {
			if (e.pointerType === "keyboard" && !Ei(e.key)) return;
			M(e);
		} else e.pointerType !== "keyboard" && x && h(e);
	}) : (N.onPressStart = (e) => {
		D.current = e.pointerType, k.current = O, A.current = C, x && (e.pointerType === "mouse" && !C || e.pointerType === "keyboard" && (!S || Di(e.key))) && h(e);
	}, N.onPress = (e) => {
		(e.pointerType === "touch" || e.pointerType === "pen" || e.pointerType === "virtual" || e.pointerType === "keyboard" && T && Ei(e.key) || e.pointerType === "mouse" && A.current) && (T ? M(e) : x && h(e));
	}), g["data-collection"] = di(n.collection), g["data-key"] = r, N.preventFocusOnPress = s, s && (N = R(N, {
		onPressStart(e) {
			e.pointerType !== "touch" && (n.setFocused(!0), n.setFocusedKey(r));
		},
		onPress(e) {
			e.pointerType === "touch" && (n.setFocused(!0), n.setFocusedKey(r));
		}
	})), j) for (let e of [
		"onPressStart",
		"onPressEnd",
		"onPressChange",
		"onPress",
		"onPressUp",
		"onClick"
	]) j[e] && (N[e] = ue(N[e], j[e]));
	let { pressProps: F, isPressed: I } = ge(N), ee = w ? (e) => {
		D.current === "mouse" && (e.stopPropagation(), e.preventDefault(), M(e));
	} : void 0, { longPressProps: L } = wi({
		isDisabled: !O,
		onLongPress(e) {
			e.pointerType === "touch" && (h(e), n.setSelectionBehavior("toggle"));
		}
	}), z = (e) => {
		D.current === "touch" && k.current && e.preventDefault();
	}, te = p !== "none" && n.isLink(r) ? (e) => {
		c.isOpening || e.preventDefault();
	} : void 0;
	return {
		itemProps: R(g, x || C || s && !u ? F : {}, O ? L : {}, {
			onDoubleClick: ee,
			onDragStartCapture: z,
			onClick: te,
			id: t
		}, s ? { onMouseDown: (e) => e.preventDefault() } : void 0),
		isPressed: I,
		isSelected: n.isSelected(r),
		isFocused: n.isFocused && n.focusedKey === r,
		isDisabled: u,
		allowsSelection: x,
		hasAction: T
	};
}
function Ei(e) {
	return e === "Enter";
}
function Di(e) {
	return e === " ";
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/collections/getChildNodes.mjs
function Oi(e, t) {
	return typeof t.getChildren == "function" ? t.getChildren(e.key) : e.childNodes;
}
function ki(e) {
	return Ai(e, 0);
}
function Ai(e, t) {
	if (t < 0) return;
	let n = 0;
	for (let r of e) {
		if (n === t) return r;
		n++;
	}
}
function ji(e, t, n) {
	if (t.parentKey === n.parentKey) return t.index - n.index;
	let r = [...Mi(e, t), t], i = [...Mi(e, n), n], a = r.slice(0, i.length).findIndex((e, t) => e !== i[t]);
	return a === -1 ? r.findIndex((e) => e === n) >= 0 ? 1 : (i.findIndex((e) => e === t), -1) : (t = r[a], n = i[a], t.index - n.index);
}
function Mi(e, t) {
	let n = [], r = t;
	for (; r?.parentKey != null;) r = e.getItem(r.parentKey), r && n.unshift(r);
	return n;
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/collections/getItemCount.mjs
var Ni = /* @__PURE__ */ new WeakMap();
function Pi(e) {
	let t = Ni.get(e);
	if (t != null) return t;
	let n = 0, r = (t) => {
		for (let i of t) i.type === "section" ? r(Oi(i, e)) : i.type === "item" && n++;
	};
	return r(e), Ni.set(e, n), n;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/listbox/useOption.mjs
function Fi(e, t, n) {
	let { key: r } = e, i = Mr.get(t), a = e.isDisabled ?? t.selectionManager.isDisabled(r), o = e.isSelected ?? t.selectionManager.isSelected(r), s = e.shouldSelectOnPressUp ?? i?.shouldSelectOnPressUp, c = e.shouldFocusOnHover ?? i?.shouldFocusOnHover, u = e.shouldUseVirtualFocus ?? i?.shouldUseVirtualFocus, d = e.isVirtualized ?? i?.isVirtualized, f = ce(), p = ce(), m = {
		role: "option",
		"aria-disabled": a || void 0,
		"aria-selected": t.selectionManager.selectionMode === "none" ? void 0 : o,
		"aria-label": e["aria-label"],
		"aria-labelledby": f,
		"aria-describedby": p
	}, h = t.collection.getItem(r);
	if (d) {
		let e = Number(h?.index);
		m["aria-posinset"] = Number.isNaN(e) ? void 0 : e + 1, m["aria-setsize"] = Pi(t.collection);
	}
	let g = i?.onAction ? () => i?.onAction?.(r) : void 0, _ = Pr(t, r), { itemProps: v, isPressed: y, isFocused: b, hasAction: x, allowsSelection: S } = Ti({
		selectionManager: t.selectionManager,
		key: r,
		ref: n,
		shouldSelectOnPressUp: s,
		allowsDifferentPressOrigin: s && c,
		isVirtualized: d,
		shouldUseVirtualFocus: u,
		isDisabled: a,
		onAction: g || h?.props?.onAction ? ue(h?.props?.onAction, g) : void 0,
		linkBehavior: i?.linkBehavior,
		UNSTABLE_itemBehavior: i?.UNSTABLE_itemBehavior,
		id: _
	}), { hoverProps: C } = fe({
		isDisabled: a || !c,
		onHoverStart() {
			ye() || (t.selectionManager.setFocused(!0), t.selectionManager.setFocusedKey(r));
		}
	}), w = H(h?.props);
	delete w.id;
	let T = l(h?.props);
	return {
		optionProps: {
			...m,
			...R(w, v, C, T),
			id: _
		},
		labelProps: { id: f },
		descriptionProps: { id: p },
		isFocused: b,
		isFocusVisible: b && t.selectionManager.isFocused && ye(),
		isSelected: o,
		isDisabled: a,
		isPressed: y,
		allowsSelection: S,
		hasAction: x
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/listbox/useListBoxSection.mjs
function Ii(e) {
	let { heading: t, "aria-label": n } = e, r = G();
	return {
		itemProps: { role: "presentation" },
		headingProps: t ? {
			id: r,
			role: "presentation",
			onMouseDown: (e) => {
				e.preventDefault();
			}
		} : {},
		groupProps: {
			role: "group",
			"aria-label": n,
			"aria-labelledby": t ? r : void 0
		}
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/inertValue.mjs
function Li(e) {
	let t = q.version.split(".");
	return parseInt(t[0], 10) >= 19 ? e : e ? "true" : void 0;
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/list/ListCollection.mjs
var Ri = class {
	constructor(e) {
		this.keyMap = /* @__PURE__ */ new Map(), this.firstKey = null, this.lastKey = null, this.iterable = e;
		let t = (e) => {
			if (this.keyMap.set(e.key, e), e.childNodes && e.type === "section") for (let n of e.childNodes) t(n);
		};
		for (let n of e) t(n);
		let n = null, r = 0, i = 0;
		for (let [e, t] of this.keyMap) n ? (n.nextKey = e, t.prevKey = n.key) : (this.firstKey = e, t.prevKey = void 0), t.type === "item" && (t.index = r++), (t.type === "section" || t.type === "item") && i++, n = t, n.nextKey = void 0;
		this._size = i, this.lastKey = n?.key ?? null;
	}
	*[Symbol.iterator]() {
		yield* this.iterable;
	}
	get size() {
		return this._size;
	}
	getKeys() {
		return this.keyMap.keys();
	}
	getKeyBefore(e) {
		let t = this.keyMap.get(e);
		return t ? t.prevKey ?? null : null;
	}
	getKeyAfter(e) {
		let t = this.keyMap.get(e);
		return t ? t.nextKey ?? null : null;
	}
	getFirstKey() {
		return this.firstKey;
	}
	getLastKey() {
		return this.lastKey;
	}
	getItem(e) {
		return this.keyMap.get(e) ?? null;
	}
	at(e) {
		let t = [...this.getKeys()];
		return this.getItem(t[e]);
	}
	getChildren(e) {
		return this.keyMap.get(e)?.childNodes || [];
	}
}, zi = class e extends Set {
	constructor(t, n, r) {
		super(t), t instanceof e ? (this.anchorKey = n ?? t.anchorKey, this.currentKey = r ?? t.currentKey) : (this.anchorKey = n ?? null, this.currentKey = r ?? null);
	}
};
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/selection/useMultipleSelectionState.mjs
function Bi(e, t) {
	if (e.size !== t.size) return !1;
	for (let n of e) if (!t.has(n)) return !1;
	return !0;
}
function Vi(e) {
	let { selectionMode: t = "none", disallowEmptySelection: n = !1, allowDuplicateSelectionEvents: r, selectionBehavior: i = "toggle", disabledBehavior: a = "all" } = e, o = (0, q.useRef)(!1), [, s] = (0, q.useState)(!1), c = (0, q.useRef)(null), l = (0, q.useRef)(null), [, u] = (0, q.useState)(null), [d, f] = ht((0, q.useMemo)(() => Hi(e.selectedKeys), [e.selectedKeys]), (0, q.useMemo)(() => Hi(e.defaultSelectedKeys, new zi()), [e.defaultSelectedKeys]), e.onSelectionChange), p = (0, q.useMemo)(() => e.disabledKeys ? new Set(e.disabledKeys) : /* @__PURE__ */ new Set(), [e.disabledKeys]), [m, h] = (0, q.useState)(i);
	i === "replace" && m === "toggle" && typeof d == "object" && d.size === 0 && h("replace");
	let g = (0, q.useRef)(i);
	return (0, q.useEffect)(() => {
		i !== g.current && (h(i), g.current = i);
	}, [i]), {
		selectionMode: t,
		disallowEmptySelection: n,
		selectionBehavior: m,
		setSelectionBehavior: h,
		get isFocused() {
			return o.current;
		},
		setFocused(e) {
			o.current = e, s(e);
		},
		get focusedKey() {
			return c.current;
		},
		get childFocusStrategy() {
			return l.current;
		},
		setFocusedKey(e, t = "first") {
			c.current = e, l.current = t, u(e);
		},
		selectedKeys: d,
		setSelectedKeys(e) {
			(r || !Bi(e, d)) && f(e);
		},
		disabledKeys: p,
		disabledBehavior: a
	};
}
function Hi(e, t) {
	return e ? e === "all" ? "all" : new zi(e) : t;
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/selection/SelectionManager.mjs
var Ui = class e {
	constructor(e, t, n) {
		this.collection = e, this.state = t, this.allowsCellSelection = n?.allowsCellSelection ?? !1, this._isSelectAll = null, this.layoutDelegate = n?.layoutDelegate || null, this.fullCollection = n?.fullCollection || null;
	}
	get selectionMode() {
		return this.state.selectionMode;
	}
	get disallowEmptySelection() {
		return this.state.disallowEmptySelection;
	}
	get selectionBehavior() {
		return this.state.selectionBehavior;
	}
	setSelectionBehavior(e) {
		this.state.setSelectionBehavior(e);
	}
	get isFocused() {
		return this.state.isFocused;
	}
	setFocused(e) {
		this.state.setFocused(e);
	}
	get focusedKey() {
		return this.state.focusedKey;
	}
	get childFocusStrategy() {
		return this.state.childFocusStrategy;
	}
	setFocusedKey(e, t) {
		(e == null || this.collection.getItem(e)) && this.state.setFocusedKey(e, t);
	}
	get selectedKeys() {
		return this.state.selectedKeys === "all" ? new Set(this.getSelectAllKeys()) : this.state.selectedKeys;
	}
	get rawSelection() {
		return this.state.selectedKeys;
	}
	isSelected(e) {
		if (this.state.selectionMode === "none") return !1;
		let t = this.getKey(e);
		return t == null ? !1 : this.state.selectedKeys === "all" ? this.canSelectItem(t) : this.state.selectedKeys.has(t);
	}
	get isEmpty() {
		return this.state.selectedKeys !== "all" && this.state.selectedKeys.size === 0;
	}
	get isSelectAll() {
		if (this.isEmpty) return !1;
		if (this.state.selectedKeys === "all") return !0;
		if (this._isSelectAll != null) return this._isSelectAll;
		let e = this.getSelectAllKeys(), t = this.state.selectedKeys;
		return this._isSelectAll = e.every((e) => t.has(e)), this._isSelectAll;
	}
	get firstSelectedKey() {
		let e = null;
		for (let t of this.state.selectedKeys) {
			let n = this.collection.getItem(t);
			(!e || n && ji(this.collection, n, e) < 0) && (e = n);
		}
		return e?.key ?? null;
	}
	get lastSelectedKey() {
		let e = null;
		for (let t of this.state.selectedKeys) {
			let n = this.collection.getItem(t);
			(!e || n && ji(this.collection, n, e) > 0) && (e = n);
		}
		return e?.key ?? null;
	}
	get disabledKeys() {
		return this.state.disabledKeys;
	}
	get disabledBehavior() {
		return this.state.disabledBehavior;
	}
	extendSelection(e) {
		if (this.selectionMode === "none") return;
		if (this.selectionMode === "single") {
			this.replaceSelection(e);
			return;
		}
		let t = this.getKey(e);
		if (t == null) return;
		let n;
		if (this.state.selectedKeys === "all") n = new zi([t], t, t);
		else {
			let e = this.state.selectedKeys, r = e.anchorKey ?? t;
			n = new zi(e, r, t);
			for (let i of this.getKeyRange(r, e.currentKey ?? t)) n.delete(i);
			for (let e of this.getKeyRange(t, r)) this.canSelectItem(e) && n.add(e);
		}
		this.state.setSelectedKeys(n);
	}
	getKeyRange(e, t) {
		let n = this.collection.getItem(e), r = this.collection.getItem(t);
		return n && r ? ji(this.collection, n, r) <= 0 ? this.getKeyRangeInternal(e, t) : this.getKeyRangeInternal(t, e) : [];
	}
	getKeyRangeInternal(e, t) {
		if (this.layoutDelegate?.getKeyRange) return this.layoutDelegate.getKeyRange(e, t);
		let n = [], r = e;
		for (; r != null;) {
			let e = this.collection.getItem(r);
			if (e && (e.type === "item" || e.type === "cell" && this.allowsCellSelection) && n.push(r), r === t) return n;
			r = this.collection.getKeyAfter(r);
		}
		return [];
	}
	getKey(e) {
		let t = this.collection.getItem(e);
		if (!t || t.type === "cell" && this.allowsCellSelection) return e;
		for (; t && t.type !== "item" && t.parentKey != null;) t = this.collection.getItem(t.parentKey);
		return !t || t.type !== "item" ? null : t.key;
	}
	toggleSelection(e) {
		if (this.selectionMode === "none") return;
		if (this.selectionMode === "single" && !this.isSelected(e)) {
			this.replaceSelection(e);
			return;
		}
		let t = this.getKey(e);
		if (t == null) return;
		let n = new zi(this.state.selectedKeys === "all" ? this.getSelectAllKeys() : this.state.selectedKeys);
		n.has(t) ? n.delete(t) : this.canSelectItem(t) && (n.add(t), n.anchorKey = t, n.currentKey = t), !(this.disallowEmptySelection && n.size === 0) && this.state.setSelectedKeys(n);
	}
	replaceSelection(e) {
		if (this.selectionMode === "none") return;
		let t = this.getKey(e);
		if (t == null) return;
		let n = this.canSelectItem(t) ? new zi([t], t, t) : new zi();
		this.state.setSelectedKeys(n);
	}
	setSelectedKeys(e) {
		if (this.selectionMode === "none") return;
		let t = new zi();
		for (let n of e) {
			let e = this.getKey(n);
			if (e != null && (t.add(e), this.selectionMode === "single")) break;
		}
		this.state.setSelectedKeys(t);
	}
	getSelectAllKeys() {
		let e = this.fullCollection ?? this.collection, t = [], n = (r) => {
			for (; r != null;) {
				if (this.canSelectItemIn(r, e)) {
					let i = e.getItem(r);
					i?.type === "item" && t.push(r), i?.hasChildNodes && (this.allowsCellSelection || i.type !== "item") && n(ki(Oi(i, e))?.key ?? null);
				}
				r = e.getKeyAfter(r);
			}
		};
		return n(e.getFirstKey()), t;
	}
	selectAll() {
		!this.isSelectAll && this.selectionMode === "multiple" && this.state.setSelectedKeys("all");
	}
	clearSelection() {
		!this.disallowEmptySelection && (this.state.selectedKeys === "all" || this.state.selectedKeys.size > 0) && this.state.setSelectedKeys(new zi());
	}
	toggleSelectAll() {
		this.isSelectAll ? this.clearSelection() : this.selectAll();
	}
	select(e, t) {
		this.selectionMode !== "none" && (this.selectionMode === "single" ? this.isSelected(e) && !this.disallowEmptySelection ? this.toggleSelection(e) : this.replaceSelection(e) : this.selectionBehavior === "toggle" || t && (t.pointerType === "touch" || t.pointerType === "virtual") ? this.toggleSelection(e) : this.replaceSelection(e));
	}
	isSelectionEqual(e) {
		if (e === this.state.selectedKeys) return !0;
		let t = this.selectedKeys;
		if (e.size !== t.size) return !1;
		for (let n of e) if (!t.has(n)) return !1;
		for (let n of t) if (!e.has(n)) return !1;
		return !0;
	}
	canSelectItem(e) {
		return this.canSelectItemIn(e, this.collection);
	}
	canSelectItemIn(e, t) {
		if (this.state.selectionMode === "none" || this.state.disabledKeys.has(e)) return !1;
		let n = t.getItem(e);
		return !(!n || n?.props?.isDisabled || n.type === "cell" && !this.allowsCellSelection);
	}
	isDisabled(e) {
		let t = this.collection.getItem(e);
		return this.state.disabledBehavior === "all" && (this.state.disabledKeys.has(e) || !!t?.props?.isDisabled) && t?.props?.disabledBehavior !== "selection";
	}
	isLink(e) {
		return !!this.collection.getItem(e)?.props?.href;
	}
	getItemProps(e) {
		return this.collection.getItem(e)?.props;
	}
	withCollection(t) {
		return new e(t, this.state, {
			allowsCellSelection: this.allowsCellSelection,
			layoutDelegate: this.layoutDelegate || void 0,
			fullCollection: this.fullCollection ?? this.collection
		});
	}
}, Wi = class {
	build(e, t) {
		return this.context = t, Gi(() => this.iterateCollection(e));
	}
	*iterateCollection(e) {
		let { children: t, items: n } = e;
		if (q.isValidElement(t) && t.type === q.Fragment) yield* this.iterateCollection({
			children: t.props.children,
			items: n
		});
		else if (typeof t == "function") {
			if (!n) throw Error("props.children was a function but props.items is missing");
			let e = 0;
			for (let r of n) yield* this.getFullNode({
				value: r,
				index: e
			}, { renderer: t }), e++;
		} else {
			let e = [];
			q.Children.forEach(t, (t) => {
				t && e.push(t);
			});
			let n = 0;
			for (let t of e) {
				let e = this.getFullNode({
					element: t,
					index: n
				}, {});
				for (let t of e) n++, yield t;
			}
		}
	}
	getKey(e, t, n, r) {
		if (e.key != null) return e.key;
		if (t.type === "cell" && t.key != null) return `${r}${t.key}`;
		let i = t.value;
		if (i != null) {
			let e = i.key ?? i.id;
			if (e == null) throw Error("No key found for item");
			return e;
		}
		return r ? `${r}.${t.index}` : `$.${t.index}`;
	}
	getChildState(e, t) {
		return { renderer: t.renderer || e.renderer };
	}
	*getFullNode(e, t, n, r) {
		if (q.isValidElement(e.element) && e.element.type === q.Fragment) {
			let i = [];
			q.Children.forEach(e.element.props.children, (e) => {
				i.push(e);
			});
			let a = e.index ?? 0;
			for (let e of i) yield* this.getFullNode({
				element: e,
				index: a++
			}, t, n, r);
			return;
		}
		let i = e.element;
		if (!i && e.value && t && t.renderer) {
			let n = this.cache.get(e.value);
			if (n && (!n.shouldInvalidate || !n.shouldInvalidate(this.context))) {
				n.index = e.index, n.parentKey = r ? r.key : null, yield n;
				return;
			}
			i = t.renderer(e.value);
		}
		if (q.isValidElement(i)) {
			let a = i.type;
			if (typeof a != "function" && typeof a.getCollectionNode != "function") {
				let e = i.type;
				throw Error(`Unknown element <${e}> in collection.`);
			}
			let o = a.getCollectionNode(i.props, this.context), s = e.index ?? 0, c = o.next();
			for (; !c.done && c.value;) {
				let a = c.value;
				e.index = s;
				let l = a.key ?? null;
				l ??= a.element ? null : this.getKey(i, e, t, n);
				let u = [...this.getFullNode({
					...a,
					key: l,
					index: s,
					wrapper: Ki(e.wrapper, a.wrapper)
				}, this.getChildState(t, a), n ? `${n}${i.key}` : i.key, r)];
				for (let t of u) {
					if (t.value = a.value ?? e.value ?? null, t.value && this.cache.set(t.value, t), e.type && t.type !== e.type) throw Error(`Unsupported type <${qi(t.type)}> in <${qi(r?.type ?? "unknown parent type")}>. Only <${qi(e.type)}> is supported.`);
					s++, yield t;
				}
				c = o.next(u);
			}
			return;
		}
		if (e.key == null || e.type == null) return;
		let a = this, o = {
			type: e.type,
			props: e.props,
			key: e.key,
			parentKey: r ? r.key : null,
			value: e.value ?? null,
			level: (r?.level ?? 0) + +(r?.type === "item"),
			index: e.index,
			rendered: e.rendered,
			textValue: e.textValue ?? "",
			"aria-label": e["aria-label"],
			wrapper: e.wrapper,
			shouldInvalidate: e.shouldInvalidate,
			hasChildNodes: e.hasChildNodes || !1,
			childNodes: Gi(function* () {
				if (!e.hasChildNodes || !e.childNodes) return;
				let n = 0;
				for (let r of e.childNodes()) {
					r.key != null && (r.key = `${o.key}${r.key}`);
					let e = a.getFullNode({
						...r,
						index: n
					}, a.getChildState(t, r), o.key, o);
					for (let t of e) n++, yield t;
				}
			})
		};
		yield o;
	}
	constructor() {
		this.cache = /* @__PURE__ */ new WeakMap();
	}
};
function Gi(e) {
	let t = [], n = null;
	return { *[Symbol.iterator]() {
		for (let e of t) yield e;
		n ||= e();
		for (let e of n) t.push(e), yield e;
	} };
}
function Ki(e, t) {
	if (e && t) return (n) => e(t(n));
	if (e) return e;
	if (t) return t;
}
function qi(e) {
	return e[0].toUpperCase() + e.slice(1);
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/collections/useCollection.mjs
function Ji(e, t, n) {
	let r = (0, q.useMemo)(() => new Wi(), []), { children: i, items: a, collection: o } = e;
	return (0, q.useMemo)(() => o || t(r.build({
		children: i,
		items: a
	}, n)), [
		r,
		i,
		a,
		o,
		n,
		t
	]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/list/useListState.mjs
function Yi(e) {
	let { filter: t, layoutDelegate: n } = e, r = Vi(e), i = (0, q.useMemo)(() => e.disabledKeys ? new Set(e.disabledKeys) : /* @__PURE__ */ new Set(), [e.disabledKeys]), a = Ji(e, (0, q.useCallback)((e) => t ? new Ri(t(e)) : new Ri(e), [t]), (0, q.useMemo)(() => ({ suppressTextValueWarning: e.suppressTextValueWarning }), [e.suppressTextValueWarning])), o = (0, q.useMemo)(() => new Ui(a, r, { layoutDelegate: n }), [
		a,
		r,
		n
	]);
	return Zi(a, o), {
		collection: a,
		disabledKeys: i,
		selectionManager: o
	};
}
function Xi(e, t) {
	let n = (0, q.useMemo)(() => t ? e.collection.filter(t) : e.collection, [e.collection, t]), r = e.selectionManager.withCollection(n);
	return Zi(n, r), {
		collection: n,
		selectionManager: r,
		disabledKeys: e.disabledKeys
	};
}
function Zi(e, t) {
	let n = (0, q.useRef)(null);
	(0, q.useEffect)(() => {
		if (t.focusedKey != null && !e.getItem(t.focusedKey) && n.current) {
			let r = n.current.getKeyAfter(t.focusedKey), i = null;
			for (; r != null;) {
				let a = e.getItem(r);
				if (a && a.type === "item" && !t.isDisabled(r)) {
					i = r;
					break;
				}
				r = n.current.getKeyAfter(r);
			}
			if (i == null) for (r = n.current.getKeyBefore(t.focusedKey); r != null;) {
				let a = e.getItem(r);
				if (a && a.type === "item" && !t.isDisabled(r)) {
					i = r;
					break;
				}
				r = n.current.getKeyBefore(r);
			}
			t.setFocusedKey(i);
		}
		n.current = e;
	}, [e, t]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useLoadMoreSentinel.mjs
function Qi(e, t) {
	let { collection: n, onLoadMore: r, scrollOffset: i = 1 } = e, a = (0, q.useRef)(null), o = be((e) => {
		for (let t of e) t.isIntersecting && r && r();
	});
	N(() => (t.current && (a.current = new IntersectionObserver(o, {
		root: un(t?.current),
		rootMargin: `0px ${100 * i}% ${100 * i}% ${100 * i}%`
	}), a.current.observe(t.current)), () => {
		a.current && a.current.disconnect();
	}), [
		n,
		t,
		i
	]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/ListBox.mjs
var $i = /*#__PURE__*/ (0, q.createContext)(null), ea = /*#__PURE__*/ (0, q.createContext)(null), ta = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	[e, t] = h(e, t, $i);
	let n = (0, q.useContext)(ea);
	return n ? /*#__PURE__*/ q.createElement(ra, {
		state: n,
		props: e,
		listBoxRef: t
	}) : /*#__PURE__*/ q.createElement(Ft, { content: /*#__PURE__*/ q.createElement(Kt, e) }, (n) => /*#__PURE__*/ q.createElement(na, {
		props: e,
		listBoxRef: t,
		collection: n
	}));
});
function na({ props: e, listBoxRef: t, collection: n }) {
	e = {
		...e,
		collection: n,
		children: null,
		items: null
	};
	let { layoutDelegate: r } = (0, q.useContext)(Qt), i = Yi({
		...e,
		layoutDelegate: r
	});
	return /*#__PURE__*/ q.createElement(ra, {
		state: i,
		props: e,
		listBoxRef: t
	});
}
function ra({ state: e, props: t, listBoxRef: n }) {
	[t, n] = h(t, n, gt);
	let { dragAndDropHooks: r, layout: i = "stack", orientation: a = "vertical", filter: o } = t, s = Xi(e, o), { collection: c, selectionManager: l } = s, u = !!r?.useDraggableCollectionState, f = !!r?.useDroppableCollectionState, { direction: p } = et(), { disabledBehavior: m, disabledKeys: g } = l, _ = bi({
		usage: "search",
		sensitivity: "base"
	}), { isVirtualized: v, layoutDelegate: y, dropTargetDelegate: b, CollectionRoot: x } = (0, q.useContext)(Qt), S = (0, q.useMemo)(() => t.keyboardDelegate || new vi({
		collection: c,
		collator: _,
		ref: n,
		disabledKeys: g,
		disabledBehavior: m,
		layout: i,
		orientation: a,
		direction: p,
		layoutDelegate: y
	}), [
		c,
		_,
		n,
		m,
		g,
		a,
		p,
		t.keyboardDelegate,
		i,
		y
	]), { listBoxProps: C } = Si({
		...t,
		shouldSelectOnPressUp: u || t.shouldSelectOnPressUp,
		keyboardDelegate: S,
		isVirtualized: v
	}, s, n);
	(0, q.useRef)(u), (0, q.useRef)(f), (0, q.useEffect)(() => {}, [u, f]);
	let w, T, E, D = !1, k = null, A = (0, q.useRef)(null);
	if (u && r) {
		w = r.useDraggableCollectionState({
			collection: c,
			selectionManager: l,
			preview: r.renderDragPreview ? A : void 0
		}), r.useDraggableCollection({}, w, n);
		let e = r.DragPreview;
		k = r.renderDragPreview ? /*#__PURE__*/ q.createElement(e, { ref: A }, r.renderDragPreview) : null;
	}
	if (f && r) {
		T = r.useDroppableCollectionState({
			collection: c,
			selectionManager: l
		});
		let e = r.dropTargetDelegate || b || new r.ListDropTargetDelegate(c, n, {
			orientation: a,
			layout: i,
			direction: p
		});
		E = r.useDroppableCollection({
			keyboardDelegate: S,
			dropTargetDelegate: e
		}, T, n), D = T.isDropTarget({ type: "root" });
	}
	let { focusProps: j, isFocused: M, isFocusVisible: N } = F(), P = s.collection.size === 0, I = {
		isDropTarget: D,
		isEmpty: P,
		isFocused: M,
		isFocusVisible: N,
		layout: t.layout || "stack",
		orientation: a,
		state: s
	}, ee = U({
		...t,
		children: void 0,
		defaultClassName: "react-aria-ListBox",
		values: I
	}), L = null;
	P && t.renderEmptyState && (L = /*#__PURE__*/ q.createElement("div", {
		role: "option",
		style: { display: "contents" }
	}, t.renderEmptyState(I)));
	let z = H(t, { global: !0 });
	return /*#__PURE__*/ q.createElement(zr, null, /*#__PURE__*/ q.createElement(O.div, {
		...R(z, ee, C, j, E?.collectionProps),
		ref: n,
		slot: t.slot || void 0,
		onScroll: t.onScroll,
		"data-drop-target": D || void 0,
		"data-empty": P || void 0,
		"data-focused": M || void 0,
		"data-focus-visible": N || void 0,
		"data-layout": t.layout || "stack",
		"data-orientation": a
	}, /*#__PURE__*/ q.createElement(d, { values: [
		[$i, t],
		[ea, s],
		[Sr, {
			dragAndDropHooks: r,
			dragState: w,
			dropState: T
		}],
		[jr, { elementType: "div" }],
		[Cr, { render: oa }],
		[Jt, {
			name: "ListBoxSection",
			render: ia
		}]
	] }, /*#__PURE__*/ q.createElement(kr, null, /*#__PURE__*/ q.createElement(x, {
		collection: c,
		scrollRef: n,
		persistedKeys: Er(l, r, T),
		renderDropIndicator: Tr(r, T)
	}))), L, k));
}
function ia(e, t, n, r = "react-aria-ListBoxSection") {
	let i = (0, q.useContext)(ea), { dragAndDropHooks: a, dropState: o } = (0, q.useContext)(Sr), { CollectionBranch: s } = (0, q.useContext)(Qt), [c, l] = m(), { headingProps: u, groupProps: d } = Ii({
		heading: l,
		"aria-label": e["aria-label"] ?? void 0
	}), f = U({
		...e,
		id: void 0,
		children: void 0,
		defaultClassName: r,
		values: void 0
	}), p = H(e, { global: !0 });
	return delete p.id, /*#__PURE__*/ q.createElement(O.section, {
		...R(p, f, d),
		ref: t
	}, /*#__PURE__*/ q.createElement(Dr.Provider, { value: {
		...u,
		ref: c
	} }, /*#__PURE__*/ q.createElement(s, {
		collection: i.collection,
		parent: n,
		renderDropIndicator: Tr(a, o)
	})));
}
var aa = /*#__PURE__*/ Ut(xt, function(e, t, n) {
	let r = i(t), a = (0, q.useContext)(ea), { dragAndDropHooks: o, dragState: s, dropState: c } = (0, q.useContext)(Sr), l = s && !(s.isDisabled || s.selectionManager.isDisabled(n.key)), { optionProps: u, labelProps: f, descriptionProps: p, ...m } = Fi({
		key: n.key,
		"aria-label": e?.["aria-label"]
	}, a, r), { hoverProps: h, isHovered: g } = fe({
		isDisabled: !m.allowsSelection && !m.hasAction && !l,
		onHoverStart: n.props.onHoverStart,
		onHoverChange: n.props.onHoverChange,
		onHoverEnd: n.props.onHoverEnd
	}), { keyboardProps: _ } = ve(e), { focusProps: v } = xe(e), y = null;
	s && o && (y = o.useDraggableItem({
		key: n.key,
		hasAction: m.hasAction
	}, s));
	let b = null;
	c && o && (b = o.useDroppableItem({ target: {
		type: "item",
		key: n.key,
		dropPosition: "on"
	} }, c, r));
	let x = s && s.isDragging(n.key), S = U({
		...e,
		id: void 0,
		children: e.children,
		defaultClassName: "react-aria-ListBoxItem",
		values: {
			...m,
			isHovered: g,
			selectionMode: a.selectionManager.selectionMode,
			selectionBehavior: a.selectionManager.selectionBehavior,
			allowsDragging: !!s,
			isDragging: x,
			isDropTarget: b?.isDropTarget
		}
	});
	(0, q.useEffect)(() => {
		n.textValue;
	}, [n.textValue]);
	let C = e.href ? O.a : O.div, w = H(e, { global: !0 });
	return delete w.id, delete w.onClick, e.href && u.tabIndex == null && (u.tabIndex = -1), /*#__PURE__*/ q.createElement(C, {
		...R(w, S, u, h, _, v, y?.dragProps, b?.dropProps),
		ref: r,
		"data-allows-dragging": !!s || void 0,
		"data-selected": m.isSelected || void 0,
		"data-disabled": m.isDisabled || void 0,
		"data-hovered": g || void 0,
		"data-focused": m.isFocused || void 0,
		"data-focus-visible": m.isFocusVisible || void 0,
		"data-pressed": m.isPressed || void 0,
		"data-dragging": x || void 0,
		"data-drop-target": b?.isDropTarget || void 0,
		"data-selection-mode": a.selectionManager.selectionMode === "none" ? void 0 : a.selectionManager.selectionMode
	}, /*#__PURE__*/ q.createElement(d, { values: [[sn, { slots: {
		[k]: f,
		label: f,
		description: p
	} }], [Ar, { isSelected: m.isSelected }]] }, S.children));
});
function oa(e, t) {
	t = i(t);
	let { dragAndDropHooks: n, dropState: r } = (0, q.useContext)(Sr), { dropIndicatorProps: a, isHidden: o, isDropTarget: s } = n.useDropIndicator(e, r, t);
	return o ? null : /*#__PURE__*/ q.createElement(ca, {
		...e,
		dropIndicatorProps: a,
		isDropTarget: s,
		ref: t
	});
}
function sa(e, t) {
	let { dropIndicatorProps: n, isDropTarget: r, ...i } = e, a = U({
		...i,
		defaultClassName: "react-aria-DropIndicator",
		values: { isDropTarget: r }
	});
	return /*#__PURE__*/ q.createElement(q.Fragment, null, /*#__PURE__*/ q.createElement(O.div, {
		...n,
		...a,
		role: "option",
		ref: t,
		"data-drop-target": r || void 0
	}));
}
var ca = /*#__PURE__*/ (0, q.forwardRef)(sa);
Ut(bt, function(e, t, n) {
	let r = (0, q.useContext)(ea), { isLoading: i, onLoadMore: a, scrollOffset: o, ...s } = e, c = (0, q.useRef)(null);
	Qi((0, q.useMemo)(() => ({
		onLoadMore: a,
		collection: r?.collection,
		sentinelRef: c,
		scrollOffset: o
	}), [
		a,
		o,
		r?.collection
	]), c);
	let l = U({
		...s,
		id: void 0,
		children: n.rendered,
		defaultClassName: "react-aria-ListBoxLoadingIndicator",
		values: void 0
	});
	return /*#__PURE__*/ q.createElement(q.Fragment, null, /*#__PURE__*/ q.createElement("div", {
		style: {
			position: "relative",
			width: 0,
			height: 0
		},
		inert: Li(!0)
	}, /*#__PURE__*/ q.createElement("div", {
		"data-testid": "loadMoreSentinel",
		ref: c,
		style: {
			position: "absolute",
			height: 1,
			width: 1
		}
	})), i && l.children && /*#__PURE__*/ q.createElement(q.Fragment, null, /*#__PURE__*/ q.createElement(O.div, {
		...R(H(e, { global: !0 }), { tabIndex: -1 }),
		...l,
		role: "option",
		ref: t
	}, l.children)));
});
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/OverlayArrow.mjs
var la = /*#__PURE__*/ (0, q.createContext)({ placement: "bottom" }), ua = typeof HTMLElement < "u" && "inert" in HTMLElement.prototype;
function da(e) {
	return e.dataset.liveAnnouncer === "true" || e.dataset.reactAriaTopLayer !== void 0;
}
var fa = /* @__PURE__ */ new WeakMap(), Z = [];
function pa(e, t) {
	let n = D(e?.[0]), r = t instanceof n.Element ? { root: t } : t, i = r?.root ?? document.body, a = r?.shouldUseInert && ua, o = new Set(e), s = /* @__PURE__ */ new Set(), c = (e) => a && e instanceof n.HTMLElement ? e.inert : e.getAttribute("aria-hidden") === "true", l = (e, t) => {
		a && e instanceof n.HTMLElement ? e.inert = t : t ? e.setAttribute("aria-hidden", "true") : (e.removeAttribute("aria-hidden"), e instanceof n.HTMLElement && (e.inert = !1));
	}, u = /* @__PURE__ */ new Set();
	if (f()) for (let t of e) {
		let e = t;
		for (; e && e !== i;) {
			let t = e.getRootNode();
			"shadowRoot" in t && u.add(t.shadowRoot), e = t.parentNode;
		}
	}
	let d = (e) => {
		for (let t of e.querySelectorAll("[data-live-announcer], [data-react-aria-top-layer]")) o.add(t);
		let t = (e) => {
			if (s.has(e) || o.has(e) || e.parentElement && s.has(e.parentElement) && e.parentElement.getAttribute("role") !== "row") return NodeFilter.FILTER_REJECT;
			for (let t of o) if (S(e, t)) return NodeFilter.FILTER_SKIP;
			return NodeFilter.FILTER_ACCEPT;
		}, n = Ir(T(e), e, NodeFilter.SHOW_ELEMENT, { acceptNode: t }), r = t(e);
		if (r === NodeFilter.FILTER_ACCEPT && p(e), r !== NodeFilter.FILTER_REJECT) {
			let e = n.nextNode();
			for (; e != null;) p(e), e = n.nextNode();
		}
	}, p = (e) => {
		let t = fa.get(e) ?? 0;
		c(e) && t === 0 || (t === 0 && l(e, !0), s.add(e), fa.set(e, t + 1));
	};
	Z.length && Z[Z.length - 1].disconnect(), d(i);
	let m = new MutationObserver((e) => {
		for (let t of e) if (t.type === "childList") {
			if (t.target.isConnected && ![...o, ...s].some((e) => S(e, t.target))) for (let e of t.addedNodes) (e instanceof HTMLElement || e instanceof SVGElement) && da(e) ? o.add(e) : e instanceof Element && d(e);
			if (f()) {
				for (let e of u) if (!e.isConnected) {
					m.disconnect();
					break;
				}
			}
		}
	});
	m.observe(i, {
		childList: !0,
		subtree: !0
	});
	let h = /* @__PURE__ */ new Set();
	if (f()) for (let e of u) {
		let t = new MutationObserver((e) => {
			for (let t of e) if (t.type === "childList") {
				if (t.target.isConnected && ![...o, ...s].some((e) => S(e, t.target))) for (let e of t.addedNodes) (e instanceof HTMLElement || e instanceof SVGElement) && da(e) ? o.add(e) : e instanceof Element && d(e);
				if (f()) {
					for (let e of u) if (!e.isConnected) {
						m.disconnect();
						break;
					}
				}
			}
		});
		t.observe(e, {
			childList: !0,
			subtree: !0
		}), h.add(t);
	}
	let g = {
		visibleNodes: o,
		hiddenNodes: s,
		observe() {
			m.observe(i, {
				childList: !0,
				subtree: !0
			});
		},
		disconnect() {
			m.disconnect();
		}
	};
	return Z.push(g), () => {
		if (m.disconnect(), f()) for (let e of h) e.disconnect();
		for (let e of s) {
			let t = fa.get(e);
			t != null && (t === 1 ? (l(e, !1), fa.delete(e)) : fa.set(e, t - 1));
		}
		g === Z[Z.length - 1] ? (Z.pop(), Z.length && Z[Z.length - 1].observe()) : Z.splice(Z.indexOf(g), 1);
	};
}
function ma(e) {
	let t = Z[Z.length - 1];
	if (t && !t.visibleNodes.has(e)) return t.visibleNodes.add(e), () => {
		t.visibleNodes.delete(e);
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/calculatePosition.mjs
var Q = {
	top: "top",
	bottom: "top",
	left: "left",
	right: "left"
}, ha = {
	top: "bottom",
	bottom: "top",
	left: "right",
	right: "left"
}, ga = {
	top: "left",
	left: "top"
}, _a = {
	top: "height",
	left: "width"
}, va = {
	width: "totalWidth",
	height: "totalHeight"
}, ya = {}, ba = () => typeof document < "u" ? window.visualViewport : null;
function xa(e, t) {
	let n = 0, r = 0, i = 0, o = 0, s = 0, c = 0, l = {}, u = (t?.scale ?? 1) > 1;
	if (e.tagName === "BODY" || e.tagName === "HTML") {
		let a = document.documentElement;
		i = a.clientWidth, o = a.clientHeight, n = t?.width ?? i, r = t?.height ?? o, l.top = a.scrollTop || e.scrollTop, l.left = a.scrollLeft || e.scrollLeft, t && (s = t.offsetTop, c = t.offsetLeft);
	} else ({width: n, height: r, top: s, left: c} = Ma(e, !1)), l.top = e.scrollTop, l.left = e.scrollLeft, i = n, o = r;
	return a() && (e.tagName === "BODY" || e.tagName === "HTML") && u && (l.top = 0, l.left = 0, s = t?.pageTop ?? 0, c = t?.pageLeft ?? 0), {
		width: n,
		height: r,
		totalWidth: i,
		totalHeight: o,
		scroll: l,
		top: s,
		left: c
	};
}
function Sa(e) {
	return {
		top: e.scrollTop,
		left: e.scrollLeft,
		width: e.scrollWidth,
		height: e.scrollHeight
	};
}
function Ca(e, t, n, r, i, a, o) {
	let s = i.scroll[e] ?? 0, c = r[_a[e]], l = o[e] + r.scroll[Q[e]] + a, u = o[e] + r.scroll[Q[e]] + c - a, d = t - s + r.scroll[Q[e]] + o[e] - r[Q[e]], f = t - s + n + r.scroll[Q[e]] + o[e] - r[Q[e]];
	return d < l ? l - d : f > u ? Math.max(u - f, l - d) : 0;
}
function wa(e) {
	let t = window.getComputedStyle(e);
	return {
		top: parseInt(t.marginTop, 10) || 0,
		bottom: parseInt(t.marginBottom, 10) || 0,
		left: parseInt(t.marginLeft, 10) || 0,
		right: parseInt(t.marginRight, 10) || 0
	};
}
function Ta(e) {
	if (ya[e]) return ya[e];
	let [t, n] = e.split(" "), r = Q[t] || "right", i = ga[r];
	Q[n] || (n = "center");
	let a = _a[r], o = _a[i];
	return ya[e] = {
		placement: t,
		crossPlacement: n,
		axis: r,
		crossAxis: i,
		size: a,
		crossSize: o
	}, ya[e];
}
function Ea(e, t, n, r, i, a, o, s, c, l, u) {
	let { placement: d, crossPlacement: f, axis: p, crossAxis: m, size: h, crossSize: g } = r, _ = {};
	_[m] = e[m] ?? 0, f === "center" ? _[m] += ((e[g] ?? 0) - (n[g] ?? 0)) / 2 : f !== m && (_[m] += (e[g] ?? 0) - (n[g] ?? 0)), _[m] += a;
	let v = e[m] - n[g] + c + l, y = e[m] + e[g] - c - l;
	if (_[m] = rn(_[m], v, y), d === p) {
		let t = s ? u[h] : u[va[h]];
		_[ha[p]] = Math.floor(t - e[p] + i);
	} else _[p] = Math.floor(e[p] + e[h] + i);
	return _;
}
function Da(e, t, n, r, i, a, o, s, c, l, u) {
	let d = (e.top == null ? c[va.height] - (e.bottom ?? 0) - o : e.top) - (c.scroll.top ?? 0), f = l ? n.top : 0, p = {
		top: Math.max(t.top + f, (u?.offsetTop ?? t.top) + f),
		bottom: Math.min(t.top + t.height + f, (u?.offsetTop ?? 0) + (u?.height ?? 0))
	};
	return s === "top" ? Math.max(0, d + o - p.top - ((i.top ?? 0) + (i.bottom ?? 0) + a)) : Math.max(0, p.bottom - d - ((i.top ?? 0) + (i.bottom ?? 0) + a));
}
function Oa(e, t, n, r, i, a, o, s) {
	let { placement: c, axis: l, size: u } = a;
	return c === l ? Math.max(0, n[l] - (o.scroll[l] ?? 0) - (e[l] + (s ? t[l] : 0)) - (r[l] ?? 0) - r[ha[l]] - i) : Math.max(0, e[u] + e[l] + (s ? t[l] : 0) - n[l] - n[u] + (o.scroll[l] ?? 0) - (r[l] ?? 0) - r[ha[l]] - i);
}
function ka(e, t, n, r, i, a, o, s, c, l, u, d, f, p, m, h, g, _) {
	let v = Ta(e), { size: y, crossAxis: b, crossSize: x, placement: S, crossPlacement: C } = v, w = Ea(t, s, n, v, u, d, l, f, m, h, c), T = u, E = Oa(s, l, t, i, a + u, v, c, g);
	if (o && n[y] > E) {
		let e = Ta(`${ha[S]} ${C}`), r = Ea(t, s, n, e, u, d, l, f, m, h, c);
		Oa(s, l, t, i, a + u, e, c, g) > E && (v = e, w = r, T = u);
	}
	let D = "bottom";
	v.axis === "top" ? v.placement === "top" ? D = "top" : v.placement === "bottom" && (D = "bottom") : v.crossAxis === "top" && (v.crossPlacement === "top" ? D = "bottom" : v.crossPlacement === "bottom" && (D = "top"));
	let O = Ca(b, w[b], n[x], s, c, a, l);
	w[b] += O;
	let k = Da(w, s, l, f, i, a, n.height, D, c, g, _);
	p && p < k && (k = p), n.height = Math.min(n.height, k), w = Ea(t, s, n, v, T, d, l, f, m, h, c), O = Ca(b, w[b], n[x], s, c, a, l), w[b] += O;
	let A = {}, j = t[b] - w[b] - i[Q[b]], M = j + .5 * t[x], N = m / 2 + h, P = Q[b] === "left" ? (i.left ?? 0) + (i.right ?? 0) : (i.top ?? 0) + (i.bottom ?? 0), F = n[x] - P - m / 2 - h;
	A[b] = rn(rn(M, t[b] + m / 2 - (w[b] + i[Q[b]]), t[b] + t[x] - m / 2 - (w[b] + i[Q[b]])), N, F), {placement: S, crossPlacement: C} = v, m ? j = A[b] : C === "right" ? j += t[x] : C === "center" && (j += t[x] / 2);
	let I = S === "left" || S === "top" ? n[y] : 0, ee = {
		x: S === "top" || S === "bottom" ? j : I,
		y: S === "left" || S === "right" ? j : I
	};
	return {
		position: w,
		maxHeight: k,
		arrowOffsetLeft: A.left,
		arrowOffsetTop: A.top,
		placement: S,
		triggerAnchorPoint: ee
	};
}
function Aa(e) {
	let { placement: t, targetNode: n, overlayNode: r, scrollNode: i, padding: a, shouldFlip: o, boundaryElement: s, offset: c, crossOffset: l, maxHeight: u, arrowSize: d = 0, arrowBoundaryOffset: f = 0, targetRect: p } = e, m = ba(), h = r instanceof HTMLElement ? Pa(r) : document.documentElement, g = h === document.documentElement, _ = window.getComputedStyle(h).position, v = !!_ && _ !== "static", y = g ? Ma(n, !1, p) : Na(n, h, !1, p);
	if (!g) {
		let { marginTop: e, marginLeft: t } = window.getComputedStyle(n);
		y.top += parseInt(e, 10) || 0, y.left += parseInt(t, 10) || 0;
	}
	let b = Ma(r, !0), x = wa(r);
	b.width += (x.left ?? 0) + (x.right ?? 0), b.height += (x.top ?? 0) + (x.bottom ?? 0);
	let C = Sa(i), w = xa(s, m), T = xa(h, m), E;
	if ((s.tagName === "BODY" || s.tagName === "HTML") && !g) {
		let e = ja(h, !1);
		E = {
			top: -(e.top - w.top),
			left: -(e.left - w.left),
			width: 0,
			height: 0
		};
	} else E = (s.tagName === "BODY" || s.tagName === "HTML") && g ? {
		top: 0,
		left: 0,
		width: 0,
		height: 0
	} : Na(s, h, !1);
	let D = S(s, h);
	return ka(t, y, b, C, x, a, o, w, T, E, c, l, v, u, d, f, D, m);
}
function ja(e, t) {
	let { top: n, left: r, width: i, height: a } = e.getBoundingClientRect();
	return t && e instanceof e.ownerDocument.defaultView.HTMLElement && (i = e.offsetWidth, a = e.offsetHeight), {
		top: n,
		left: r,
		width: i,
		height: a
	};
}
function Ma(e, t, n) {
	let { top: r, left: i, width: a, height: o } = n || ja(e, t), { scrollTop: s, scrollLeft: c, clientTop: l, clientLeft: u } = document.documentElement;
	return {
		top: r + s - l,
		left: i + c - u,
		width: a,
		height: o
	};
}
function Na(e, t, n, r) {
	let i = window.getComputedStyle(e), a;
	if (i.position === "fixed") a = r || ja(e, n);
	else {
		a = Ma(e, n, r);
		let i = Ma(t, n), o = window.getComputedStyle(t);
		i.top += (parseInt(o.borderTopWidth, 10) || 0) - t.scrollTop, i.left += (parseInt(o.borderLeftWidth, 10) || 0) - t.scrollLeft, a.top -= i.top, a.left -= i.left;
	}
	return a.top -= parseInt(i.marginTop, 10) || 0, a.left -= parseInt(i.marginLeft, 10) || 0, a;
}
function Pa(e) {
	let t = e.offsetParent;
	if (t && t === document.body && window.getComputedStyle(t).position === "static" && !Fa(t) && (t = document.documentElement), t == null) for (t = e.parentElement; t && !Fa(t);) t = t.parentElement;
	return t || document.documentElement;
}
function Fa(e) {
	let t = window.getComputedStyle(e);
	return t.transform !== "none" || /transform|perspective/.test(t.willChange) || t.filter !== "none" || t.contain === "paint" || "backdropFilter" in t && t.backdropFilter !== "none" || "WebkitBackdropFilter" in t && t.WebkitBackdropFilter !== "none";
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/useCloseOnScroll.mjs
var Ia = /* @__PURE__ */ new WeakMap();
function La(e) {
	let { triggerRef: t, isOpen: n, onClose: r } = e;
	(0, q.useEffect)(() => {
		if (!n || r === null) return;
		let e = (e) => {
			let n = E(e);
			if (!t.current || n instanceof Node && !S(n, t.current) || n instanceof HTMLInputElement || n instanceof HTMLTextAreaElement) return;
			let i = r || Ia.get(t.current);
			i && i();
		};
		return window.addEventListener("scroll", e, !0), () => {
			window.removeEventListener("scroll", e, !0);
		};
	}, [
		n,
		r,
		t
	]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useResizeObserver.mjs
function Ra() {
	return window.ResizeObserver !== void 0;
}
function za(e) {
	let { ref: t, box: n, onResize: r } = e, i = be(r);
	(0, q.useEffect)(() => {
		let e = t?.current;
		if (e) if (Ra()) {
			let t = new window.ResizeObserver((e) => {
				e.length && i();
			});
			return t.observe(e, { box: n }), () => {
				e && t.unobserve(e);
			};
		} else return window.addEventListener("resize", i, !1), () => {
			window.removeEventListener("resize", i, !1);
		};
	}, [t, n]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/useOverlayPosition.mjs
var Ba = typeof document < "u" ? window.visualViewport : null;
function Va(e) {
	let { direction: t } = et(), { arrowSize: n, targetRef: r, overlayRef: i, arrowRef: a, scrollRef: s = i, placement: c = "bottom", containerPadding: l = 12, shouldFlip: u = !0, boundaryElement: d = typeof document < "u" ? document.body : null, offset: f = 0, crossOffset: p = 0, shouldUpdatePosition: m = !0, isOpen: h = !0, onClose: g, maxHeight: _, arrowBoundaryOffset: v = 0, getTargetRect: y } = e, [b, x] = (0, q.useState)(null), S = [
		m,
		c,
		i.current,
		r.current,
		a?.current,
		s.current,
		l,
		u,
		d,
		f,
		p,
		h,
		t,
		_,
		v,
		n
	], C = (0, q.useRef)(Ba?.scale);
	(0, q.useEffect)(() => {
		h && (C.current = Ba?.scale);
	}, [h]);
	let w = (0, q.useCallback)(() => {
		if (m === !1 || !h || !i.current || !r.current || !d || Ba?.scale !== C.current) return;
		let e = null;
		if (s.current && Se(s.current)) {
			let t = o()?.getBoundingClientRect(), n = s.current.getBoundingClientRect();
			e = {
				type: "top",
				offset: (t?.top ?? 0) - n.top
			}, e.offset > n.height / 2 && (e.type = "bottom", e.offset = (t?.bottom ?? 0) - n.bottom);
		}
		let g = i.current;
		!_ && i.current && (g.style.top = "0px", g.style.bottom = "", g.style.maxHeight = (window.visualViewport?.height ?? window.innerHeight) + "px");
		let b = Aa({
			placement: Ua(c, t),
			overlayNode: i.current,
			targetNode: r.current,
			scrollNode: s.current || i.current,
			padding: l,
			shouldFlip: u,
			boundaryElement: d,
			offset: f,
			crossOffset: p,
			maxHeight: _,
			arrowSize: n ?? (a?.current ? ja(a.current, !0).width : 0),
			arrowBoundaryOffset: v,
			targetRect: y?.(r.current)
		});
		if (!b.position) return;
		g.style.top = "", g.style.bottom = "", g.style.left = "", g.style.right = "", Object.keys(b.position).forEach((e) => g.style[e] = b.position[e] + "px"), g.style.maxHeight = b.maxHeight == null ? "" : b.maxHeight + "px";
		let S = o();
		if (e && S && s.current) {
			let t = S.getBoundingClientRect(), n = s.current.getBoundingClientRect(), r = t[e.type] - n[e.type];
			s.current.scrollTop += r - e.offset;
		}
		x(b);
	}, S);
	N(w, S), Ha(w), za({
		ref: i,
		onResize: w
	}), za({
		ref: r,
		onResize: w
	});
	let T = (0, q.useRef)(!1);
	N(() => {
		let e, t = () => {
			T.current = !0, clearTimeout(e), e = setTimeout(() => {
				T.current = !1;
			}, 500), w();
		}, n = () => {
			T.current && t();
		};
		return Ba?.addEventListener("resize", t), Ba?.addEventListener("scroll", n), () => {
			Ba?.removeEventListener("resize", t), Ba?.removeEventListener("scroll", n);
		};
	}, [w]);
	let E = (0, q.useCallback)(() => {
		T.current || g?.();
	}, [g, T]);
	return La({
		triggerRef: r,
		isOpen: h,
		onClose: g && E
	}), {
		overlayProps: { style: {
			position: b ? "absolute" : "fixed",
			top: b ? void 0 : 0,
			left: b ? void 0 : 0,
			zIndex: 1e5,
			...b?.position,
			maxHeight: b?.maxHeight ?? "100vh"
		} },
		placement: b?.placement ?? null,
		triggerAnchorPoint: b?.triggerAnchorPoint ?? null,
		arrowProps: {
			"aria-hidden": "true",
			role: "presentation",
			style: {
				left: b?.arrowOffsetLeft,
				top: b?.arrowOffsetTop
			}
		},
		updatePosition: w
	};
}
function Ha(e) {
	N(() => (window.addEventListener("resize", e, !1), () => {
		window.removeEventListener("resize", e, !1);
	}), [e]);
}
function Ua(e, t) {
	return t === "rtl" ? e.replace("start", "right").replace("end", "left") : e.replace("start", "left").replace("end", "right");
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/interactions/useInteractOutside.mjs
function Wa(e) {
	let { ref: t, onInteractOutside: n, isDisabled: r, onInteractOutsideStart: i } = e, a = (0, q.useRef)({
		isPointerDown: !1,
		ignoreEmulatedMouseEvents: !1
	}), o = be((e) => {
		n && Ga(e, t) && (i && i(e), a.current.isPointerDown = !0);
	}), s = be((e) => {
		n && n(e);
	});
	(0, q.useEffect)(() => {
		let e = a.current;
		if (r) return;
		let n = t.current, i = T(n);
		if (typeof PointerEvent < "u") {
			let n = (n) => {
				e.isPointerDown && Ga(n, t) && s(n), e.isPointerDown = !1;
			};
			return i.addEventListener("pointerdown", o, !0), i.addEventListener("click", n, !0), () => {
				i.removeEventListener("pointerdown", o, !0), i.removeEventListener("click", n, !0);
			};
		}
	}, [t, r]);
}
function Ga(e, t) {
	if (e.button > 0) return !1;
	let n = E(e);
	if (n) {
		let e = n.ownerDocument;
		if (!e || !S(e.documentElement, n) || n.closest("[data-react-aria-top-layer]")) return !1;
	}
	return t.current ? !e.composedPath().includes(t.current) : !1;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/useOverlay.mjs
var Ka = [];
function qa(e, t) {
	let { onClose: n, shouldCloseOnBlur: r, isOpen: i, isDismissable: a = !1, isKeyboardDismissDisabled: o = !1, shouldCloseOnInteractOutside: s } = e, c = (0, q.useRef)(void 0);
	(0, q.useEffect)(() => {
		if (i && !Ka.includes(t)) return Ka.push(t), () => {
			let e = Ka.indexOf(t);
			e >= 0 && Ka.splice(e, 1);
		};
	}, [i, t]);
	let l = () => {
		Ka[Ka.length - 1] === t && n && n();
	}, u = (e) => {
		let n = Ka[Ka.length - 1];
		c.current = n, (!s || s(E(e))) && n === t && e.stopPropagation();
	}, d = (e) => {
		(!s || s(E(e))) && (Ka[Ka.length - 1] === t && e.stopPropagation(), c.current === t && l()), c.current = void 0;
	}, f = (e) => {
		e.key === "Escape" && !o && !e.nativeEvent.isComposing && (e.stopPropagation(), e.preventDefault(), l());
	};
	Wa({
		ref: t,
		onInteractOutside: a && i ? d : void 0,
		onInteractOutsideStart: u
	});
	let { focusWithinProps: p } = oe({
		isDisabled: !r,
		onBlurWithin: (e) => {
			!e.relatedTarget || Jr(e.relatedTarget) || (!s || s(e.relatedTarget)) && n?.();
		}
	});
	return {
		overlayProps: {
			onKeyDown: f,
			...p
		},
		underlayProps: {}
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/usePreventScroll.mjs
var Ja = typeof document < "u" && window.visualViewport, Ya = 0, Xa;
function Za(e = {}) {
	let { isDisabled: t } = e;
	N(() => {
		if (!t) return Ya++, Ya === 1 && (Xa = b() ? $a() : Qa()), () => {
			Ya--, Ya === 0 && Xa();
		};
	}, [t]);
}
function Qa() {
	let e = window.innerWidth - document.documentElement.clientWidth;
	return ue(e > 0 && ("scrollbarGutter" in document.documentElement.style ? eo(document.documentElement, "scrollbarGutter", "stable") : eo(document.documentElement, "paddingRight", `${e}px`)), eo(document.documentElement, "overflow", "hidden"));
}
function $a() {
	let e = eo(document.documentElement, "overflow", "hidden"), t, n = !1, r = (e) => {
		let r = E(e);
		t = ln(r) ? r : un(r, !0), n = !1;
		let i = r.ownerDocument.defaultView.getSelection();
		i && !i.isCollapsed && i.containsNode(r, !0) && (n = !0), e.composedPath().some((e) => e instanceof HTMLInputElement && e.type === "range") && (n = !0), "selectionStart" in r && "selectionEnd" in r && r.selectionStart < r.selectionEnd && r.ownerDocument.activeElement === r && (n = !0);
	}, i = document.createElement("style"), a = de();
	a && (i.nonce = a), i.textContent = "@layer {\n  * {\n    overscroll-behavior: contain;\n  }\n}", document.head.prepend(i);
	let s = (e) => {
		if (!(e.touches.length === 2 || n)) {
			if (!t || t === document.documentElement || t === document.body) {
				e.preventDefault();
				return;
			}
			t.scrollHeight === t.clientHeight && t.scrollWidth === t.clientWidth && e.preventDefault();
		}
	}, c = (e) => {
		let t = E(e), n = e.relatedTarget;
		n && Ve(n) ? (n.focus({ preventScroll: !0 }), no(n, Ve(t))) : n || (t.parentElement?.closest("[tabindex]"))?.focus({ preventScroll: !0 });
	}, l = HTMLElement.prototype.focus;
	HTMLElement.prototype.focus = function(e) {
		let t = o(), n = t != null && Ve(t);
		l.call(this, {
			...e,
			preventScroll: !0
		}), (!e || !e.preventScroll) && no(this, n);
	};
	let u = ue(to(document, "touchstart", r, {
		passive: !1,
		capture: !0
	}), to(document, "touchmove", s, {
		passive: !1,
		capture: !0
	}), to(document, "blur", c, !0));
	return () => {
		e(), u(), i.remove(), HTMLElement.prototype.focus = l;
	};
}
function eo(e, t, n) {
	let r = e.style[t];
	return e.style[t] = n, () => {
		e.style[t] = r;
	};
}
function to(e, t, n, r) {
	return e.addEventListener(t, n, r), () => {
		e.removeEventListener(t, n, r);
	};
}
function no(e, t) {
	t || !Ja ? ro(e) : Ja.addEventListener("resize", () => ro(e), { once: !0 });
}
function ro(e) {
	let t = document.scrollingElement || document.documentElement, n = e;
	for (; n && n !== t;) {
		let e = un(n);
		if (e !== document.documentElement && e !== document.body && e !== n) {
			let t = e.getBoundingClientRect(), r = n.getBoundingClientRect();
			if (r.top < t.top || r.bottom > t.top + n.clientHeight) {
				let n = t.bottom;
				Ja && (n = Math.min(n, Ja.offsetTop + Ja.height));
				let i = r.top - t.top - ((n - t.top) / 2 - r.height / 2);
				e.scrollTo({
					top: Math.max(0, Math.min(e.scrollHeight - e.clientHeight, e.scrollTop + i)),
					behavior: "smooth"
				});
			}
		}
		n = e.parentElement;
	}
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/usePopover.mjs
function io(e, t) {
	let { triggerRef: n, popoverRef: r, groupRef: i, isNonModal: a, isKeyboardDismissDisabled: o, shouldCloseOnInteractOutside: s, ...c } = e, l = c.trigger === "SubmenuTrigger", { overlayProps: u, underlayProps: d } = qa({
		isOpen: t.isOpen,
		onClose: t.close,
		shouldCloseOnBlur: !0,
		isDismissable: !a || l,
		isKeyboardDismissDisabled: o,
		shouldCloseOnInteractOutside: s
	}, i ?? r), { overlayProps: f, arrowProps: p, placement: m, triggerAnchorPoint: h } = Va({
		...c,
		targetRef: n,
		overlayRef: r,
		isOpen: t.isOpen,
		onClose: a && !l ? t.close : null
	});
	return Za({ isDisabled: a || !t.isOpen }), (0, q.useEffect)(() => {
		if (t.isOpen && r.current) return a ? ma(i?.current ?? r.current) : pa([i?.current ?? r.current], { shouldUseInert: !0 });
	}, [
		a,
		t.isOpen,
		r,
		i
	]), {
		popoverProps: R(u, f),
		arrowProps: p,
		underlayProps: d,
		placement: m,
		triggerAnchorPoint: h
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/ar-AE.mjs
var ao = {};
ao = { dismiss: "تجاهل" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/bg-BG.mjs
var oo = {};
oo = { dismiss: "Отхвърляне" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/cs-CZ.mjs
var so = {};
so = { dismiss: "Odstranit" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/da-DK.mjs
var co = {};
co = { dismiss: "Luk" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/de-DE.mjs
var lo = {};
lo = { dismiss: "Schließen" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/el-GR.mjs
var uo = {};
uo = { dismiss: "Απόρριψη" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/en-US.mjs
var fo = {};
fo = { dismiss: "Dismiss" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/es-ES.mjs
var po = {};
po = { dismiss: "Descartar" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/et-EE.mjs
var mo = {};
mo = { dismiss: "Lõpeta" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/fi-FI.mjs
var ho = {};
ho = { dismiss: "Hylkää" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/fr-FR.mjs
var go = {};
go = { dismiss: "Rejeter" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/he-IL.mjs
var _o = {};
_o = { dismiss: "התעלם" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/hr-HR.mjs
var vo = {};
vo = { dismiss: "Odbaci" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/hu-HU.mjs
var yo = {};
yo = { dismiss: "Elutasítás" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/it-IT.mjs
var bo = {};
bo = { dismiss: "Ignora" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/ja-JP.mjs
var xo = {};
xo = { dismiss: "閉じる" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/ko-KR.mjs
var So = {};
So = { dismiss: "무시" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/lt-LT.mjs
var Co = {};
Co = { dismiss: "Atmesti" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/lv-LV.mjs
var wo = {};
wo = { dismiss: "Nerādīt" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/nb-NO.mjs
var To = {};
To = { dismiss: "Lukk" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/nl-NL.mjs
var Eo = {};
Eo = { dismiss: "Negeren" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/pl-PL.mjs
var Do = {};
Do = { dismiss: "Zignoruj" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/pt-BR.mjs
var Oo = {};
Oo = { dismiss: "Descartar" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/pt-PT.mjs
var ko = {};
ko = { dismiss: "Dispensar" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/ro-RO.mjs
var Ao = {};
Ao = { dismiss: "Revocare" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/ru-RU.mjs
var jo = {};
jo = { dismiss: "Пропустить" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/sk-SK.mjs
var Mo = {};
Mo = { dismiss: "Zrušiť" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/sl-SI.mjs
var No = {};
No = { dismiss: "Opusti" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/sr-SP.mjs
var Po = {};
Po = { dismiss: "Odbaci" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/sv-SE.mjs
var Fo = {};
Fo = { dismiss: "Avvisa" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/tr-TR.mjs
var Io = {};
Io = { dismiss: "Kapat" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/uk-UA.mjs
var Lo = {};
Lo = { dismiss: "Скасувати" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/zh-CN.mjs
var Ro = {};
Ro = { dismiss: "取消" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/overlays/zh-TW.mjs
var zo = {};
zo = { dismiss: "關閉" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/intlStrings.mjs
var Bo = {};
Bo = {
	"ar-AE": ao,
	"bg-BG": oo,
	"cs-CZ": so,
	"da-DK": co,
	"de-DE": lo,
	"el-GR": uo,
	"en-US": fo,
	"es-ES": po,
	"et-EE": mo,
	"fi-FI": ho,
	"fr-FR": go,
	"he-IL": _o,
	"hr-HR": vo,
	"hu-HU": yo,
	"it-IT": bo,
	"ja-JP": xo,
	"ko-KR": So,
	"lt-LT": Co,
	"lv-LV": wo,
	"nb-NO": To,
	"nl-NL": Eo,
	"pl-PL": Do,
	"pt-BR": Oo,
	"pt-PT": ko,
	"ro-RO": Ao,
	"ru-RU": jo,
	"sk-SK": Mo,
	"sl-SI": No,
	"sr-SP": Po,
	"sv-SE": Fo,
	"tr-TR": Io,
	"uk-UA": Lo,
	"zh-CN": Ro,
	"zh-TW": zo
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/DismissButton.mjs
function Vo(e) {
	return e && e.__esModule ? e.default : e;
}
function Ho(e) {
	let { onDismiss: t, ...n } = e, r = Ue(n, pt(Vo(Bo), "@react-aria/overlays").format("dismiss")), i = () => {
		t && t();
	};
	return /*#__PURE__*/ q.createElement(yn, null, /*#__PURE__*/ q.createElement("button", {
		...r,
		tabIndex: -1,
		onClick: i,
		style: {
			width: 1,
			height: 1
		}
	}));
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/interactions/PressResponder.mjs
var Uo = /*#__PURE__*/ q.forwardRef(({ children: e, ...t }, n) => {
	let r = (0, q.useRef)(!1), a = (0, q.useContext)(z), o = R(a || {}, {
		...t,
		register() {
			r.current = !0, a && a.register();
		}
	});
	return o.ref = i(n || a?.ref), j(a, o.ref), (0, q.useEffect)(() => {
		r.current ||= !0;
	}, []), /*#__PURE__*/ q.createElement(z.Provider, { value: o }, e);
});
function Wo({ children: e }) {
	let t = (0, q.useMemo)(() => ({ register: () => {} }), []);
	return /*#__PURE__*/ q.createElement(z.Provider, { value: t }, e);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/PortalProvider.mjs
var Go = /*#__PURE__*/ (0, q.createContext)({});
function Ko() {
	return (0, q.useContext)(Go) ?? {};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/Overlay.mjs
var qo = /*#__PURE__*/ q.createContext(null);
function Jo(e) {
	let t = re(), { portalContainer: n = t ? null : document.body, isExiting: r } = e, [i, a] = (0, q.useState)(!1), o = (0, q.useMemo)(() => ({
		contain: i,
		setContain: a
	}), [i, a]), { getContainer: s } = Ko();
	if (!e.portalContainer && s && (n = s()), !n) return null;
	let c = e.children;
	return e.disableFocusManagement || (c = /*#__PURE__*/ q.createElement(zr, {
		restoreFocus: !0,
		contain: (e.shouldContainFocus || i) && !r
	}, c)), c = /*#__PURE__*/ q.createElement(qo.Provider, { value: o }, /*#__PURE__*/ q.createElement(Wo, null, c)), /*#__PURE__*/ jt.createPortal(c, n);
}
function Yo() {
	let e = (0, q.useContext)(qo)?.setContain;
	N(() => {
		e?.(!0);
	}, [e]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/overlays/useOverlayTriggerState.mjs
function Xo(e) {
	let [t, n] = ht(e.isOpen, e.defaultOpen || !1, e.onOpenChange);
	return {
		isOpen: t,
		setOpen: n,
		open: (0, q.useCallback)(() => {
			n(!0);
		}, [n]),
		close: (0, q.useCallback)(() => {
			n(!1);
		}, [n]),
		toggle: (0, q.useCallback)(() => {
			n(!t);
		}, [n, t])
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/animation.mjs
function Zo(e, t = !0) {
	let [n, r] = (0, q.useState)(!0), i = n && t;
	return N(() => {
		if (i && e.current && "getAnimations" in e.current) for (let t of e.current.getAnimations()) t instanceof CSSTransition && t.cancel();
	}, [e, i]), $o(e, i, (0, q.useCallback)(() => r(!1), [])), i;
}
function Qo(e, t) {
	let [n, r] = (0, q.useState)(t ? "open" : "closed");
	switch (n) {
		case "open":
			t || r("exiting");
			break;
		case "closed":
		case "exiting":
			t && r("open");
			break;
	}
	let i = n === "exiting";
	return $o(e, i, (0, q.useCallback)(() => {
		r((e) => e === "exiting" ? "closed" : e);
	}, [])), i;
}
function $o(e, t, n) {
	N(() => {
		if (t && e.current) {
			if (!("getAnimations" in e.current)) {
				n();
				return;
			}
			let t = e.current.getAnimations();
			if (t.length === 0) {
				n();
				return;
			}
			let r = !1;
			return Promise.allSettled(t.map((e) => e.finished)).then(() => {
				r || (0, jt.flushSync)(() => {
					n();
				});
			}), () => {
				r = !0;
			};
		}
	}, [
		e,
		t,
		n
	]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Popover.mjs
var es = /*#__PURE__*/ (0, q.createContext)(null), ts = /*#__PURE__*/ (0, q.createContext)(null), ns = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	[e, t] = h(e, t, es);
	let n = (0, q.useContext)(dc), r = Xo(e), i = e.isOpen != null || e.defaultOpen != null || !n ? r : n, a = Qo(t, i.isOpen) || e.isExiting || !1, o = Ee(), { direction: s } = et();
	if (o) {
		let t = e.children;
		return typeof t == "function" && (t = t({
			trigger: e.trigger || null,
			placement: "bottom",
			isEntering: !1,
			isExiting: !1,
			defaultChildren: null
		})), /*#__PURE__*/ q.createElement(q.Fragment, null, t);
	}
	return i && !i.isOpen && !a ? null : /*#__PURE__*/ q.createElement(rs, {
		...e,
		triggerRef: e.triggerRef,
		state: i,
		popoverRef: t,
		isExiting: a,
		dir: s
	});
});
function rs({ state: e, isExiting: t, UNSTABLE_portalContainer: n, clearContexts: r, ...i }) {
	let a = (0, q.useRef)(null), o = (0, q.useRef)(null), s = (0, q.useContext)(ts), c = s && i.trigger === "SubmenuTrigger", { popoverProps: l, underlayProps: u, arrowProps: d, placement: f, triggerAnchorPoint: p } = io({
		...i,
		offset: i.offset ?? 8,
		arrowRef: a,
		groupRef: c ? s : o
	}, e), m = i.popoverRef, h = Zo(m, !!f) || i.isEntering || !1, g = U({
		...i,
		defaultClassName: "react-aria-Popover",
		values: {
			trigger: i.trigger || null,
			placement: f,
			isEntering: h,
			isExiting: t
		}
	}), _ = !i.isNonModal || i.trigger === "SubmenuTrigger", [v, y] = (0, q.useState)(!1);
	N(() => {
		m.current && y(_ && !m.current.querySelector("[role=dialog]"));
	}, [m, _]), (0, q.useEffect)(() => {
		v && (i.trigger !== "SubmenuTrigger" || C() !== "pointer") && m.current && !Se(m.current) && P(m.current);
	}, [
		v,
		m,
		i.trigger
	]);
	let b = (0, q.useMemo)(() => {
		let e = g.children;
		if (r) for (let t of r) e = /*#__PURE__*/ q.createElement(t.Provider, { value: null }, e);
		return e;
	}, [g.children, r]), [x, S] = (0, q.useState)(null), w = (0, q.useCallback)(() => {
		i.triggerRef.current && S(i.triggerRef.current.getBoundingClientRect().width + "px");
	}, [i.triggerRef]);
	N(w, [w]), za({
		ref: g.style?.["--trigger-width"] ? void 0 : i.triggerRef,
		onResize: w
	});
	let T = {
		...l.style,
		"--trigger-anchor-point": p ? `${p.x}px ${p.y}px` : void 0,
		...g.style,
		"--trigger-width": g.style?.["--trigger-width"] || x
	}, E = /*#__PURE__*/ q.createElement(O.div, {
		...R(H(i, { global: !0 }), l),
		...g,
		role: v ? "dialog" : void 0,
		tabIndex: v ? -1 : void 0,
		"aria-label": i["aria-label"],
		"aria-labelledby": i["aria-labelledby"],
		ref: m,
		slot: i.slot || void 0,
		style: T,
		dir: i.dir,
		"data-trigger": i.trigger,
		"data-placement": f,
		"data-entering": h || void 0,
		"data-exiting": t || void 0
	}, !i.isNonModal && /*#__PURE__*/ q.createElement(Ho, { onDismiss: e.close }), /*#__PURE__*/ q.createElement(la.Provider, { value: {
		...d,
		placement: f,
		ref: a
	} }, b), /*#__PURE__*/ q.createElement(Ho, { onDismiss: e.close }));
	return c ? /*#__PURE__*/ q.createElement(Jo, {
		...i,
		shouldContainFocus: v,
		isExiting: t,
		portalContainer: n ?? s?.current ?? void 0
	}, E) : /*#__PURE__*/ q.createElement(Jo, {
		...i,
		shouldContainFocus: v,
		isExiting: t,
		portalContainer: n
	}, !i.isNonModal && e.isOpen && /*#__PURE__*/ q.createElement("div", {
		"data-testid": "underlay",
		...u,
		style: {
			position: "fixed",
			inset: 0
		}
	}), /*#__PURE__*/ q.createElement("div", {
		ref: o,
		style: { display: "contents" }
	}, /*#__PURE__*/ q.createElement(ts.Provider, { value: o }, E)));
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Keyboard.mjs
var is = /*#__PURE__*/ (0, q.createContext)({}), as = {};
as = { longPressMessage: "اضغط مطولاً أو اضغط على Alt + السهم لأسفل لفتح القائمة" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/bg-BG.mjs
var os = {};
os = { longPressMessage: "Натиснете продължително или натиснете Alt+ стрелка надолу, за да отворите менюто" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/cs-CZ.mjs
var ss = {};
ss = { longPressMessage: "Dlouhým stiskem nebo stisknutím kláves Alt + šipka dolů otevřete nabídku" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/da-DK.mjs
var cs = {};
cs = { longPressMessage: "Langt tryk eller tryk på Alt + pil ned for at åbne menuen" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/de-DE.mjs
var ls = {};
ls = { longPressMessage: "Drücken Sie lange oder drücken Sie Alt + Nach-unten, um das Menü zu öffnen" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/el-GR.mjs
var us = {};
us = { longPressMessage: "Πιέστε παρατεταμένα ή πατήστε Alt + κάτω βέλος για να ανοίξετε το μενού" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/en-US.mjs
var ds = {};
ds = { longPressMessage: "Long press or press Alt + ArrowDown to open menu" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/es-ES.mjs
var fs = {};
fs = { longPressMessage: "Mantenga pulsado o pulse Alt + flecha abajo para abrir el menú" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/et-EE.mjs
var ps = {};
ps = { longPressMessage: "Menüü avamiseks vajutage pikalt või vajutage klahve Alt + allanool" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/fi-FI.mjs
var ms = {};
ms = { longPressMessage: "Avaa valikko painamalla pohjassa tai näppäinyhdistelmällä Alt + Alanuoli" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/fr-FR.mjs
var hs = {};
hs = { longPressMessage: "Appuyez de manière prolongée ou appuyez sur Alt\xA0+\xA0Flèche vers le bas pour ouvrir le menu." };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/he-IL.mjs
var gs = {};
gs = { longPressMessage: "לחץ לחיצה ארוכה או הקש Alt + ArrowDown כדי לפתוח את התפריט" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/hr-HR.mjs
var _s = {};
_s = { longPressMessage: "Dugo pritisnite ili pritisnite Alt + strelicu prema dolje za otvaranje izbornika" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/hu-HU.mjs
var vs = {};
vs = { longPressMessage: "Nyomja meg hosszan, vagy nyomja meg az Alt + lefele nyíl gombot a menü megnyitásához" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/it-IT.mjs
var ys = {};
ys = { longPressMessage: "Premi a lungo o premi Alt + Freccia giù per aprire il menu" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/ja-JP.mjs
var bs = {};
bs = { longPressMessage: "長押しまたは Alt+下矢印キーでメニューを開く" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/ko-KR.mjs
var xs = {};
xs = { longPressMessage: "길게 누르거나 Alt + 아래쪽 화살표를 눌러 메뉴 열기" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/lt-LT.mjs
var Ss = {};
Ss = { longPressMessage: "Norėdami atidaryti meniu, nuspaudę palaikykite arba paspauskite „Alt + ArrowDown“." };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/lv-LV.mjs
var Cs = {};
Cs = { longPressMessage: "Lai atvērtu izvēlni, turiet nospiestu vai nospiediet taustiņu kombināciju Alt + lejupvērstā bultiņa" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/nb-NO.mjs
var ws = {};
ws = { longPressMessage: "Langt trykk eller trykk Alt + PilNed for å åpne menyen" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/nl-NL.mjs
var Ts = {};
Ts = { longPressMessage: "Druk lang op Alt + pijl-omlaag of druk op Alt om het menu te openen" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/pl-PL.mjs
var Es = {};
Es = { longPressMessage: "Naciśnij i przytrzymaj lub naciśnij klawisze Alt + Strzałka w dół, aby otworzyć menu" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/pt-BR.mjs
var Ds = {};
Ds = { longPressMessage: "Pressione e segure ou pressione Alt + Seta para baixo para abrir o menu" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/pt-PT.mjs
var Os = {};
Os = { longPressMessage: "Prima continuamente ou prima Alt + Seta Para Baixo para abrir o menu" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/ro-RO.mjs
var ks = {};
ks = { longPressMessage: "Apăsați lung sau apăsați pe Alt + săgeată în jos pentru a deschide meniul" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/ru-RU.mjs
var As = {};
As = { longPressMessage: "Нажмите и удерживайте или нажмите Alt + Стрелка вниз, чтобы открыть меню" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/sk-SK.mjs
var js = {};
js = { longPressMessage: "Ponuku otvoríte dlhým stlačením alebo stlačením klávesu Alt + klávesu so šípkou nadol" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/sl-SI.mjs
var Ms = {};
Ms = { longPressMessage: "Za odprtje menija pritisnite in držite gumb ali pritisnite Alt+puščica navzdol" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/sr-SP.mjs
var Ns = {};
Ns = { longPressMessage: "Dugo pritisnite ili pritisnite Alt + strelicu prema dole da otvorite meni" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/sv-SE.mjs
var Ps = {};
Ps = { longPressMessage: "Håll nedtryckt eller tryck på Alt + pil nedåt för att öppna menyn" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/tr-TR.mjs
var Fs = {};
Fs = { longPressMessage: "Menüyü açmak için uzun basın veya Alt + Aşağı Ok tuşuna basın" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/uk-UA.mjs
var Is = {};
Is = { longPressMessage: "Довго або звичайно натисніть комбінацію клавіш Alt і стрілка вниз, щоб відкрити меню" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/zh-CN.mjs
var Ls = {};
Ls = { longPressMessage: "长按或按 Alt + 向下方向键以打开菜单" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/intl/menu/zh-TW.mjs
var Rs = {};
Rs = { longPressMessage: "長按或按 Alt+向下鍵以開啟功能表" };
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/menu/intlStrings.mjs
var zs = {};
zs = {
	"ar-AE": as,
	"bg-BG": os,
	"cs-CZ": ss,
	"da-DK": cs,
	"de-DE": ls,
	"el-GR": us,
	"en-US": ds,
	"es-ES": fs,
	"et-EE": ps,
	"fi-FI": ms,
	"fr-FR": hs,
	"he-IL": gs,
	"hr-HR": _s,
	"hu-HU": vs,
	"it-IT": ys,
	"ja-JP": bs,
	"ko-KR": xs,
	"lt-LT": Ss,
	"lv-LV": Cs,
	"nb-NO": ws,
	"nl-NL": Ts,
	"pl-PL": Es,
	"pt-BR": Ds,
	"pt-PT": Os,
	"ro-RO": ks,
	"ru-RU": As,
	"sk-SK": js,
	"sl-SI": Ms,
	"sr-SP": Ns,
	"sv-SE": Ps,
	"tr-TR": Fs,
	"uk-UA": Is,
	"zh-CN": Ls,
	"zh-TW": Rs
};
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/useOverlayTrigger.mjs
function Bs(e, t, n) {
	let { type: r } = e, { isOpen: i } = t;
	(0, q.useEffect)(() => {
		n && n.current && Ia.set(n.current, t.close);
	});
	let a;
	r === "menu" ? a = !0 : r === "listbox" && (a = "listbox");
	let o = G();
	return {
		triggerProps: {
			"aria-haspopup": a,
			"aria-expanded": i,
			"aria-controls": i ? o : void 0,
			onPress: t.toggle
		},
		overlayProps: { id: o }
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/menu/useMenuTrigger.mjs
function Vs(e) {
	return e && e.__esModule ? e.default : e;
}
function Hs(e, t, n) {
	let { type: r = "menu", isDisabled: i, trigger: a = "press" } = e, o = G(), { triggerProps: s, overlayProps: c } = Bs({ type: r }, t, n), l = (e) => {
		if (!i && !(a === "longPress" && !e.altKey) && n && n.current) switch (e.key) {
			case "Enter":
			case " ": if (a === "longPress" || e.isDefaultPrevented()) return;
			case "ArrowDown":
				"continuePropagation" in e || e.stopPropagation(), e.preventDefault(), t.toggle("first");
				break;
			case "ArrowUp":
				"continuePropagation" in e || e.stopPropagation(), e.preventDefault(), t.toggle("last");
				break;
			default: "continuePropagation" in e && e.continuePropagation();
		}
	}, u = pt(Vs(zs), "@react-aria/menu"), { longPressProps: d } = wi({
		isDisabled: i || a !== "longPress",
		accessibilityDescription: u.format("longPressMessage"),
		onLongPressStart() {
			t.close();
		},
		onLongPress() {
			t.open("first");
		}
	}), f = {
		preventFocusOnPress: !0,
		onPressStart(e) {
			e.pointerType !== "touch" && e.pointerType !== "keyboard" && !i && (g(e.target), t.open(e.pointerType === "virtual" ? "first" : null));
		},
		onPress(e) {
			e.pointerType === "touch" && !i && (g(e.target), t.toggle());
		}
	};
	return delete s.onPress, {
		menuTriggerProps: {
			...s,
			...a === "press" ? f : d,
			id: o,
			onKeyDown: l
		},
		menuProps: {
			...c,
			"aria-labelledby": o,
			autoFocus: t.focusStrategy || !0,
			onClose: t.close
		}
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/menu/utils.mjs
var Us = /* @__PURE__ */ new WeakMap();
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/menu/useMenu.mjs
function Ws(e, t, n) {
	let { shouldFocusWrap: r = !0, onKeyDown: i, onKeyUp: a, ...o } = e;
	!e["aria-label"] && e["aria-labelledby"];
	let s = H(e, { labelable: !0 }), { listProps: c } = xi({
		...o,
		ref: n,
		selectionManager: t.selectionManager,
		collection: t.collection,
		disabledKeys: t.disabledKeys,
		shouldFocusWrap: r,
		linkBehavior: "override"
	});
	return Us.set(t, {
		onClose: e.onClose,
		onAction: e.onAction,
		shouldUseVirtualFocus: e.shouldUseVirtualFocus
	}), { menuProps: R(s, {
		onKeyDown: i,
		onKeyUp: a
	}, {
		role: "menu",
		...c,
		onKeyDown: (t) => {
			(t.key !== "Escape" || e.shouldUseVirtualFocus) && c.onKeyDown?.(t);
		}
	}) };
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/menu/useMenuItem.mjs
function Gs(e, t, n) {
	let { id: r, key: i, closeOnSelect: a, shouldCloseOnSelect: o, isVirtualized: c, "aria-haspopup": u, onPressStart: d, onPressUp: f, onPress: p, onPressChange: m, onPressEnd: h, onClick: g, onHoverStart: _, onHoverChange: v, onHoverEnd: b, onKeyDown: x, onKeyUp: S, onFocus: C, onFocusChange: T, onBlur: D, selectionManager: O = t.selectionManager } = e, k = !!u, A = k && e["aria-expanded"] === "true", j = e.isDisabled ?? O.isDisabled(i), M = e.isSelected ?? O.isSelected(i), N = Us.get(t), P = t.collection.getItem(i), F = e.onClose || N.onClose, I = y(), ee = () => {
		if (!k && (P?.props?.onAction ? P.props.onAction() : e.onAction && e.onAction(i), N.onAction)) {
			let e = N.onAction;
			e(i, P?.value);
		}
	}, L = "menuitem";
	k || (O.selectionMode === "single" ? L = "menuitemradio" : O.selectionMode === "multiple" && (L = "menuitemcheckbox"));
	let z = ce(), te = ce(), B = ce(), re = {
		id: r,
		"aria-disabled": j || void 0,
		role: L,
		"aria-label": e["aria-label"],
		"aria-labelledby": z,
		"aria-describedby": [
			e["aria-describedby"],
			te,
			B
		].filter(Boolean).join(" ") || void 0,
		"aria-controls": e["aria-controls"],
		"aria-haspopup": u,
		"aria-expanded": e["aria-expanded"]
	};
	if (O.selectionMode !== "none" && !k && (re["aria-checked"] = M), c) {
		let e = Number(P?.index);
		re["aria-posinset"] = Number.isNaN(e) ? void 0 : e + 1, re["aria-setsize"] = Pi(t.collection);
	}
	let ie = (0, q.useRef)(!1), ae = (e) => {
		m?.(e), ie.current = e;
	}, oe = (0, q.useRef)(null), V = (e) => {
		e.pointerType !== "keyboard" && (oe.current = { pointerType: e.pointerType }), e.pointerType === "mouse" && (ie.current || e.target.click()), f?.(e);
	}, se = (e) => {
		g?.(e), ee(), w(e, I, P.props.href, P?.props.routerOptions);
		let t = oe.current?.pointerType === "keyboard" ? oe.current?.key === "Enter" || O.selectionMode === "none" || O.isLink(i) : O.selectionMode !== "multiple" || O.isLink(i);
		t = o ?? a ?? t, F && !k && t && F(), oe.current = null;
	}, { itemProps: le, isFocused: ue } = Ti({
		id: r,
		selectionManager: O,
		key: i,
		ref: n,
		shouldSelectOnPressUp: !0,
		allowsDifferentPressOrigin: !0,
		linkBehavior: "none",
		shouldUseVirtualFocus: N.shouldUseVirtualFocus
	}), { pressProps: de, isPressed: U } = ge({
		onPressStart: d,
		onPress: p,
		onPressUp: V,
		onPressChange: ae,
		onPressEnd: h,
		isDisabled: j
	}), { hoverProps: W } = fe({
		isDisabled: j,
		onHoverStart(e) {
			!ye() && !(A && u) && (O.setFocused(!0), O.setFocusedKey(i)), _?.(e);
		},
		onHoverChange: v,
		onHoverEnd: b
	}), { keyboardProps: G } = ve({
		onKeyDown: (e) => {
			if (e.repeat) {
				e.continuePropagation();
				return;
			}
			switch (e.key) {
				case " ":
					oe.current = {
						pointerType: "keyboard",
						key: " "
					}, E(e).click(), s("keyboard");
					break;
				case "Enter":
					oe.current = {
						pointerType: "keyboard",
						key: "Enter"
					}, E(e).tagName !== "A" && E(e).click(), s("keyboard");
					break;
				default:
					k || e.continuePropagation(), x?.(e);
					break;
			}
		},
		onKeyUp: S
	}), { focusableProps: pe } = ne({
		onBlur: D,
		onFocus: C,
		onFocusChange: T
	}, n), me = H(P?.props);
	delete me.id;
	let he = l(P?.props);
	return {
		menuItemProps: {
			...re,
			...R(me, he, k ? {
				onFocus: le.onFocus,
				"data-collection": le["data-collection"],
				"data-key": le["data-key"]
			} : le, de, W, G, pe, N.shouldUseVirtualFocus || k ? { onMouseDown: (e) => e.preventDefault() } : void 0, j ? void 0 : { onClick: se }),
			tabIndex: le.tabIndex != null && A && !N.shouldUseVirtualFocus ? -1 : le.tabIndex
		},
		labelProps: { id: z },
		descriptionProps: { id: te },
		keyboardShortcutProps: { id: B },
		isFocused: ue,
		isFocusVisible: ue && O.isFocused && ye() && !A,
		isSelected: M,
		isPressed: U,
		isDisabled: j
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/menu/useMenuSection.mjs
function Ks(e) {
	let { heading: t, "aria-label": n } = e, r = G();
	return {
		itemProps: { role: "presentation" },
		headingProps: t ? {
			id: r,
			role: "presentation"
		} : {},
		groupProps: {
			role: "group",
			"aria-label": n,
			"aria-labelledby": t ? r : void 0
		}
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/menu/useMenuTriggerState.mjs
function qs(e) {
	let t = Xo(e), [n, r] = (0, q.useState)(null), [i, a] = (0, q.useState)([]), o = () => {
		a([]), t.close();
	}, s = (e, t) => {
		a((n) => t > n.length ? n : [...n.slice(0, t), e]);
	}, c = (e, t) => {
		a((n) => n[t] === e ? n.slice(0, t) : n);
	};
	return {
		focusStrategy: n,
		...t,
		open(e = null) {
			r(e), t.open();
		},
		toggle(e = null) {
			r(e), t.toggle();
		},
		close() {
			o();
		},
		expandedKeysStack: i,
		openSubmenu: s,
		closeSubmenu: c
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/tree/TreeCollection.mjs
var Js = class {
	constructor(e, { expandedKeys: t } = {}) {
		this.keyMap = /* @__PURE__ */ new Map(), this.firstKey = null, this.lastKey = null, this.iterable = e, t ||= /* @__PURE__ */ new Set();
		let n = (e) => {
			if (this.keyMap.set(e.key, e), e.childNodes && (e.type === "section" || t.has(e.key))) for (let t of e.childNodes) n(t);
		};
		for (let t of e) n(t);
		let r = null, i = 0;
		for (let [e, t] of this.keyMap) r ? (r.nextKey = e, t.prevKey = r.key) : (this.firstKey = e, t.prevKey = void 0), t.type === "item" && (t.index = i++), r = t, r.nextKey = void 0;
		this.lastKey = r?.key ?? null;
	}
	*[Symbol.iterator]() {
		yield* this.iterable;
	}
	get size() {
		return this.keyMap.size;
	}
	getKeys() {
		return this.keyMap.keys();
	}
	getKeyBefore(e) {
		let t = this.keyMap.get(e);
		return t ? t.prevKey ?? null : null;
	}
	getKeyAfter(e) {
		let t = this.keyMap.get(e);
		return t ? t.nextKey ?? null : null;
	}
	getFirstKey() {
		return this.firstKey;
	}
	getLastKey() {
		return this.lastKey;
	}
	getItem(e) {
		return this.keyMap.get(e) ?? null;
	}
	at(e) {
		let t = [...this.getKeys()];
		return this.getItem(t[e]);
	}
};
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/tree/useTreeState.mjs
function Ys(e) {
	let { onExpandedChange: t } = e, [n, r] = ht(e.expandedKeys ? new Set(e.expandedKeys) : void 0, e.defaultExpandedKeys ? new Set(e.defaultExpandedKeys) : /* @__PURE__ */ new Set(), t), i = Vi(e), a = (0, q.useMemo)(() => e.disabledKeys ? new Set(e.disabledKeys) : /* @__PURE__ */ new Set(), [e.disabledKeys]), o = Ji(e, (0, q.useCallback)((e) => new Js(e, { expandedKeys: n }), [n]), null);
	return (0, q.useEffect)(() => {
		i.focusedKey != null && !o.getItem(i.focusedKey) && i.setFocusedKey(null);
	}, [o, i.focusedKey]), {
		collection: o,
		expandedKeys: n,
		disabledKeys: a,
		toggleKey: (e) => {
			r(Xs(n, e));
		},
		setExpandedKeys: r,
		selectionManager: new Ui(o, i)
	};
}
function Xs(e, t) {
	let n = new Set(e);
	return n.has(t) ? n.delete(t) : n.add(t), n;
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Menu.mjs
var Zs = /*#__PURE__*/ (0, q.createContext)(null), Qs = /*#__PURE__*/ (0, q.createContext)(null), $s = /*#__PURE__*/ (0, q.createContext)(null), ec = /*#__PURE__*/ (0, q.createContext)(null);
function tc(e) {
	let t = qs(e), n = (0, q.useRef)(null), { menuTriggerProps: r, menuProps: i } = Hs({
		...e,
		type: "menu"
	}, t, n), a = (0, q.useRef)(null);
	return Ee() ? null : /*#__PURE__*/ q.createElement(d, { values: [
		[Zs, {
			...i,
			ref: a
		}],
		[dc, t],
		[$s, t],
		[es, {
			trigger: "MenuTrigger",
			triggerRef: n,
			scrollRef: a,
			placement: "bottom start",
			"aria-labelledby": i["aria-labelledby"]
		}]
	] }, /*#__PURE__*/ q.createElement(Uo, {
		...r,
		ref: n,
		isPressed: t.isOpen
	}, e.children));
}
var nc = /*#__PURE__*/ (0, q.createContext)(null);
(class extends vt {
	static {
		this.type = "submenutrigger";
	}
	filter(e, t, n) {
		let r = e.getItem(this.firstChildKey);
		if (r && n(r.textValue, this)) {
			let n = this.clone();
			return t.addDescendants(n, e), n;
		}
		return null;
	}
});
var rc = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	return [e, t] = h(e, t, Zs), /*#__PURE__*/ q.createElement(Ft, { content: /*#__PURE__*/ q.createElement(Kt, e) }, (n) => /*#__PURE__*/ q.createElement(ic, {
		props: e,
		collection: n,
		menuRef: t
	}));
});
function ic({ props: e, collection: t, menuRef: n }) {
	[e, n] = h(e, n, gt);
	let { filter: r, ...i } = e, a = (0, q.useMemo)(() => r ? t.filter(r) : t, [t, r]), o = Ys({
		...e,
		collection: a,
		children: void 0
	}), s = (0, q.useContext)($s), { isVirtualized: c, CollectionRoot: l } = (0, q.useContext)(Qt), { menuProps: u } = Ws({
		...e,
		isVirtualized: c,
		onClose: e.onClose || s?.close
	}, o, n), f = U({
		...e,
		children: void 0,
		defaultClassName: "react-aria-Menu",
		values: { isEmpty: o.collection.size === 0 }
	}), p = null;
	o.collection.size === 0 && e.renderEmptyState && (p = /*#__PURE__*/ q.createElement("div", {
		role: "menuitem",
		style: { display: "contents" }
	}, e.renderEmptyState()));
	let m = H(e, { global: !0 });
	return /*#__PURE__*/ q.createElement(zr, null, /*#__PURE__*/ q.createElement(O.div, {
		...R(m, f, u),
		ref: n,
		slot: e.slot || void 0,
		"data-empty": o.collection.size === 0 || void 0,
		onScroll: e.onScroll
	}, /*#__PURE__*/ q.createElement(d, { values: [
		[Qs, o],
		[jr, { elementType: "div" }],
		[Jt, {
			name: "MenuSection",
			render: oc
		}],
		[nc, {
			parentMenuRef: n,
			shouldUseVirtualFocus: i?.shouldUseVirtualFocus
		}],
		[sc, { shouldCloseOnSelect: e.shouldCloseOnSelect }],
		[gt, null],
		[_t, null],
		[ec, o.selectionManager],
		[$s, s ?? qs({})]
	] }, /*#__PURE__*/ q.createElement(kr, null, /*#__PURE__*/ q.createElement(l, {
		collection: o.collection,
		persistedKeys: $t(o.selectionManager.focusedKey),
		scrollRef: n
	}))), p));
}
var ac = class extends Ui {
	constructor(e, t) {
		super(e.collection, t), this.parent = e;
	}
	get focusedKey() {
		return this.parent.focusedKey;
	}
	get isFocused() {
		return this.parent.isFocused;
	}
	setFocusedKey(e, t) {
		return this.parent.setFocusedKey(e, t);
	}
	setFocused(e) {
		this.parent.setFocused(e);
	}
	get childFocusStrategy() {
		return this.parent.childFocusStrategy;
	}
};
function oc(e, t, n, r = "react-aria-MenuSection") {
	let i = (0, q.useContext)(Qs), { CollectionBranch: a } = (0, q.useContext)(Qt), [o, s] = m(), { headingProps: c, groupProps: l } = Ks({
		heading: s,
		"aria-label": n.props["aria-label"] ?? void 0
	}), u = U({
		...e,
		id: void 0,
		children: void 0,
		defaultClassName: r,
		className: n.props?.className,
		style: n.props?.style,
		values: void 0
	}), f = (0, q.useContext)(ec), p = Vi(e), h = e.selectionMode == null ? f : new ac(f, p), g = x(sc)?.shouldCloseOnSelect, _ = H(e, { global: !0 });
	return delete _.id, /*#__PURE__*/ q.createElement(O.section, {
		...R(_, u, l),
		ref: t
	}, /*#__PURE__*/ q.createElement(d, { values: [
		[Dr, {
			...c,
			ref: o
		}],
		[ec, h],
		[sc, { shouldCloseOnSelect: e.shouldCloseOnSelect ?? g }]
	] }, /*#__PURE__*/ q.createElement(a, {
		collection: i.collection,
		parent: n
	})));
}
var sc = /*#__PURE__*/ (0, q.createContext)(null), cc = /*#__PURE__*/ Ut(xt, function(e, t, n) {
	[e, t] = h(e, t, sc);
	let r = x(sc)?.id, a = (0, q.useContext)(Qs), o = i(t), s = (0, q.useContext)(ec), { isVirtualized: c } = (0, q.useContext)(Qt), { menuItemProps: l, labelProps: u, descriptionProps: f, keyboardShortcutProps: p, ...m } = Gs({
		...e,
		id: r,
		key: n.key,
		selectionManager: s,
		isVirtualized: c
	}, a, o), { hoverProps: g, isHovered: _ } = fe({ isDisabled: m.isDisabled }), v = U({
		...e,
		id: void 0,
		children: n.rendered,
		defaultClassName: "react-aria-MenuItem",
		values: {
			...m,
			isHovered: _,
			isFocusVisible: m.isFocusVisible,
			selectionMode: s.selectionMode,
			selectionBehavior: s.selectionBehavior,
			hasSubmenu: !!e["aria-haspopup"],
			isOpen: e["aria-expanded"] === "true"
		}
	}), y = e.href ? O.a : O.div, b = H(e, { global: !0 });
	return delete b.id, delete b.onClick, /*#__PURE__*/ q.createElement(y, {
		...R(b, v, l, g),
		ref: o,
		"data-disabled": m.isDisabled || void 0,
		"data-hovered": _ || void 0,
		"data-focused": m.isFocused || void 0,
		"data-focus-visible": m.isFocusVisible || void 0,
		"data-pressed": m.isPressed || void 0,
		"data-selected": m.isSelected || void 0,
		"data-selection-mode": s.selectionMode === "none" ? void 0 : s.selectionMode,
		"data-has-submenu": !!e["aria-haspopup"] || void 0,
		"data-open": e["aria-expanded"] === "true" || void 0
	}, /*#__PURE__*/ q.createElement(d, { values: [
		[sn, { slots: {
			[k]: u,
			label: u,
			description: f
		} }],
		[is, p],
		[Ar, { isSelected: m.isSelected }]
	] }, v.children));
});
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/dialog/useDialog.mjs
function lc(e, t) {
	let { role: n = "dialog" } = e, r = ce();
	r = e["aria-label"] ? void 0 : r;
	let i = (0, q.useRef)(!1);
	return (0, q.useEffect)(() => {
		if (t.current && !Se(t.current)) {
			P(t.current);
			let e = setTimeout(() => {
				(o() === t.current || o() === document.body) && (i.current = !0, t.current && (t.current.blur(), P(t.current)), i.current = !1);
			}, 500);
			return () => {
				clearTimeout(e);
			};
		}
	}, [t]), Yo(), (0, q.useRef)(!1), (0, q.useEffect)(() => {}), {
		dialogProps: {
			...H(e, { labelable: !0 }),
			role: n,
			tabIndex: -1,
			"aria-labelledby": e["aria-labelledby"] || r,
			onBlur: (e) => {
				i.current && e.stopPropagation();
			}
		},
		titleProps: { id: r }
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Dialog.mjs
var uc = /*#__PURE__*/ (0, q.createContext)(null), dc = /*#__PURE__*/ (0, q.createContext)(null), fc = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	let n = e["aria-labelledby"];
	[e, t] = h(e, t, uc);
	let { dialogProps: r, titleProps: i } = lc({
		...e,
		"aria-labelledby": n
	}, t), a = (0, q.useContext)(dc);
	!r["aria-label"] && !r["aria-labelledby"] && e["aria-labelledby"] && (r["aria-labelledby"] = e["aria-labelledby"]);
	let o = U({
		defaultClassName: "react-aria-Dialog",
		className: e.className,
		style: e.style,
		children: e.children,
		values: { close: a?.close || (() => {}) }
	}), s = H(e, { global: !0 });
	return /*#__PURE__*/ q.createElement(O.section, {
		...R(s, o, r),
		render: e.render,
		ref: t,
		slot: e.slot || void 0
	}, /*#__PURE__*/ q.createElement(d, { values: [[an, { slots: {
		[k]: {},
		title: {
			...i,
			level: 2
		}
	} }], [Te, { slots: {
		[k]: {},
		close: { onPress: () => a?.close() }
	} }]] }, o.children));
});
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/i18n/useListFormatter.mjs
function pc(e = {}) {
	let { locale: t } = et();
	return (0, q.useMemo)(() => new Intl.ListFormat(t, e), [t, e]);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/overlays/useModalOverlay.mjs
function mc(e, t, n) {
	let { overlayProps: r, underlayProps: i } = qa({
		...e,
		isOpen: t.isOpen,
		onClose: t.close
	}, n);
	return Za({ isDisabled: !t.isOpen }), Yo(), (0, q.useEffect)(() => {
		if (t.isOpen && n.current) return pa([n.current], { shouldUseInert: !0 });
	}, [t.isOpen, n]), {
		modalProps: R(r),
		underlayProps: i
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/utils/useViewportSize.mjs
var $ = typeof document < "u" && window.visualViewport;
function hc() {
	let e = re(), [t, n] = (0, q.useState)(() => e ? {
		width: 0,
		height: 0
	} : gc());
	return (0, q.useEffect)(() => {
		let e = (e) => {
			n((t) => e.width === t.width && e.height === t.height ? t : e);
		}, t = () => {
			$ && $.scale > 1 || e(gc());
		}, r, i = (t) => {
			$ && $.scale > 1 || Ve(E(t)) && (r = requestAnimationFrame(() => {
				let t = o();
				(!t || !Ve(t)) && e({
					width: document.documentElement.clientWidth,
					height: document.documentElement.clientHeight
				});
			}));
		};
		return e(gc()), b() && window.addEventListener("blur", i, !0), $ ? $.addEventListener("resize", t) : window.addEventListener("resize", t), () => {
			cancelAnimationFrame(r), b() && window.removeEventListener("blur", i, !0), $ ? $.removeEventListener("resize", t) : window.removeEventListener("resize", t);
		};
	}, []), t;
}
function gc() {
	return {
		width: $ ? Math.min($.width * $.scale, document.documentElement.clientWidth) : document.documentElement.clientWidth,
		height: $ ? $.height * $.scale : document.documentElement.clientHeight
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Modal.mjs
var _c = /*#__PURE__*/ (0, q.createContext)(null), vc = /*#__PURE__*/ (0, q.createContext)(null), yc = /*#__PURE__*/ (0, q.forwardRef)(function(e, t) {
	if ((0, q.useContext)(vc)) return /*#__PURE__*/ q.createElement(Cc, {
		...e,
		modalRef: t
	}, e.children);
	let { isDismissable: n, isKeyboardDismissDisabled: r, isOpen: i, defaultOpen: a, onOpenChange: o, children: s, isEntering: c, isExiting: l, UNSTABLE_portalContainer: u, shouldCloseOnInteractOutside: d, ...f } = e;
	return /*#__PURE__*/ q.createElement(xc, {
		isDismissable: n,
		isKeyboardDismissDisabled: r,
		isOpen: i,
		defaultOpen: a,
		onOpenChange: o,
		isEntering: c,
		isExiting: l,
		UNSTABLE_portalContainer: u,
		shouldCloseOnInteractOutside: d
	}, /*#__PURE__*/ q.createElement(Cc, {
		...f,
		modalRef: t
	}, s));
});
function bc(e, t) {
	[e, t] = h(e, t, _c);
	let n = (0, q.useContext)(dc), r = Xo(e), a = e.isOpen != null || e.defaultOpen != null || !n ? r : n, o = i(t), s = (0, q.useRef)(null), c = Qo(o, a.isOpen), l = Qo(s, a.isOpen), u = c || l || e.isExiting || !1, d = re();
	return !a.isOpen && !u || d ? null : /*#__PURE__*/ q.createElement(Sc, {
		...e,
		state: a,
		isExiting: u,
		overlayRef: o,
		modalRef: s
	});
}
var xc = /*#__PURE__*/ (0, q.forwardRef)(bc);
function Sc({ UNSTABLE_portalContainer: e, ...t }) {
	let n = t.modalRef, { state: r } = t, { modalProps: i, underlayProps: a } = mc(t, r, n), o = Zo(t.overlayRef) || t.isEntering || !1, s = U({
		...t,
		defaultClassName: "react-aria-ModalOverlay",
		values: {
			isEntering: o,
			isExiting: t.isExiting,
			state: r
		}
	}), c = hc(), l, u;
	if (typeof document < "u") {
		let e = ln(document.body) ? document.body : document.scrollingElement || document.documentElement, t = e.getBoundingClientRect().width % 1, n = e.getBoundingClientRect().height % 1;
		l = e.scrollWidth - t, u = e.scrollHeight - n;
	}
	let f = {
		...s.style,
		"--visual-viewport-width": c.width + "px",
		"--visual-viewport-height": c.height + "px",
		"--page-width": l === void 0 ? void 0 : l + "px",
		"--page-height": u === void 0 ? void 0 : u + "px"
	};
	return /*#__PURE__*/ q.createElement(Jo, {
		isExiting: t.isExiting,
		portalContainer: e
	}, /*#__PURE__*/ q.createElement(O.div, {
		...R(H(t, { global: !0 }), a),
		...s,
		style: f,
		ref: t.overlayRef,
		"data-entering": o || void 0,
		"data-exiting": t.isExiting || void 0
	}, /*#__PURE__*/ q.createElement(d, { values: [[vc, {
		modalProps: i,
		modalRef: n,
		isExiting: t.isExiting,
		isDismissable: t.isDismissable
	}], [dc, r]] }, s.children)));
}
function Cc(e) {
	let { modalProps: t, modalRef: n, isExiting: r, isDismissable: a } = (0, q.useContext)(vc), o = (0, q.useContext)(dc), s = i((0, q.useMemo)(() => he(e.modalRef, n), [e.modalRef, n])), c = Zo(s), l = U({
		...e,
		defaultClassName: "react-aria-Modal",
		values: {
			isEntering: c,
			isExiting: r,
			state: o
		}
	});
	return /*#__PURE__*/ q.createElement(O.div, {
		...R(H(e, { global: !0 }), t),
		...l,
		ref: s,
		"data-entering": c || void 0,
		"data-exiting": r || void 0
	}, a && /*#__PURE__*/ q.createElement(Ho, { onDismiss: o.close }), l.children);
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/select/useSelect.mjs
var wc = /* @__PURE__ */ new WeakMap();
function Tc(e, t, n) {
	let { keyboardDelegate: r, isDisabled: i, isRequired: a, name: o, form: c, validationBehavior: l = "aria" } = e, u = bi({
		usage: "search",
		sensitivity: "base"
	}), d = (0, q.useMemo)(() => r || new vi(t.collection, t.disabledKeys, n, u), [
		r,
		t.collection,
		t.disabledKeys,
		u,
		n
	]), { menuTriggerProps: f, menuProps: p } = Hs({
		isDisabled: i,
		type: "listbox"
	}, t, n), m = (e) => {
		if (t.selectionManager.selectionMode !== "multiple") switch (e.key) {
			case "ArrowLeft": {
				e.preventDefault();
				let n = t.selectedKey == null ? d.getFirstKey?.() : d.getKeyAbove?.(t.selectedKey);
				n != null && t.setSelectedKey(n);
				break;
			}
			case "ArrowRight": {
				e.preventDefault();
				let n = t.selectedKey == null ? d.getFirstKey?.() : d.getKeyBelow?.(t.selectedKey);
				n != null && t.setSelectedKey(n);
				break;
			}
		}
	}, { typeSelectProps: h } = pi({
		keyboardDelegate: d,
		selectionManager: t.selectionManager,
		onTypeSelect(e) {
			t.setSelectedKey(e);
		}
	}), { isInvalid: g, validationErrors: _, validationDetails: v } = t.displayValidation, { labelProps: y, fieldProps: b, descriptionProps: x, errorMessageProps: C } = Fn({
		...e,
		labelElementType: "span",
		isInvalid: g,
		errorMessage: e.errorMessage || _
	});
	t.selectionManager.selectionMode === "multiple" && (h = {});
	let w = H(e, { labelable: !0 }), T = R(h, f, b), E = G();
	return wc.set(t, {
		isDisabled: i,
		isRequired: a,
		name: o,
		form: c,
		validationBehavior: l
	}), {
		labelProps: {
			...y,
			onClick: () => {
				e.isDisabled || (n.current?.focus(), s("keyboard"));
			}
		},
		triggerProps: R(w, {
			...T,
			isDisabled: i,
			onKeyDown: ue(T.onKeyDown, m, e.onKeyDown),
			onKeyUp: e.onKeyUp,
			"aria-labelledby": [
				E,
				T["aria-labelledby"],
				T["aria-label"] && !T["aria-labelledby"] ? T.id : null
			].filter(Boolean).join(" "),
			onFocus(n) {
				t.isFocused || (e.onFocus && e.onFocus(n), e.onFocusChange && e.onFocusChange(!0), t.setFocused(!0));
			},
			onBlur(n) {
				t.isOpen || (e.onBlur && e.onBlur(n), e.onFocusChange && e.onFocusChange(!1), t.setFocused(!1));
			}
		}),
		valueProps: { id: E },
		menuProps: {
			...p,
			onAction: void 0,
			autoFocus: t.focusStrategy || !0,
			shouldSelectOnPressUp: !0,
			shouldFocusOnHover: !0,
			disallowEmptySelection: !0,
			linkBehavior: "selection",
			onBlur: (n) => {
				S(n.currentTarget, n.relatedTarget) || (e.onBlur && e.onBlur(n), e.onFocusChange && e.onFocusChange(!1), t.setFocused(!1));
			},
			"aria-labelledby": [b["aria-labelledby"], T["aria-label"] && !b["aria-labelledby"] ? T.id : null].filter(Boolean).join(" ")
		},
		descriptionProps: x,
		errorMessageProps: C,
		isInvalid: g,
		validationErrors: _,
		validationDetails: v,
		hiddenSelectProps: {
			isDisabled: i,
			name: o,
			label: e.label,
			state: t,
			triggerRef: n,
			form: c
		}
	};
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria@3.50.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria/dist/private/select/HiddenSelect.mjs
function Ec(e, t, n) {
	let r = wc.get(t) || {}, { autoComplete: i, name: a = r.name, form: o = r.form, isDisabled: s = r.isDisabled } = e, { validationBehavior: c, isRequired: l } = r, { visuallyHiddenProps: u } = vn({ style: {
		position: "fixed",
		top: 0,
		left: 0
	} });
	In(e.selectRef, t.defaultValue, t.setValue), Ln({
		validationBehavior: c,
		focus: () => n.current?.focus()
	}, t, e.selectRef);
	let d = t.setValue, f = (0, q.useCallback)((e) => {
		let t = E(e);
		t.multiple ? d(Array.from(t.selectedOptions, (e) => e.value)) : d(e.currentTarget.value);
	}, [d]);
	return {
		containerProps: {
			...u,
			"aria-hidden": !0,
			"data-react-aria-prevent-focus": !0,
			"data-a11y-ignore": "aria-hidden-focus"
		},
		inputProps: { style: { display: "none" } },
		selectProps: {
			tabIndex: -1,
			autoComplete: i,
			disabled: s,
			multiple: t.selectionManager.selectionMode === "multiple",
			required: c === "native" && l,
			name: a,
			form: o,
			value: t.value ?? "",
			onChange: f,
			onInput: f
		}
	};
}
function Dc(e) {
	let { state: t, triggerRef: n, label: r, name: i, form: a, isDisabled: o } = e, s = (0, q.useRef)(null), c = (0, q.useRef)(null), { containerProps: l, selectProps: u } = Ec({
		...e,
		selectRef: t.collection.size <= 300 ? s : c
	}, t, n), d = Array.isArray(t.value) ? t.value : [t.value];
	if (t.collection.size <= 300) return /*#__PURE__*/ q.createElement("div", {
		...l,
		"data-testid": "hidden-select-container"
	}, /*#__PURE__*/ q.createElement("label", null, r, /*#__PURE__*/ q.createElement("select", {
		...u,
		ref: s
	}, /*#__PURE__*/ q.createElement("option", {
		value: "",
		label: "\xA0"
	}, "\xA0"), [...t.collection.getKeys()].map((e) => {
		let n = t.collection.getItem(e);
		if (n && n.type === "item") return /*#__PURE__*/ q.createElement("option", {
			key: n.key,
			value: n.key
		}, n.textValue);
	}), t.collection.size === 0 && i && d.map((e, t) => /*#__PURE__*/ q.createElement("option", {
		key: t,
		value: e ?? ""
	})))));
	if (i) {
		let { validationBehavior: e } = wc.get(t) || {};
		d.length === 0 && (d = [null]);
		let n = d.map((t, n) => {
			let r = {
				type: "hidden",
				autoComplete: u.autoComplete,
				name: i,
				form: a,
				disabled: o,
				value: t ?? ""
			};
			return e === "native" ? /*#__PURE__*/ q.createElement("input", {
				key: n,
				...r,
				ref: n === 0 ? c : null,
				style: { display: "none" },
				type: "text",
				required: n === 0 && u.required,
				onChange: () => {}
			}) : /*#__PURE__*/ q.createElement("input", {
				key: n,
				...r,
				ref: n === 0 ? c : null
			});
		});
		return /*#__PURE__*/ q.createElement(q.Fragment, null, n);
	}
	return null;
}
//#endregion
//#region ../../node_modules/.pnpm/react-stately@3.48.0_react@19.2.4/node_modules/react-stately/dist/private/select/useSelectState.mjs
function Oc(e) {
	let { selectionMode: t = "single", shouldCloseOnSelect: n = t === "single" } = e, r = Xo(e), [i, a] = (0, q.useState)(null), o = (0, q.useMemo)(() => e.defaultValue === void 0 ? t === "single" ? e.defaultSelectedKey ?? null : [] : e.defaultValue, [
		e.defaultValue,
		e.defaultSelectedKey,
		t
	]), [s, c] = ht((0, q.useMemo)(() => e.value === void 0 ? t === "single" ? e.selectedKey : void 0 : e.value, [
		e.value,
		e.selectedKey,
		t
	]), o, e.onChange), l = t === "single" && Array.isArray(s) ? s[0] : s, u = (n) => {
		if (t === "single") {
			let t = Array.isArray(n) ? n[0] ?? null : n;
			c(t), t !== l && e.onSelectionChange?.(t);
		} else {
			let e = [];
			Array.isArray(n) ? e = n : n != null && (e = [n]), c(e);
		}
	}, d = Yi({
		...e,
		selectionMode: t,
		disallowEmptySelection: t === "single",
		allowDuplicateSelectionEvents: !0,
		selectedKeys: (0, q.useMemo)(() => kc(l), [l]),
		onSelectionChange: (e) => {
			if (e !== "all") {
				if (t === "single") {
					let t = e.values().next().value ?? null;
					u(t);
				} else u([...e]);
				n && r.close(), m.commitValidation();
			}
		}
	}), f = d.selectionManager.firstSelectedKey, p = (0, q.useMemo)(() => [...d.selectionManager.selectedKeys].map((e) => d.collection.getItem(e)).filter((e) => e != null), [d.selectionManager.selectedKeys, d.collection]), m = On({
		...e,
		value: Array.isArray(l) && l.length === 0 ? null : l
	}), [h, g] = (0, q.useState)(!1), [_] = (0, q.useState)(l);
	return {
		...m,
		...d,
		...r,
		value: l,
		defaultValue: o ?? _,
		setValue: u,
		selectedKey: f,
		setSelectedKey: u,
		selectedItem: p[0] ?? null,
		selectedItems: p,
		defaultSelectedKey: e.defaultSelectedKey ?? (e.selectionMode === "single" ? _ : null),
		focusStrategy: i,
		open(t = null) {
			(d.collection.size !== 0 || e.allowsEmptyCollection) && (a(t), r.open());
		},
		toggle(t = null) {
			(d.collection.size !== 0 || e.allowsEmptyCollection) && (a(t), r.toggle());
		},
		isFocused: h,
		setFocused: g
	};
}
function kc(e) {
	if (e !== void 0) return e === null ? [] : Array.isArray(e) ? e : [e];
}
//#endregion
//#region ../../node_modules/.pnpm/react-aria-components@1.19.0_react-dom@19.2.4_react@19.2.4__react@19.2.4/node_modules/react-aria-components/dist/private/Select.mjs
function Ac(e) {
	return e && e.__esModule ? e.default : e;
}
var jc = /*#__PURE__*/ (0, q.createContext)(null), Mc = /*#__PURE__*/ (0, q.createContext)(null), Nc = /*#__PURE__*/ we(function(e, t) {
	[e, t] = h(e, t, jc);
	let { children: n, isDisabled: r = !1, isInvalid: i = !1, isRequired: a = !1 } = e, o = (0, q.useMemo)(() => typeof n == "function" ? n({
		isOpen: !1,
		isDisabled: r,
		isInvalid: i,
		isRequired: a,
		isFocused: !1,
		isFocusVisible: !1,
		defaultChildren: null
	}) : n, [
		n,
		r,
		i,
		a
	]);
	return /*#__PURE__*/ q.createElement(Ft, { content: o }, (n) => /*#__PURE__*/ q.createElement(Fc, {
		props: e,
		collection: n,
		selectRef: t
	}));
}), Pc = [
	en,
	Te,
	sn
];
function Fc({ props: e, selectRef: t, collection: n }) {
	let { validationBehavior: r } = x(Pn) || {}, i = e.validationBehavior ?? r ?? "native", a = Oc({
		...e,
		collection: n,
		children: void 0,
		validationBehavior: i
	}), { isFocusVisible: o, focusProps: s } = F({ within: !0 }), c = (0, q.useRef)(null), [l, u] = m(!e["aria-label"] && !e["aria-labelledby"]), { labelProps: f, triggerProps: p, valueProps: h, menuProps: g, descriptionProps: _, errorMessageProps: v, hiddenSelectProps: y, ...b } = Tc({
		...A(e),
		label: u,
		validationBehavior: i
	}, a, c), S = (0, q.useMemo)(() => ({
		isOpen: a.isOpen,
		isFocused: a.isFocused,
		isFocusVisible: o,
		isDisabled: e.isDisabled || !1,
		isInvalid: b.isInvalid || !1,
		isRequired: e.isRequired || !1
	}), [
		a.isOpen,
		a.isFocused,
		o,
		e.isDisabled,
		b.isInvalid,
		e.isRequired
	]), C = U({
		...e,
		values: S,
		defaultClassName: "react-aria-Select"
	}), w = H(e, { global: !0 });
	delete w.id;
	let T = (0, q.useRef)(null);
	return /*#__PURE__*/ q.createElement(d, { values: [
		[jc, e],
		[Mc, a],
		[Ic, h],
		[en, {
			...f,
			ref: l,
			elementType: "span"
		}],
		[Te, {
			...p,
			ref: c,
			isPressed: a.isOpen,
			autoFocus: e.autoFocus
		}],
		[dc, a],
		[es, {
			trigger: "Select",
			triggerRef: c,
			scrollRef: T,
			placement: "bottom start",
			"aria-labelledby": g["aria-labelledby"],
			clearContexts: Pc
		}],
		[$i, {
			...g,
			ref: T
		}],
		[ea, a],
		[sn, { slots: {
			description: _,
			errorMessage: v
		} }],
		[bn, b]
	] }, /*#__PURE__*/ q.createElement(O.div, {
		...R(w, C, s),
		ref: t,
		slot: e.slot || void 0,
		"data-focused": a.isFocused || void 0,
		"data-focus-visible": o || void 0,
		"data-open": a.isOpen || void 0,
		"data-disabled": e.isDisabled || void 0,
		"data-invalid": b.isInvalid || void 0,
		"data-required": e.isRequired || void 0
	}, C.children, /*#__PURE__*/ q.createElement(Dc, {
		...y,
		autoComplete: e.autoComplete
	})));
}
var Ic = /*#__PURE__*/ (0, q.createContext)(null), Lc = /*#__PURE__*/ we(function(e, t) {
	[e, t] = h(e, t, Ic);
	let n = (0, q.useContext)(Mc), { placeholder: r } = x(jc), i = n.selectedItems.map((e) => {
		let t = e.props?.children;
		return typeof t == "function" && (t = t({
			isHovered: !1,
			isPressed: !1,
			isSelected: !1,
			isFocused: !1,
			isFocusVisible: !1,
			isDisabled: !1,
			selectionMode: "single",
			selectionBehavior: "toggle"
		})), t;
	}), a = pc(), o = (0, q.useMemo)(() => n.selectedItems.map((e) => e?.textValue), [n.selectedItems]), s = n.selectionManager.selectionMode, c = (0, q.useMemo)(() => s === "single" ? o[0] ?? "" : a.format(o), [
		s,
		a,
		o
	]), l = (0, q.useMemo)(() => {
		if (s === "single") return i[0];
		let e = a.formatToParts(o);
		if (e.length === 0) return null;
		let t = 0;
		return e.map((e) => e.type === "element" ? /*#__PURE__*/ q.createElement(q.Fragment, { key: t }, i[t++]) : e.value);
	}, [
		s,
		a,
		o,
		i
	]), u = pt(Ac(xr), "react-aria-components"), d = U({
		...e,
		defaultChildren: l ?? r ?? u.format("selectPlaceholder"),
		defaultClassName: "react-aria-SelectValue",
		values: {
			selectedItem: n.selectedItems[0]?.value ?? null,
			selectedItems: (0, q.useMemo)(() => n.selectedItems.map((e) => e.value ?? null), [n.selectedItems]),
			selectedText: c,
			isPlaceholder: n.selectedItems.length === 0,
			state: n
		}
	}), f = H(e, { global: !0 });
	return /*#__PURE__*/ q.createElement(O.span, {
		ref: t,
		...f,
		...d,
		"data-placeholder": n.selectedItems.length === 0 || void 0
	}, /*#__PURE__*/ q.createElement(sn.Provider, { value: void 0 }, d.children));
});
//#endregion
//#region ../../packages/ui-react/src/brands/useBrandBehavior.ts
function Rc() {
	let e = le()?.brand ?? I;
	return {
		brand: e,
		components: pe[e].components
	};
}
//#endregion
//#region ../../packages/icons/src/custom/FilterIcon.tsx
var zc = (0, q.forwardRef)(function({ className: e, width: t = 16, height: n = 16, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 16 16",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fill: "currentColor",
			d: "M6.00004 3.33325C7.08733 3.33325 8.00093 4.07694 8.2599 5.08339L15.3334 5.08325V6.24992L8.25975 6.25036C8.00057 7.25652 7.08712 7.99992 6.00004 7.99992C4.91296 7.99992 3.99951 7.25652 3.74033 6.25036L1.33337 6.24992V5.08325L3.74018 5.08339C3.99916 4.07694 4.91275 3.33325 6.00004 3.33325ZM6.00004 4.49992C5.35571 4.49992 4.83337 5.02225 4.83337 5.66659C4.83337 6.31092 5.35571 6.83325 6.00004 6.83325C6.64437 6.83325 7.16671 6.31092 7.16671 5.66659C7.16671 5.02225 6.64437 4.49992 6.00004 4.49992ZM10.6667 8.58325C11.754 8.58325 12.6676 9.32694 12.9266 10.3334L15.3334 10.3333V11.4999L12.9264 11.5004C12.6672 12.5065 11.7538 13.2499 10.6667 13.2499C9.57963 13.2499 8.66618 12.5065 8.407 11.5004L1.33337 11.4999V10.3333L8.40685 10.3334C8.66582 9.32694 9.57942 8.58325 10.6667 8.58325ZM10.6667 9.74992C10.0224 9.74992 9.50004 10.2723 9.50004 10.9166C9.50004 11.5609 10.0224 12.0833 10.6667 12.0833C11.311 12.0833 11.8334 11.5609 11.8334 10.9166C11.8334 10.2723 11.311 9.74992 10.6667 9.74992Z"
		})
	});
});
zc.displayName = "FilterIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/ArrowIcon.tsx
var Bc = (0, q.forwardRef)(function({ className: e, width: t = 18, height: n = 18, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 18 18",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M6.88965 14.0285C7.1252 13.7786 7.06675 13.3414 6.83002 13.0915L3.05238 9.59356L16.6894 9.59356C17.0436 9.59356 17.2798 9.28124 17.2798 8.96892C17.2798 8.65661 16.9846 8.34366 16.6894 8.34366L3.05238 8.34366L6.83002 4.90816C7.06675 4.65893 7.1252 4.28353 6.88965 3.97121C6.65351 3.72198 6.2993 3.65889 6.00412 3.90937L0.926535 8.46921C0.86809 8.53168 0.86809 8.59414 0.808464 8.59414L0.750021 8.65661C0.690985 8.71907 0.690985 8.844 0.690985 8.90646C0.690985 8.96892 0.690985 9.09385 0.750021 9.15631C0.750021 9.15631 0.750021 9.21878 0.808464 9.21878C0.86809 9.28124 0.86809 9.34371 0.926535 9.34371L6.06316 14.0909C6.2993 14.3408 6.71195 14.2783 6.88965 14.0285Z",
			fill: "currentColor"
		})
	});
});
Bc.displayName = "ArrowIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/CheckIcon.tsx
var Vc = (0, q.forwardRef)(function({ className: e, width: t = 10, height: n = 10, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 10 10",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M3.4109 8.64148C3.2517 8.64148 3.0989 8.57619 2.98609 8.4608L0.175253 5.56911C-0.0587506 5.32848 -0.0583506 4.93837 0.176053 4.69815C0.410456 4.45752 0.790062 4.45793 1.02447 4.69897L3.4109 7.15334L8.97538 1.43073C9.20939 1.1901 9.58899 1.18969 9.8238 1.42991C10.0586 1.67013 10.0586 2.06023 9.8246 2.30128L3.83571 8.4608C3.72291 8.5766 3.5701 8.64148 3.4109 8.64148Z",
			fill: "currentColor"
		})
	});
});
Vc.displayName = "CheckIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/ChevronIcon.tsx
var Hc = (0, q.forwardRef)(function({ className: e, width: t = 10, height: n = 10, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 10 10",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M4.96853 7.75719L4.96047 7.76522L0 3.06207L0.875377 2.23248L4.97135 6.11598L9.06369 2.23478L9.93867 3.06436L4.97901 7.76751L4.96853 7.75719Z",
			fill: "currentColor"
		})
	});
});
Hc.displayName = "ChevronIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/ClockIcon.tsx
var Uc = (0, q.forwardRef)(function({ className: e, width: t = 12, height: n = 12, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 12 12",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M8 8.5C7.86812 8.49812 7.74232 8.44421 7.65 8.35L5.5 6.205V2.5C5.5 2.22386 5.72386 2 6 2C6.27614 2 6.5 2.22386 6.5 2.5V5.79L8.355 7.645C8.54886 7.84002 8.54886 8.15498 8.355 8.35C8.26146 8.44543 8.13362 8.49945 8 8.5ZM6 1C3.23858 1 1 3.23858 1 6C1 8.76142 3.23858 11 6 11C8.76142 11 11 8.76142 11 6C11 4.67392 10.4732 3.40215 9.53553 2.46447C8.59785 1.52678 7.32608 1 6 1ZM6 12C2.68629 12 0 9.31371 0 6C0 2.68629 2.68629 0 6 0C9.31371 0 12 2.68629 12 6C12 7.5913 11.3679 9.11742 10.2426 10.2426C9.11742 11.3679 7.5913 12 6 12Z",
			fill: "currentColor"
		})
	});
});
Uc.displayName = "ClockIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/CloseIcon.tsx
var Wc = (0, q.forwardRef)(function({ className: e, width: t = 16, height: n = 16, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 16 16",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M2.14783 13.8522C2.24166 13.9468 2.3694 14 2.50266 14C2.63591 14 2.76364 13.9468 2.85749 13.8522L8 8.70428L13.1425 13.8522C13.2364 13.9468 13.3641 14 13.4973 14C13.6306 14 13.7583 13.9468 13.8522 13.8522C13.9468 13.7583 14 13.6306 14 13.4973C14 13.364 13.9468 13.2363 13.8522 13.1425L8.70466 7.99957L13.8522 2.85669C14.0481 2.66071 14.0481 2.34297 13.8522 2.14699C13.6562 1.95101 13.3385 1.95101 13.1425 2.14699L8 7.29487L2.85749 2.14699C2.66152 1.95101 2.34379 1.95101 2.14783 2.14699C1.95186 2.34297 1.95186 2.66071 2.14783 2.85669L7.29534 7.99957L2.14783 13.1425C2.05322 13.2363 2 13.364 2 13.4973C2 13.6306 2.05322 13.7583 2.14783 13.8522Z",
			fill: "currentColor"
		})
	});
});
Wc.displayName = "CloseIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/ErrorIcon.tsx
var Gc = (0, q.forwardRef)(function({ className: e, width: t = 16, height: n = 16, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsxs)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 16 16",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: [
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M8.01001 8.98999C7.46001 8.98999 7.01001 8.53999 7.01001 7.98999V4.98999C7.01001 4.43999 7.46001 3.98999 8.01001 3.98999C8.56001 3.98999 9.01001 4.43999 9.01001 4.98999V7.98999C9.01001 8.53999 8.56001 8.98999 8.01001 8.98999Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M8 11.99C8.55228 11.99 9 11.5423 9 10.99C9 10.4377 8.55228 9.98999 8 9.98999C7.44772 9.98999 7 10.4377 7 10.99C7 11.5423 7.44772 11.99 8 11.99Z",
				fill: "currentColor"
			}),
			/* @__PURE__ */ (0, K.jsx)("path", {
				d: "M8 16C3.59 16 0 12.41 0 8C0 3.59 3.59 0 8 0C12.41 0 16 3.59 16 8C16 12.41 12.41 16 8 16ZM8 2C4.69 2 2 4.69 2 8C2 11.31 4.69 14 8 14C11.31 14 14 11.31 14 8C14 4.69 11.31 2 8 2Z",
				fill: "currentColor"
			})
		]
	});
});
Gc.displayName = "ErrorIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/GlobeIcon.tsx
var Kc = (0, q.forwardRef)(function({ className: e, width: t = 18, height: n = 18, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 18 18",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fill: "currentColor",
			d: "M18 9.00024C18 4.03754 13.9625 0 9.00024 0C4.03802 0 0 4.03754 0 9.00024C0 13.9629 3.86623 17.8258 8.68209 17.9942C8.7128 17.9981 8.74352 18.0005 8.77423 18.0005C8.78958 18.0005 8.80542 17.999 8.82077 17.9981C8.88028 17.999 8.93978 18.0005 8.99976 18.0005C9.05638 18.0005 9.11253 17.9995 9.16867 17.9986C9.18355 17.9995 9.19794 18.0005 9.21282 18.0005C9.24209 18.0005 9.27136 17.9981 9.30015 17.9947C14.1242 17.8359 18 13.8622 18 9.00024ZM6.43971 11.9533C6.23481 11.2455 6.10429 10.5137 6.05151 9.76754H11.9365C11.8837 10.5137 11.7532 11.245 11.5483 11.9533H6.44019H6.43971ZM10.9686 13.4888C10.4796 14.5316 9.81648 15.5009 8.99352 16.3627C8.17056 15.5009 7.5074 14.5316 7.01842 13.4888H10.9686ZM2.14545 6.04671H4.85618C4.67671 6.75834 4.56202 7.48868 4.515 8.23247H1.57538C1.65407 7.46373 1.85082 6.72907 2.14593 6.04671H2.14545ZM8.99352 1.66511C9.8088 2.51926 10.4676 3.47898 10.9557 4.51116H7.03186C7.51987 3.47898 8.17824 2.51926 8.994 1.66511H8.99352ZM11.5392 6.04671C11.7465 6.7545 11.8794 7.4858 11.9341 8.23247H6.05294C6.10765 7.48628 6.24057 6.7545 6.44787 6.04671H11.5392ZM16.4251 8.23247H13.4725C13.425 7.48916 13.3108 6.75834 13.1313 6.04671H15.8545C16.1497 6.72907 16.3459 7.46325 16.4251 8.23247ZM1.5749 9.76801H4.5126C4.55818 10.5113 4.67143 11.2421 4.84898 11.9538H2.14545C1.85034 11.2714 1.65407 10.5372 1.5749 9.76801ZM13.4744 9.76801H16.4251C16.3464 10.5367 16.1497 11.2714 15.8545 11.9538H13.1381C13.3156 11.2421 13.4289 10.5113 13.4744 9.76801ZM14.9601 4.51116H12.6323C12.2551 3.57255 11.759 2.68049 11.1514 1.85178C12.687 2.31484 14.0157 3.26016 14.9606 4.51116H14.9601ZM6.83224 1.85706C6.22617 2.68433 5.73096 3.57495 5.35475 4.51116H3.03991C3.98091 3.26448 5.3034 2.32156 6.83224 1.85706ZM3.03991 13.4888H5.34371C5.71608 14.4222 6.20602 15.3099 6.80632 16.1353C5.28853 15.6679 3.97611 14.7283 3.04039 13.4888H3.03991ZM11.1774 16.1401C11.7791 15.3133 12.2705 14.4241 12.6433 13.4888H14.9596C14.0205 14.7326 12.7019 15.6741 11.1774 16.1401Z"
		})
	});
});
Kc.displayName = "GlobeIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/InfoIcon.tsx
var qc = (0, q.forwardRef)(function({ className: e, width: t = 24, height: n = 24, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsxs)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 24 24",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: [/* @__PURE__ */ (0, K.jsx)("circle", {
			cx: "12",
			cy: "12",
			r: "11.5",
			fill: "white"
		}), /* @__PURE__ */ (0, K.jsx)("path", {
			d: "M12.834 16.833H9.5V15.167H11.167V11.834H10.333V10.167H12.833V15.167H14.5V16.834L12.834 16.833ZM10.333 7.25C10.333 7.00282 10.4063 6.76118 10.5436 6.55564C10.6809 6.3501 10.8761 6.18989 11.1044 6.09525C11.3328 6.00061 11.584 5.9758 11.8265 6.02395C12.0689 6.0721 12.2917 6.19105 12.4665 6.36576C12.6414 6.54048 12.7605 6.76311 12.8089 7.00552C12.8572 7.24794 12.8326 7.49924 12.7381 7.72766C12.6437 7.95609 12.4836 8.15138 12.2782 8.28885C12.0728 8.42632 11.8312 8.4998 11.584 8.5C11.4198 8.50013 11.2571 8.4679 11.1053 8.40514C10.9536 8.34238 10.8156 8.25032 10.6995 8.13424C10.5833 8.01815 10.4911 7.88031 10.4282 7.72859C10.3654 7.57686 10.333 7.41424 10.333 7.25Z",
			fill: "currentColor",
			stroke: "none"
		})]
	});
});
qc.displayName = "InfoIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/MapIcon.tsx
var Jc = (0, q.forwardRef)(function({ className: e, width: t = 20, height: n = 20, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 20 20",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M13 12.505L10.43 14.0492C10.3033 14.1233 10.1567 14.1667 10 14.1667C9.84333 14.1667 9.69667 14.1233 9.57167 14.0483L7 12.5033C5.30333 11.4842 4.16667 9.625 4.16667 7.5C4.16667 4.27833 6.77833 1.66667 10 1.66667C13.2225 1.66667 15.8333 4.27833 15.8333 7.5C15.8333 9.625 14.6975 11.4842 13 12.505ZM10 0C5.8575 0 2.5 3.3575 2.5 7.5C2.5 9.95667 3.68083 12.1367 5.50583 13.505L7.49667 14.9967C8.51083 15.7567 9.16667 16.9675 9.16667 18.3333V19.1667C9.16667 19.6275 9.54 20 10 20C10.46 20 10.8333 19.6275 10.8333 19.1667V18.3333C10.8333 16.9675 11.49 15.7567 12.5033 14.9967L14.4958 13.505C16.32 12.1367 17.5 9.95667 17.5 7.5C17.5 3.3575 14.1425 0 10 0ZM10 5C11.3808 5 12.5 6.11917 12.5 7.5C12.5 8.88083 11.3808 10 10 10C8.61917 10 7.5 8.88083 7.5 7.5C7.5 6.11917 8.61917 5 10 5Z",
			fill: "currentColor"
		})
	});
});
Jc.displayName = "MapIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/MenuIcon.tsx
var Yc = (0, q.forwardRef)(function({ className: e, width: t = 16, height: n = 16, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 16 16",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M0 12.7654H16V15.052H0V12.7654ZM0 9.33203H16V7.05203H0V9.33203ZM0 1.33203V3.6187H16V1.33203H0Z",
			fill: "currentColor"
		})
	});
});
Yc.displayName = "MenuIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/SaveIcon.tsx
var Xc = (0, q.forwardRef)(function({ className: e, width: t = 20, height: n = 20, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 20 20",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			d: "M4.18769 20C4.07764 20 3.96706 19.9786 3.86182 19.9354C3.54184 19.8034 3.3335 19.4914 3.3335 19.1453V2.50968C3.33296 1.1261 4.45906 0 5.84264 0H14.1292C15.5427 0 16.6923 1.1496 16.6923 2.5631V19.1367C16.6923 19.4818 16.4845 19.7933 16.1656 19.9257C15.8467 20.0582 15.4797 19.9861 15.235 19.7425L9.98912 14.5276L4.79347 19.7473C4.63001 19.9119 4.41045 19.9989 4.18769 19.9989V20ZM9.98592 12.4651C10.2039 12.4651 10.4218 12.5479 10.5885 12.7135L14.9828 17.0817V2.5631C14.9828 2.09247 14.5998 1.70945 14.1292 1.70945H5.84264C5.40139 1.70945 5.04241 2.06843 5.04241 2.50968V17.0747L9.38013 12.7167C9.54734 12.5489 9.76636 12.4651 9.98592 12.4651Z",
			fill: "currentColor"
		})
	});
});
Xc.displayName = "SaveIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/SelectArrowIcon.tsx
var Zc = (0, q.forwardRef)(function({ className: e, width: t = 14, height: n = 14, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 14 14",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			fillRule: "evenodd",
			clipRule: "evenodd",
			d: "M6.99886 10.7883L6.98751 10.7998L0 4.09874L1.23309 2.91675L7.00284 8.44994L12.7675 2.92001L14 4.102L7.01362 10.803L6.99886 10.7883V10.7883Z",
			fill: "currentColor"
		})
	});
});
Zc.displayName = "SelectArrowIcon";
//#endregion
//#region ../../packages/icons/src/custom/brands/default/NavArrowIcon.tsx
var Qc = (0, q.forwardRef)(function({ className: e, width: t = 25, height: n = 25, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsx)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 25 25",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: /* @__PURE__ */ (0, K.jsx)("path", {
			d: "M9.375 18.75L15.625 12.5L9.375 6.25",
			stroke: "currentColor",
			strokeWidth: "1.25",
			strokeLinecap: "round",
			strokeLinejoin: "round"
		})
	});
});
Qc.displayName = "NavArrowIcon";
//#endregion
//#region ../../packages/icons/src/custom/brandIcons.ts
var $c = {
	globe: Kc,
	chevron: Hc,
	selectArrow: Zc,
	close: Wc,
	filter: zc,
	info: qc,
	error: Gc,
	map: Jc,
	save: Xc,
	menu: Yc,
	arrow: Bc,
	navArrow: Qc,
	clock: Uc,
	check: Vc
}, el = {
	"range-rover": () => Promise.resolve({}),
	discovery: () => Promise.resolve({}),
	jaguar: () => import("./jaguar-L6JOr8EB.js").then((e) => e.jaguarIconOverrides)
}, tl = /* @__PURE__ */ new Map(), nl = /* @__PURE__ */ new Map();
function rl(e) {
	if (!tl.has(e)) {
		let t = el[e]().then((t) => (nl.set(e, t), {
			...$c,
			...t
		}));
		tl.set(e, t);
	}
	return tl.get(e);
}
function il() {
	return $c;
}
//#endregion
//#region ../../packages/icons/src/custom/location/BelgiumMap.tsx
var al = (0, q.forwardRef)(function({ className: e, width: t = 19, height: n = 19, ...r }, i) {
	return /* @__PURE__ */ (0, K.jsxs)("svg", {
		ref: i,
		xmlns: "http://www.w3.org/2000/svg",
		viewBox: "0 0 19 19",
		width: t,
		height: n,
		fill: "none",
		className: e,
		...r,
		children: [
			/* @__PURE__ */ (0, K.jsx)("mask", {
				id: "mask0_16568_64172",
				style: { maskType: "alpha" },
				maskUnits: "userSpaceOnUse",
				x: "0",
				y: "0",
				width: "17",
				height: "17",
				children: /* @__PURE__ */ (0, K.jsx)("circle", {
					cx: "8.75",
					cy: "8.75",
					r: "8",
					fill: "#D9D9D9"
				})
			}),
			/* @__PURE__ */ (0, K.jsxs)("g", {
				mask: "url(#mask0_16568_64172)",
				children: [
					/* @__PURE__ */ (0, K.jsx)("rect", {
						x: "-3.39258",
						y: "0.75",
						width: "8.28571",
						height: "16",
						fill: "#020203"
					}),
					/* @__PURE__ */ (0, K.jsx)("rect", {
						x: "4.89355",
						y: "0.75",
						width: "8.28571",
						height: "16",
						fill: "#FFDE0D"
					}),
					/* @__PURE__ */ (0, K.jsx)("rect", {
						x: "13.084",
						y: "0.75",
						width: "8.28571",
						height: "16",
						fill: "#E41915"
					})
				]
			}),
			/* @__PURE__ */ (0, K.jsx)("rect", {
				x: "0.375",
				y: "0.375",
				width: "16.75",
				height: "16.75",
				rx: "8.375",
				stroke: "#E9ECEC",
				strokeWidth: "0.75"
			})
		]
	});
});
al.displayName = "BelgiumMap";
//#endregion
//#region ../../packages/icons/src/custom/locationIcons.ts
var ol = { be: al };
Object.keys(ol);
function sl(e) {
	return ol[e] ?? null;
}
//#endregion
//#region ../../packages/ui-react/src/components/internal/Field/field.variants.ts
var cl = B("flex flex-col gap-1.5"), ll = B([
	"field-label",
	"select-none",
	"data-[disabled]:field-label-disabled"
]), ul = B([
	"field-control",
	"transition-colors",
	"data-[hovered]:field-control-hover",
	"data-[focused]:field-control-focus",
	se(),
	"disabled:cursor-not-allowed disabled:opacity-60 disabled:bg-control-bg-disabled",
	"data-disabled:cursor-not-allowed data-disabled:opacity-60 data-disabled:bg-control-bg-disabled",
	"aria-invalid:field-control-invalid",
	"data-invalid:field-control-invalid",
	"aria-[valid=true]:field-control-valid",
	"data-valid:field-control-valid",
	"data-focused:data-invalid:field-control-focus-invalid",
	"data-[focused]:data-[valid]:field-control-focus-valid",
	"placeholder:field-placeholder"
], {
	variants: { size: {
		sm: "field-control-sm",
		md: "field-control-md",
		lg: "field-control-lg"
	} },
	defaultVariants: { size: "md" }
}), dl = B(["text-caption text-text-muted"]);
B(["field-status text-accent-error"]);
var fl = B("relative", {
	variants: { labelPlacement: {
		outside: "flex flex-col gap-2",
		inside: ["field-floating-root"],
		inline: "flex flex-row items-center gap-2"
	} },
	defaultVariants: { labelPlacement: "outside" }
}), pl = B([], {
	variants: { labelPlacement: {
		outside: "",
		inside: "ds-floating-label field-floating-label",
		inline: "shrink-0"
	} },
	defaultVariants: { labelPlacement: "outside" }
}), ml = B("field-status text-accent-error flex gap-2 items-center", {
	variants: { errorStyle: {
		inline: "",
		divider: "field-floating-status"
	} },
	defaultVariants: { errorStyle: "inline" }
});
//#endregion
//#region ../../packages/ui-react/src/icons/useBrandIcons.ts
function hl() {
	let { brand: e } = Rc(), [t, n] = (0, q.useState)(() => ({
		brand: e,
		icons: il()
	})), r = t.brand === e ? t.icons : il();
	return (0, q.useEffect)(() => {
		let t = !0;
		return rl(e).then((r) => {
			t && n({
				brand: e,
				icons: r
			});
		}), () => {
			t = !1;
		};
	}, [e]), r;
}
//#endregion
//#region ../../packages/ui-react/src/components/primitives/IconButton/IconButton.variants.ts
var gl = B([
	"inline-flex items-center justify-center shrink-0",
	"transition-colors",
	"cursor-pointer select-none",
	"data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50"
], {
	variants: {
		variant: {
			ghost: [
				"text-icon bg-transparent",
				"hover:text-icon-hover hover:bg-transparent",
				se()
			],
			surface: [
				"rounded-full",
				"bg-icon-surface text-icon",
				"hover:bg-icon-surface-hover hover:text-icon-hover",
				se()
			],
			outline: [
				"rounded-full",
				"border border-icon text-icon bg-transparent",
				"hover:bg-icon-surface-hover hover:text-icon-hover",
				se()
			]
		},
		size: {
			xs: "h-4 w-4",
			sm: "h-6 w-6",
			md: "h-8 w-8",
			lg: "h-12 w-12"
		}
	},
	defaultVariants: {
		variant: "ghost",
		size: "md"
	}
}), _l = {
	xs: "h-[--icon-button-icon-xs] w-[--icon-button-icon-xs]",
	sm: "h-[--icon-button-icon-sm] w-[--icon-button-icon-sm]",
	md: "h-[--icon-button-icon-md] w-[--icon-button-icon-md]",
	lg: "h-[--icon-button-icon-lg] w-[--icon-button-icon-lg]"
};
//#endregion
//#region ../../packages/ui-react/src/components/primitives/IconButton/IconButton.tsx
function vl({ icon: e, variant: t, size: n = "md", className: r, iconClassName: i, isDisabled: a, ref: o, ...s }) {
	let c = n ?? "md";
	return /* @__PURE__ */ (0, K.jsx)(De, {
		...s,
		ref: o,
		type: "button",
		isDisabled: a,
		className: V(gl({
			variant: t,
			size: c
		}), r),
		children: /* @__PURE__ */ (0, K.jsx)(W, {
			icon: e,
			className: V(_l[c], i)
		})
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/primitives/Menu/Menu.variants.ts
var yl = B([
	"min-w-(--trigger-width)",
	"px-4 rounded-b-md bg-surface shadow-md overflow-auto",
	"focus-visible:outline-none",
	"data-entering:animate-in data-entering:fade-in",
	"data-exiting:animate-out data-exiting:fade-out"
]), bl = B(["outline-none"]), xl = B([
	"group flex items-center justify-between gap-2",
	"cursor-pointer select-none py-8",
	"text-body-sm text-text-primary",
	"outline-none shadow-between last:shadow-none",
	"data-hovered:bg-control-option-hover",
	"data-focus-visible:bg-control-option-hover",
	se(),
	"data-disabled:cursor-not-allowed",
	"data-disabled:opacity-60",
	"data-disabled:hover:bg-transparent"
]);
//#endregion
//#region ../../packages/ui-react/src/components/primitives/Menu/Menu.tsx
function Sl({ children: e, ...t }) {
	return /* @__PURE__ */ (0, K.jsx)(tc, {
		...t,
		children: e
	});
}
function Cl({ className: e, children: t, ref: n, ...r }) {
	return /* @__PURE__ */ (0, K.jsx)(ns, {
		ref: n,
		className: V(yl(), e),
		...r,
		children: t
	});
}
function wl({ className: e, children: t, ref: n, ...r }) {
	return /* @__PURE__ */ (0, K.jsx)(rc, {
		ref: n,
		className: V(bl(), e),
		...r,
		children: t
	});
}
function Tl({ className: e, children: t, ref: n, ...r }) {
	return /* @__PURE__ */ (0, K.jsx)(cc, {
		ref: n,
		className: V(xl(), e),
		...r,
		children: t
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/internal/OverlayPanel/overlayPanel.variants.ts
var El = B([
	"flex flex-col min-h-0 gap-6 lg:gap-8",
	"overflow-hidden",
	"outline-none",
	"flex-1"
]), Dl = B(["flex items-start justify-between gap-4"]), Ol = B(["text-h3 text-text-primary", "m-0"]), kl = B(["pb-2", "text-body text-text-primary"]), Al = B(["text-body text-text-primary", "flex-1 overflow-y-auto min-h-0"]), jl = B([B([
	"inline-flex items-center justify-center",
	"rounded-full",
	"absolute top-3 right-3",
	"text-overlay-close-text bg-overlay-close hover:text-text-primary hover:bg-surface-muted",
	"cursor-pointer",
	"transition-colors",
	se()
])(), "h-12 w-12"]);
B([
	"fill-surface stroke-border-subtle",
	"block rotate-180",
	"group-data-[placement=bottom]:rotate-0",
	"group-data-[placement=left]:rotate-90",
	"group-data-[placement=right]:-rotate-90",
	"group-data-[placement=left]:-translate-x-[3.5px]",
	"group-data-[placement=right]:translate-x-[3.5px]"
]);
//#endregion
//#region ../../packages/ui-react/src/components/internal/Field/FieldLabel.tsx
function Ml({ label: e, isRequired: t, requiredIndicator: n = " *", className: r }) {
	return /* @__PURE__ */ (0, K.jsxs)(tn, {
		className: V(ll(), r),
		children: [e, t && n ? /* @__PURE__ */ (0, K.jsx)("span", {
			"aria-hidden": "true",
			className: "text-accent-error",
			children: n
		}) : null]
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/internal/Field/FieldLabelPlacement.ts
function Nl(e) {
	switch (e) {
		case "stacked": return "outside";
		case "floating": return "inside";
		case "inline": return "inline";
	}
}
function Pl({ explicit: e, brandLayout: t, componentDefault: n = "outside", hasLabel: r = !0 }) {
	if (e != null) return e;
	let i = t === void 0 ? n : Nl(t);
	return i === "inside" && !r ? "outside" : i;
}
//#endregion
//#region ../../packages/ui-react/src/components/internal/Field/FieldStructure.tsx
function Fl({ resolvedLabelPlacement: e, label: t, isRequired: n, requiredIndicator: r = " *", hasValue: i, isInvalid: a, stackClassName: o, labelClassName: s, children: c }) {
	return e === "inline" ? /* @__PURE__ */ (0, K.jsxs)(K.Fragment, { children: [t && /* @__PURE__ */ (0, K.jsx)(Ml, {
		label: t,
		isRequired: n,
		requiredIndicator: r,
		className: "shrink-0"
	}), /* @__PURE__ */ (0, K.jsx)("div", {
		className: o,
		"data-has-value": i ? "true" : "false",
		"data-invalid": a ? !0 : void 0,
		children: c
	})] }) : /* @__PURE__ */ (0, K.jsxs)("div", {
		className: o,
		"data-has-value": i ? "true" : "false",
		"data-invalid": a ? !0 : void 0,
		children: [t && /* @__PURE__ */ (0, K.jsx)(Ml, {
			label: t,
			isRequired: n,
			requiredIndicator: r,
			className: s
		}), c]
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/primitives/Select/Select.variants.ts
var Il = "min-w-0 flex-1 leading-normal", Ll = B([
	"flex items-center gap-2",
	"text-left",
	"cursor-pointer",
	"bg-white",
	"hover:bg-surface",
	"aria-expanded:select-expanded",
	se()
], {
	variants: {
		variant: {
			default: "",
			borderless: "border-transparent bg-transparent"
		},
		labelPlacement: {
			outside: "read-only:bg-control-bg h-9.5 min-h-9.5 items-center",
			inline: "read-only:bg-transparent h-9.5 min-h-9.5",
			inside: "read-only:bg-control-bg h-auto min-h-[44px] items-center field-floating-input-md field-floating-control"
		},
		size: {
			standard: "",
			compact: ""
		}
	},
	defaultVariants: {
		variant: "default",
		labelPlacement: "outside",
		size: "standard"
	}
}), Rl = B(Il), zl = B([Il, "data-placeholder:text-text-muted"]), Bl = fl, Vl = pl, Hl = ml, Ul = B([
	"select-list",
	"scrollbar-subtle",
	"data-entering:animate-in data-entering:fade-in",
	"data-exiting:animate-out data-exiting:fade-out"
]), Wl = B([
	"group flex items-center gap-3",
	"cursor-pointer select-none px-2 py-3",
	"whitespace-nowrap",
	"text-body text-text-primary",
	"outline-none shadow-between last:shadow-none",
	"data-hovered:bg-control-option-hover",
	"data-focus-visible:bg-control-option-hover",
	se(),
	"data-disabled:cursor-not-allowed",
	"data-disabled:opacity-60",
	"data-disabled:hover:bg-transparent"
]);
//#endregion
//#region ../../packages/ui-react/src/components/primitives/Select/Select.tsx
function Gl(e) {
	return typeof e == "string" || typeof e == "number" ? String(e) : (0, q.isValidElement)(e) ? Gl(e.props.children) : "";
}
function Kl({ label: e, description: t, errorMessage: n, placeholder: r = "Select…", variant: i, size: a = "standard", labelPlacement: o, value: s, defaultValue: c, onChange: l, onOpenChange: u, requiredIndicator: d = " *", isRequired: f, isInvalid: p, renderValue: m, triggerTestId: h, listBoxTestId: g, className: _, children: v, ref: y, ...b }) {
	let { components: x } = Rc(), S = x.field.layout, { selectArrow: C, error: w } = hl(), T = Pl({
		explicit: o,
		brandLayout: S,
		componentDefault: "outside",
		hasLabel: !!e
	}), E = s !== void 0, [D, O] = (0, q.useState)(c !== void 0), [k, A] = (0, q.useState)(!1), [j, M] = (0, q.useState)(), N = (0, q.useRef)(null), P = (0, q.useRef)(null), F = E ? s != null : D;
	(0, q.useLayoutEffect)(() => {
		let e = requestAnimationFrame(() => {
			let e = Array.from(P.current?.children ?? []).map((e) => e.getBoundingClientRect().width), t = Math.max(0, ...e);
			t > 0 && M(t + 34);
		});
		return () => cancelAnimationFrame(e);
	}, [v]), (0, q.useLayoutEffect)(() => {
		if (!k) return;
		let e = requestAnimationFrame(() => {
			let e = N.current?.getBoundingClientRect().width;
			e && e > 0 && M(e);
		});
		return () => cancelAnimationFrame(e);
	}, [k]);
	let I = V(ul(), Ll({
		variant: i,
		size: a,
		labelPlacement: T
	})), ee = () => m === void 0 ? /* @__PURE__ */ (0, K.jsx)(Lc, {
		className: zl(),
		children: ({ isPlaceholder: e, selectedText: t }) => e ? r : t ?? r
	}) : /* @__PURE__ */ (0, K.jsx)("span", {
		className: Rl(),
		children: m
	});
	return /* @__PURE__ */ (0, K.jsxs)(Nc, {
		ref: y,
		isRequired: f,
		isInvalid: p,
		placeholder: r,
		value: s,
		defaultValue: c,
		onChange: (e) => {
			E || O(e != null), l?.(e);
		},
		onOpenChange: (e) => {
			A(e), u?.(e);
		},
		...b,
		className: V(cl(), "w-full max-w-full", T === "inline" && "flex-row items-center gap-2", _),
		children: [
			/* @__PURE__ */ (0, K.jsxs)(Fl, {
				resolvedLabelPlacement: T,
				label: e,
				isRequired: f,
				requiredIndicator: d,
				hasValue: F,
				isInvalid: p,
				stackClassName: Bl({ labelPlacement: T }),
				labelClassName: Vl({ labelPlacement: T }),
				children: [/* @__PURE__ */ (0, K.jsxs)(De, {
					"data-testid": h,
					className: V(I, "group max-w-full"),
					style: j ? { minWidth: `${j}px` } : void 0,
					children: [ee(), /* @__PURE__ */ (0, K.jsx)(W, {
						icon: C,
						size: "unset",
						className: V("field-select-arrow shrink-0 text-text-brand transition-transform duration-base ease-standard group-aria-expanded:rotate-180", T === "inside" && "absolute right-3 top-8 -translate-y-1/2")
					})]
				}), T === "inside" && /* @__PURE__ */ (0, K.jsx)(xn, {
					className: Hl({ errorStyle: x.field.errorStyle }),
					children: n
				})]
			}),
			t && /* @__PURE__ */ (0, K.jsx)(cn, {
				slot: "description",
				className: dl(),
				children: t
			}),
			T !== "inside" && /* @__PURE__ */ (0, K.jsxs)(xn, {
				className: Hl({ errorStyle: x.field.errorStyle }),
				children: [/* @__PURE__ */ (0, K.jsx)(W, { icon: w }), n]
			}),
			/* @__PURE__ */ (0, K.jsx)("div", {
				ref: P,
				"aria-hidden": !0,
				className: "pointer-events-none absolute left-0 top-0 -z-10 h-0 w-0 overflow-hidden opacity-0",
				children: q.Children.toArray(v).map((e, t) => {
					let n = Gl(e), r = (0, q.isValidElement)(e) && e.key !== null ? e.key : `option-${t}`;
					return /* @__PURE__ */ (0, K.jsx)("span", {
						className: V(Wl(), "w-max"),
						children: n
					}, r);
				})
			}),
			/* @__PURE__ */ (0, K.jsx)(ns, {
				ref: N,
				placement: "bottom",
				shouldFlip: !1,
				offset: -1,
				className: Ul(),
				children: /* @__PURE__ */ (0, K.jsx)(ta, {
					"data-testid": g,
					className: "outline-none",
					children: v
				})
			})
		]
	});
}
function ql({ id: e, className: t, children: n, ...r }) {
	let { check: i } = hl();
	return /* @__PURE__ */ (0, K.jsxs)(aa, {
		id: e,
		textValue: typeof n == "string" || typeof n == "number" ? String(n) : void 0,
		...r,
		className: V(Wl(), t),
		children: [/* @__PURE__ */ (0, K.jsx)(W, {
			icon: i,
			size: "xs",
			className: "hidden shrink-0 text-brand group-data-selected:inline-block"
		}), /* @__PURE__ */ (0, K.jsx)("span", { children: n })]
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/internal/OverlayPanel/OverlayPanel.tsx
function Jl({ ref: e, children: t, isDismissable: n = !0, isKeyboardDismissDisabled: r, overlayClassName: i, className: a, role: o = "dialog", overlayBaseClassName: s, containerBaseClassName: c, dialogBaseClassName: l, dialogTestId: u, isOpen: d, onOpenChange: f }) {
	return /* @__PURE__ */ (0, K.jsx)(xc, {
		isOpen: d,
		onOpenChange: f,
		isDismissable: n,
		isKeyboardDismissDisabled: r,
		className: V(s, i),
		children: /* @__PURE__ */ (0, K.jsx)(yc, {
			ref: e,
			isDismissable: n,
			isKeyboardDismissDisabled: r,
			className: V(c, a),
			children: /* @__PURE__ */ (0, K.jsx)(fc, {
				role: o,
				"data-testid": u,
				className: l,
				children: t
			})
		})
	});
}
function Yl({ ref: e, className: t, children: n, baseClassName: r }) {
	return /* @__PURE__ */ (0, K.jsx)("div", {
		ref: e,
		className: V(r, t),
		children: n
	});
}
function Xl({ ref: e, className: t, children: n, level: r = 2, baseClassName: i }) {
	return /* @__PURE__ */ (0, K.jsx)(on, {
		ref: e,
		slot: "title",
		level: r,
		className: V(i, t),
		children: n
	});
}
function Zl({ ref: e, className: t, children: n, baseClassName: r }) {
	return /* @__PURE__ */ (0, K.jsx)("p", {
		ref: e,
		slot: "description",
		className: V(r, t),
		children: n
	});
}
function Ql({ ref: e, label: t = "Close", className: n, baseClassName: r, testId: i }) {
	let { close: a } = hl();
	return /* @__PURE__ */ (0, K.jsx)(vl, {
		ref: e,
		"data-testid": i,
		slot: "close",
		icon: a,
		"aria-label": t,
		className: V(r, n)
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/Modal/Modal.variants.ts
var $l = B([
	"fixed inset-0 z-modal",
	"flex justify-center",
	"bg-black/50",
	"motion-safe:data-[entering]:animate-[ds-backdrop-fade-in_240ms_cubic-bezier(0.2,0,0,1)]",
	"motion-safe:data-[exiting]:animate-[ds-backdrop-fade-out_200ms_cubic-bezier(0.4,0,1,1)]",
	"motion-reduce:data-[entering]:animate-none motion-reduce:data-[exiting]:animate-none"
], {
	variants: { position: {
		center: "items-center",
		top: "items-start pt-8"
	} },
	defaultVariants: { position: "center" }
}), eu = B([
	"relative w-full",
	"bg-surface text-text-primary",
	"rounded-lg shadow-xl",
	"max-h-overlay-safe",
	"flex flex-col",
	"outline-none",
	"motion-safe:data-[entering]:animate-[ds-modal-grow-in_260ms_cubic-bezier(0.16,1,0.3,1)]",
	"motion-safe:data-[exiting]:animate-[ds-modal-grow-out_180ms_cubic-bezier(0.4,0,1,1)]",
	"motion-reduce:data-[entering]:animate-none motion-reduce:data-[exiting]:animate-none"
], {
	variants: { size: {
		sm: "mx-9 px-6 pb-6 max-w-sm",
		md: "mx-9 px-10 py-15 lg:py-12.5 max-w-md",
		lg: "mx-9 px-10 py-15 lg:py-12.5 max-w-lg",
		xl: "mx-9 px-12 py-15 lg:py-12.5 max-w-2xl",
		fullscreen: "h-screen max-h-screen max-w-none rounded-none px-10 py-15 lg:py-12.5"
	} },
	defaultVariants: { size: "md" }
}), tu = B([El()]), nu = B([Dl()]), ru = B([Ol()]), iu = B([kl()]), au = B([Al()]), ou = B([], {
	variants: { direction: {
		row: "flex items-center justify-end gap-2",
		stacked: "flex flex-col gap-4 [&>*]:w-full"
	} },
	defaultVariants: { direction: "row" }
}), su = B([jl()]);
//#endregion
//#region ../../packages/ui-react/src/components/composed/Modal/Modal.tsx
function cu({ size: e, isDismissable: t = !0, isKeyboardDismissDisabled: n, role: r = "dialog", className: i, overlayClassName: a, position: o, children: s, ref: c, isOpen: l, onOpenChange: u, dialogTestId: d }) {
	return /* @__PURE__ */ (0, K.jsx)(Jl, {
		ref: c,
		isDismissable: t,
		isKeyboardDismissDisabled: n,
		overlayClassName: a,
		className: i,
		role: r,
		overlayBaseClassName: $l({ position: o }),
		containerBaseClassName: eu({ size: e }),
		dialogBaseClassName: tu(),
		dialogTestId: d,
		isOpen: l,
		onOpenChange: u,
		children: s
	});
}
function lu({ className: e, children: t, ref: n }) {
	return /* @__PURE__ */ (0, K.jsx)(Yl, {
		ref: n,
		className: e,
		baseClassName: nu(),
		children: t
	});
}
function uu({ level: e = 2, className: t, children: n, ref: r }) {
	return /* @__PURE__ */ (0, K.jsx)(Xl, {
		ref: r,
		level: e,
		className: t,
		baseClassName: ru(),
		children: n
	});
}
function du({ className: e, children: t, ref: n }) {
	return /* @__PURE__ */ (0, K.jsx)(Zl, {
		ref: n,
		className: e,
		baseClassName: iu(),
		children: t
	});
}
function fu({ className: e, children: t, ref: n }) {
	return /* @__PURE__ */ (0, K.jsx)(Yl, {
		ref: n,
		className: e,
		baseClassName: au(),
		children: t
	});
}
function pu({ direction: e, className: t, children: n, ref: r }) {
	return /* @__PURE__ */ (0, K.jsx)(Yl, {
		ref: r,
		className: t,
		baseClassName: ou({ direction: e }),
		children: n
	});
}
function mu({ label: e = "Close", className: t, ref: n, testId: r }) {
	return /* @__PURE__ */ (0, K.jsx)(Ql, {
		ref: n,
		label: e,
		className: t,
		baseClassName: su(),
		testId: r
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderShell.variants.ts
var hu = {
	root: "header-shell lg:header-desktop-height sticky top-0 z-sticky border-b backdrop-blur",
	grid: "lg:layout-container grid grid-cols-3 lg:grid-cols-5 items-stretch gap-x-6 gap-y-0 lg:gap-y-6 h-full",
	menuCell: "header-main-row lg:header-desktop-height justify-self-start flex items-center lg:hidden",
	contextCell: "header-secondary-row px-4 lg:px-0 lg:header-desktop-height justify-self-start flex items-stretch row-start-2 lg:row-start-auto col-span-3 lg:col-span-2",
	rightCluster: "justify-self-end hidden lg:flex items-center gap-6 col-span-2"
}, gu = {
	item: "header-nav-item p-4 w-full justify-between",
	back: "header-nav-item p-4 w-full grid grid-cols-9 items-center gap-2 bg-surface-pressed text-text-primary rounded-none border-b border-border-subtle-30 uppercase",
	secondaryPanel: "absolute inset-0 z-popover overflow-y-auto scrollbar-subtle bg-surface header-mobile-panel motion-safe:animate-[header-menu-level-in_200ms_ease-out]"
}, _u = { trigger: "border border-border-light rounded-md p-2.5 flex items-center gap-2 group-hover:border-border-dark group-aria-expanded:border-border-dark" }, vu = { trigger: "link-standalone hover:text-brand-hover group p-4 rounded-none lg:h-full aria-expanded:bg-surface-pressed" };
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderShell.tsx
function yu({ brand: e, brandHomeHref: t, brandLabel: n, currentPath: r, onNavigate: i, getConfirmContent: a, startSlot: o, navSlot: s, endSlot: c }) {
	let l = n ?? pe[e].displayName, u = (0, q.useRef)(null), [d, f] = (0, q.useState)(null), [p, m] = (0, q.useState)(null), [h, g] = (0, q.useState)(!1), _ = (0, q.useCallback)((e) => {
		if (e === r) return;
		let t = a?.(r, e) ?? null;
		if (!t) {
			i(e);
			return;
		}
		f(e), m(t), g(!0);
	}, [
		r,
		a,
		i
	]), v = (0, q.useCallback)((e) => {
		g(e);
	}, []);
	return (0, q.useLayoutEffect)(() => {
		if (!u.current) return;
		let e = () => {
			document.documentElement.style.setProperty("--layout-header-offset", `${u.current.offsetHeight}px`);
		};
		e();
		let t = new ResizeObserver(e);
		return t.observe(u.current), () => t.disconnect();
	}, []), /* @__PURE__ */ (0, K.jsxs)(K.Fragment, { children: [/* @__PURE__ */ (0, K.jsx)("header", {
		ref: u,
		className: hu.root,
		children: /* @__PURE__ */ (0, K.jsxs)("div", {
			className: hu.grid,
			children: [
				/* @__PURE__ */ (0, K.jsx)("div", {
					className: hu.menuCell,
					children: o?.(_)
				}),
				/* @__PURE__ */ (0, K.jsx)("div", {
					className: hu.contextCell,
					children: s?.(_)
				}),
				/* @__PURE__ */ (0, K.jsx)(me, {
					href: t,
					"aria-label": `${l} home`,
					"data-testid": "brand-logo",
					className: "header-brand-link",
					children: /* @__PURE__ */ (0, K.jsx)(Me, {
						brand: e,
						title: l,
						className: "h-2 w-2 md:h-2.5 md:w-2.5"
					})
				}),
				/* @__PURE__ */ (0, K.jsx)("div", {
					className: hu.rightCluster,
					children: c?.(_)
				})
			]
		})
	}), /* @__PURE__ */ (0, K.jsx)(cu, {
		role: "alertdialog",
		dialogTestId: "view-change-dialog",
		size: "md",
		isOpen: h,
		onOpenChange: v,
		children: ({ close: e }) => p ? /* @__PURE__ */ (0, K.jsxs)(K.Fragment, { children: [
			/* @__PURE__ */ (0, K.jsxs)(lu, { children: [/* @__PURE__ */ (0, K.jsx)(uu, {
				level: 2,
				children: p.title
			}), /* @__PURE__ */ (0, K.jsx)(mu, {
				label: p.closeLabel,
				testId: "view-change-close"
			})] }),
			/* @__PURE__ */ (0, K.jsx)(fu, { children: /* @__PURE__ */ (0, K.jsx)(du, { children: p.description }) }),
			/* @__PURE__ */ (0, K.jsxs)(pu, {
				direction: "stacked",
				children: [/* @__PURE__ */ (0, K.jsx)(Oe, {
					"data-testid": "view-change-confirm",
					variant: "primary",
					fullWidth: !0,
					onPress: () => {
						let t = d;
						e(), t && i(t);
					},
					children: p.confirmLabel
				}), /* @__PURE__ */ (0, K.jsx)(Oe, {
					"data-testid": "view-change-cancel",
					variant: "secondary",
					fullWidth: !0,
					onPress: () => {
						e();
					},
					children: p.cancelLabel
				})]
			})
		] }) : null
	})] });
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderNav.tsx
function bu({ "aria-label": e, children: t, className: n }) {
	return /* @__PURE__ */ (0, K.jsx)("nav", {
		"aria-label": e,
		className: V("flex h-full items-stretch gap-6 lg:gap-10", n),
		children: t
	});
}
function xu({ children: e, href: t, active: n = !1, tone: r = "default", onNavigate: i, className: a, buttonVariant: o = "inline", buttonSize: s, buttonProps: c, linkProps: l, itemProps: u }) {
	let d = V(r === "muted" ? "header-nav-item-muted" : "header-nav-item", n && "header-nav-active-indicator", a);
	return t ? /* @__PURE__ */ (0, K.jsx)(me, {
		...l,
		href: t,
		variant: "standalone",
		size: "auto",
		"aria-current": n ? "page" : void 0,
		className: d,
		onClick: (e) => {
			!i || n || (e.preventDefault(), i());
		},
		onKeyDown: (e) => {
			!i || n || e.key !== "Enter" || (e.preventDefault(), i());
		},
		...u,
		children: e
	}) : /* @__PURE__ */ (0, K.jsx)(Oe, {
		...c,
		variant: o,
		size: s,
		"aria-current": n ? "page" : void 0,
		className: d,
		onPress: i ?? c?.onPress,
		...u,
		children: e
	});
}
function Su({ items: e, "aria-label": t, onNavigate: n }) {
	let { chevron: r } = hl();
	return /* @__PURE__ */ (0, K.jsx)(bu, {
		"aria-label": t,
		children: e.map((e) => e.kind === "current" ? /* @__PURE__ */ (0, K.jsx)("span", {
			"aria-current": "page",
			className: "header-nav-item header-nav-active-indicator",
			children: e.label
		}, e.key) : e.kind === "back" ? /* @__PURE__ */ (0, K.jsxs)(me, {
			href: e.href,
			variant: "standalone",
			size: "auto",
			className: "header-nav-item header-nav-item-muted",
			children: [/* @__PURE__ */ (0, K.jsx)(W, {
				icon: r,
				size: "sm",
				className: "rotate-90 h-2.5 w-2.5",
				"aria-hidden": "true"
			}), e.label]
		}, e.key) : /* @__PURE__ */ (0, K.jsx)(xu, {
			href: e.href,
			active: e.active,
			tone: "muted",
			itemProps: e.itemProps,
			onNavigate: n ? () => n(e.href) : void 0,
			children: e.label
		}, e.key))
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderAction.tsx
function Cu({ href: e, children: t, "aria-label": n }) {
	return /* @__PURE__ */ (0, K.jsx)(xu, {
		href: e,
		linkProps: { "aria-label": n },
		children: t
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderBrandMenu.tsx
function wu({ brands: e, label: t, onSelect: n, renderMode: r = "menu" }) {
	let [i, a] = (0, q.useTransition)(), { chevron: o } = hl();
	if (e.length === 0) return null;
	let s = /* @__PURE__ */ (0, K.jsx)(wl, {
		"aria-label": t,
		onAction: (t) => {
			typeof t != "string" || !e.includes(t) || a(() => {
				n(t);
			});
		},
		children: e.map((e) => /* @__PURE__ */ (0, K.jsx)(Tl, {
			id: e,
			textValue: pe[e].displayName,
			"data-testid": "brand-option",
			children: /* @__PURE__ */ (0, K.jsx)(Me, {
				brand: e,
				height: 10
			})
		}, e))
	});
	return r === "list" ? s : /* @__PURE__ */ (0, K.jsxs)(Sl, { children: [/* @__PURE__ */ (0, K.jsxs)(xu, {
		buttonVariant: "composed",
		className: vu.trigger,
		buttonProps: {
			"aria-label": t,
			isDisabled: i
		},
		itemProps: { "data-testid": "header-brand-switcher" },
		children: [
			/* @__PURE__ */ (0, K.jsx)(W, {
				icon: o,
				size: "xs",
				className: "transition-transform duration-base ease-standard group-aria-expanded:rotate-180"
			}),
			" ",
			t
		]
	}), /* @__PURE__ */ (0, K.jsx)(Cl, {
		offset: 0,
		children: s
	})] });
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderLocaleMenu.tsx
function Tu({ locale: e, label: t, children: n }) {
	let { globe: r, chevron: i } = hl();
	return /* @__PURE__ */ (0, K.jsxs)(Sl, { children: [/* @__PURE__ */ (0, K.jsx)(xu, {
		className: "group h-full",
		buttonVariant: "composed",
		buttonProps: { "aria-label": t },
		itemProps: { "data-testid": "header-locale-switcher" },
		children: /* @__PURE__ */ (0, K.jsxs)("div", {
			className: _u.trigger,
			children: [
				e,
				/* @__PURE__ */ (0, K.jsx)(W, {
					icon: r,
					className: "h-4.5 w-4.5 rounded-full",
					"aria-hidden": "true"
				}),
				/* @__PURE__ */ (0, K.jsx)(W, {
					icon: i,
					size: "xs",
					className: "transition-transform duration-base ease-standard group-aria-expanded:rotate-180"
				})
			]
		})
	}), /* @__PURE__ */ (0, K.jsx)(Cl, {
		offset: 1,
		children: n
	})] });
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderLocaleForm.tsx
function Eu({ activeLocale: e, locales: t, onSubmit: n, market: r, localeLabel: i, confirmLabel: a, confirmButtonRef: o, className: s, isLoading: c }) {
	let [l, u] = (0, q.useState)(e), [d, f] = (0, q.useState)(e);
	e !== d && (f(e), u(e));
	let p = l !== e, m = r?.icon ? sl(r.icon) : null;
	return /* @__PURE__ */ (0, K.jsxs)("form", {
		className: V("space-y-6", s),
		onSubmit: (e) => {
			e.preventDefault(), p && !c && n(l);
		},
		children: [
			r ? /* @__PURE__ */ (0, K.jsxs)("div", {
				className: "flex gap-2",
				children: [m ? /* @__PURE__ */ (0, K.jsx)(W, {
					icon: m,
					size: "md",
					"aria-hidden": "true"
				}) : null, /* @__PURE__ */ (0, K.jsx)("span", {
					className: "text-text-secondary text-sm font-medium",
					children: r.label
				})]
			}) : null,
			/* @__PURE__ */ (0, K.jsx)(Kl, {
				"aria-label": i,
				label: i,
				value: l,
				triggerTestId: "locale-select",
				onChange: (e) => {
					typeof e == "string" && t.some((t) => t.id === e) && u(e);
				},
				children: t.map((e) => /* @__PURE__ */ (0, K.jsx)(ql, {
					id: e.id,
					"data-testid": "locale-option",
					children: e.label
				}, e.id))
			}),
			/* @__PURE__ */ (0, K.jsx)(Oe, {
				ref: o,
				"data-testid": "locale-confirm",
				type: "submit",
				variant: "primary",
				fullWidth: !0,
				isLoading: c,
				isDisabled: !p || c,
				children: a
			})
		]
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderMobileNav.tsx
function Du({ label: e, buttonId: t, onPress: n, href: r, disabled: i, icon: a, badge: o, badgeTone: s, hasSubmenu: c, buttonRef: l, testId: u }) {
	let { chevron: d } = hl();
	return i ? null : /* @__PURE__ */ (0, K.jsxs)(Oe, {
		id: t,
		"data-testid": u,
		...r ? { href: r } : {
			ref: l,
			onClick: n
		},
		variant: "composed",
		className: gu.item,
		children: [/* @__PURE__ */ (0, K.jsxs)("div", {
			className: "flex items-center gap-2",
			children: [a ? /* @__PURE__ */ (0, K.jsx)(W, {
				icon: a,
				size: "md",
				badge: o,
				badgeTone: s,
				"aria-hidden": "true"
			}) : null, e]
		}), c ? /* @__PURE__ */ (0, K.jsx)(W, {
			icon: d,
			size: "xs",
			className: "-rotate-90",
			"aria-hidden": "true"
		}) : null]
	});
}
function Ou({ label: e, onPress: t, buttonRef: n }) {
	let { arrow: r } = hl();
	return /* @__PURE__ */ (0, K.jsxs)(Oe, {
		ref: n,
		variant: "composed",
		onClick: t,
		className: gu.back,
		children: [/* @__PURE__ */ (0, K.jsx)(W, {
			icon: r,
			size: "sm",
			"aria-hidden": "true"
		}), /* @__PURE__ */ (0, K.jsx)("span", {
			className: "col-span-7",
			children: e
		})]
	});
}
//#endregion
//#region ../../packages/ui-react/src/components/composed/HeaderShell/HeaderMobileMenu.tsx
function ku({ openLabel: e, closeLabel: t, label: n, navigationLabel: r, items: i, currentPath: a }) {
	let [o, s] = (0, q.useState)(!1), [c, l] = (0, q.useState)(null), u = (0, q.useId)(), d = (0, q.useRef)(null), f = (0, q.useRef)(null), p = (0, q.useRef)(a), { close: m, menu: h } = hl(), g = i.find((e) => e.key === c)?.panel, _ = (0, q.useCallback)(() => {
		s(!1), l(null);
	}, []), v = (0, q.useCallback)(() => {
		_(), requestAnimationFrame(() => d.current?.focus());
	}, [_]);
	return (0, q.useEffect)(() => {
		if (p.current === a || (p.current = a, !o)) return;
		let e = requestAnimationFrame(_);
		return () => cancelAnimationFrame(e);
	}, [
		_,
		a,
		o
	]), (0, q.useEffect)(() => {
		if (!o) return;
		let e = (e) => {
			if (e.key === "Escape") {
				v();
				return;
			}
			if (e.key !== "Tab") return;
			let t = f.current, n = t ? Array.from(t.querySelectorAll("a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex=\"-1\"])")).filter((e) => !e.closest("[inert], [aria-hidden=\"true\"]")) : [], r = [d.current, ...n].filter((e) => !!e);
			if (!r.length) return;
			let i = r.indexOf(document.activeElement);
			e.shiftKey && i <= 0 ? (e.preventDefault(), r.at(-1)?.focus()) : !e.shiftKey && (i === -1 || i === r.length - 1) && (e.preventDefault(), r[0]?.focus());
		};
		return document.addEventListener("keydown", e), () => document.removeEventListener("keydown", e);
	}, [v, o]), (0, q.useEffect)(() => {
		if (!c) return;
		let e = requestAnimationFrame(() => {
			let e = f.current?.querySelector(`[data-header-mobile-panel="${c}"]`);
			(e?.querySelector("[data-header-mobile-panel-content]") ?? e)?.querySelector("a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex=\"-1\"])")?.focus();
		});
		return () => cancelAnimationFrame(e);
	}, [c]), /* @__PURE__ */ (0, K.jsxs)(K.Fragment, { children: [/* @__PURE__ */ (0, K.jsxs)(xu, {
		buttonVariant: "composed",
		className: "lg:hidden h-full px-4 rounded-none text-on-surface-pressed aria-expanded:bg-surface-pressed aria-expanded:text-on-surface-pressed",
		itemProps: { "data-testid": "header-mobile-menu" },
		buttonProps: {
			ref: d,
			"aria-expanded": o,
			"aria-controls": u,
			"aria-label": o ? t : e,
			onClick: () => {
				o ? v() : s(!0);
			}
		},
		children: [/* @__PURE__ */ (0, K.jsx)(W, {
			icon: o ? m : h,
			size: "sm"
		}), /* @__PURE__ */ (0, K.jsx)("span", { children: n })]
	}), o ? /* @__PURE__ */ (0, K.jsxs)("section", {
		id: u,
		ref: f,
		className: V("lg:hidden absolute left-0 right-0 z-overlay bg-surface", "header-mobile-panel border-b header-main-row-panel overflow-y-auto scrollbar-subtle", "motion-safe:animate-[header-menu-slide-in_300ms_ease-out]"),
		children: [/* @__PURE__ */ (0, K.jsx)("nav", {
			"aria-label": r,
			"aria-hidden": g ? !0 : void 0,
			inert: g ? !0 : void 0,
			children: /* @__PURE__ */ (0, K.jsx)("ul", {
				className: "flex flex-col py-2 pl-4 pr-6",
				children: i.map((e) => /* @__PURE__ */ (0, K.jsx)("li", {
					className: e.divider ? "shadow-between" : void 0,
					children: /* @__PURE__ */ (0, K.jsx)(Du, {
						buttonId: e.panel ? `${u}-${e.key}` : void 0,
						label: e.label,
						href: e.href,
						onPress: e.panel ? () => l(e.key) : e.onPress,
						disabled: e.disabled,
						icon: e.icon,
						badge: e.badge,
						badgeTone: e.badgeTone,
						hasSubmenu: !!e.panel,
						testId: e.testId
					})
				}, e.key))
			})
		}), g ? /* @__PURE__ */ (0, K.jsxs)("nav", {
			"aria-label": g.navigationLabel,
			"data-header-mobile-panel": c,
			className: gu.secondaryPanel,
			children: [/* @__PURE__ */ (0, K.jsx)(Ou, {
				label: g.backLabel,
				onPress: () => {
					let e = c;
					l(null), e && requestAnimationFrame(() => {
						document.getElementById(`${u}-${e}`)?.focus();
					});
				}
			}), /* @__PURE__ */ (0, K.jsx)("div", {
				"data-header-mobile-panel-content": !0,
				className: "p-10",
				children: g.renderContent(_)
			})]
		}) : null]
	}) : null] });
}
//#endregion
//#region src/mount-header.tsx
var Au = _e(), ju = {
	title: "Change view?",
	description: "Leaving this page will discard unsaved changes.",
	confirmLabel: "Continue",
	cancelLabel: "Stay here",
	closeLabel: "Close"
}, Mu = {
	contextNavigation: [],
	brands: [],
	otherLinks: [],
	languages: []
};
function Nu({ content: e }) {
	let { map: t } = hl();
	return /* @__PURE__ */ (0, K.jsx)(ku, {
		openLabel: "Open menu",
		closeLabel: "Close menu",
		label: "Menu",
		navigationLabel: "Primary navigation",
		items: [...e.brands.length ? [{
			key: "brands",
			label: "Our other brands",
			panel: {
				navigationLabel: "Our other brands",
				backLabel: "Our other brands",
				renderContent: () => /* @__PURE__ */ (0, K.jsx)(wu, {
					brands: e.brands,
					label: "Our other brands",
					renderMode: "list",
					onSelect: (e) => {
						window.location.href = `/${e}/en_gb`;
					}
				})
			}
		}] : [], ...e.otherLinks.map((e) => ({
			key: e.id,
			label: e.label,
			href: e.href,
			icon: e.id === "retailers" ? t : void 0
		}))]
	});
}
function Pu({ brand: e, content: t }) {
	let n = window.location.pathname, r = t.contextNavigation.map((e) => ({
		...e,
		active: e.kind === "link" && n === new URL(e.href, window.location.origin).pathname
	}));
	return /* @__PURE__ */ (0, K.jsx)(M, {
		brand: e,
		children: /* @__PURE__ */ (0, K.jsx)(yu, {
			brand: e,
			brandHomeHref: t.logoHref ?? "",
			currentPath: n,
			onNavigate: (e) => {
				window.location.href = e;
			},
			getConfirmContent: () => ju,
			startSlot: () => /* @__PURE__ */ (0, K.jsx)(Nu, { content: t }),
			navSlot: (e) => /* @__PURE__ */ (0, K.jsx)(Su, {
				items: r,
				"aria-label": "Primary",
				onNavigate: e
			}),
			endSlot: (e) => /* @__PURE__ */ (0, K.jsxs)(K.Fragment, { children: [
				t.brands.length ? /* @__PURE__ */ (0, K.jsx)(wu, {
					brands: t.brands,
					label: "Our other brands",
					onSelect: (t) => e(`/${t}/en_gb`)
				}) : null,
				t.otherLinks.map((e) => /* @__PURE__ */ (0, K.jsx)(Cu, {
					href: e.href,
					children: e.label
				}, e.id)),
				t.languages.length ? /* @__PURE__ */ (0, K.jsx)(Tu, {
					locale: t.languages[0].label,
					label: "Select language",
					children: /* @__PURE__ */ (0, K.jsx)(Eu, {
						activeLocale: t.languages[0].id,
						locales: t.languages.map(({ id: e, label: t }) => ({
							id: e,
							label: t
						})),
						localeLabel: "Language",
						confirmLabel: "Confirm",
						onSubmit: (n) => {
							let r = t.languages.find((e) => e.id === n);
							r && e(r.href);
						},
						className: "p-8 min-w-60"
					})
				}) : null
			] })
		})
	});
}
var Fu = /* @__PURE__ */ new WeakMap();
function Iu(e, t = "range-rover", n = Mu) {
	let r = Fu.get(e);
	return r || (r = (0, Au.createRoot)(e), Fu.set(e, r)), r.render(/* @__PURE__ */ (0, K.jsx)(Pu, {
		brand: t,
		content: n
	})), () => Lu(e);
}
function Lu(e) {
	let t = Fu.get(e);
	t && (t.unmount(), Fu.delete(e));
}
//#endregion
export { Iu as mountHeader, Lu as unmountHeader };
